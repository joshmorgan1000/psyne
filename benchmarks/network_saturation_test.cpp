...
void init_thread_buffer_pool(size_t buffer_size) {
    if (!message_buffer_pool) {
        message_buffer_pool =
            std::make_unique<BufferPool>(buffer_size, 64); // 64 initial buffers
    }
}
...
void server_connection_worker(uint32_t conn_id, const NetworkTestConfig &config,
                              ConnectionStats &stats) {
    try {
        // Initialize buffer pool for this thread
        size_t max_message_size =
            sizeof(NetworkTestMessage) + config.message_size;
        ...
        for (size_t i = 0; i < config.messages_per_conn; ++i) {
            ...
            if (msg->connection_id != conn_id) {
                log_error("[Server ", conn_id,
                          "] Wrong connection ID: expected ", conn_id, ", got ",
                          msg->connection_id);
                stats.errors++;
            }
            ...
            stats.bytes_received +=
                sizeof(NetworkTestMessage) + msg->payload_size;
            ...
            if (config.bidirectional) {
                ...
                if (buffer && buffer->capacity >= reply_size) {
                    ...
                    auto *reply_msg =
                        reinterpret_cast<NetworkTestMessage *>(buffer->ptr());
                    ...
                    std::memcpy(reply_msg->payload, msg->payload,
                                msg->payload_size);
                    ...
                    // Buffer automatically returns to pool when 'buffer' goes
                    // out of scope
                } else {
                    log_error("[Server ", conn_id,
                              "] Failed to get buffer for reply");
                    stats.errors++;
                }
            }
        }
...
void client_connection_worker(uint32_t conn_id, const NetworkTestConfig &config,
                              ConnectionStats &stats) {
    try {
        // Initialize buffer pool for this thread
        size_t max_message_size =
        ...
        log_info("[Client ", conn_id, "] Connecting to ", config.remote_host,
                 ":", (config.port + conn_id));
        ...
        for (size_t i = 0; i < config.messages_per_conn; ++i) {
            ...
            if (buffer && buffer->capacity >= total_size) {
                ...
                // Buffer automatically returns to pool when 'buffer' goes out
                // of scope
            } else {
                log_error("[Client ", conn_id,
                          "] Failed to get buffer for message construction");
                stats.errors++;
                break;
            }
...
void print_statistics(const std::vector<ConnectionStats> &all_stats,
    ...
    for (const auto &stats : all_stats) {
        ...
        all_latencies.insert(all_latencies.end(), stats.latencies.begin(),
                             stats.latencies.end());
    ...
    auto duration =
        duration_cast<milliseconds>(latest_end - earliest_start).count();
    ...
    log_info("  Total messages: ",
             config.num_connections * config.messages_per_conn);
    ...
    if (duration > 0) {
        ...
        double send_bandwidth =
            (total_bytes_sent / 1024.0 / 1024.0) / (duration / 1000.0);
        double recv_bandwidth =
            (total_bytes_received / 1024.0 / 1024.0) / (duration / 1000.0);

        log_info("  Send rate: ", std::fixed, std::setprecision(2), send_rate,
                 " msg/s");
        log_info("  Receive rate: ", std::fixed, std::setprecision(2),
                 recv_rate, " msg/s");
        log_info("  Send bandwidth: ", std::fixed, std::setprecision(2),
                 send_bandwidth, " MB/s");
        log_info("  Receive bandwidth: ", std::fixed, std::setprecision(2),
                 recv_bandwidth, " MB/s");
        log_info("  Total bandwidth: ", std::fixed, std::setprecision(2),
                 (send_bandwidth + recv_bandwidth), " MB/s");
    }
    ...
    if (!all_latencies.empty()) {
        ...
        log_info("  Median: ", all_latencies[all_latencies.size() / 2] / 1000.0,
                 " μs");
        log_info(
            "  90th: ", all_latencies[all_latencies.size() * 90 / 100] / 1000.0,
            " μs");
        log_info(
            "  99th: ", all_latencies[all_latencies.size() * 99 / 100] / 1000.0,
            " μs");
...
void print_usage(const char *program) {
    ...
    log_info("Options:");
    log_info("  --host <hostname>      Remote host (client mode, default: "
             "localhost)");
    log_info("  --port <port>          Base port number (default: 10000)");
    log_info(
        "  --connections <num>    Number of parallel connections (default: 4)");
    log_info("  --size <bytes>         Message payload size (default: 1024)");
    log_info(
        "  --count <num>          Messages per connection (default: 100000)");
    ...
    log_info("  Client: ", program,
             " client --host 192.168.1.100 --port 10000 --connections 8");
}
...
int main(int argc, char *argv[]) {
    ...
    if (config.is_server) {
        ...
        for (size_t i = 0; i < config.num_connections; ++i) {
            workers.emplace_back(server_connection_worker, i, std::ref(config),
                                 std::ref(all_stats[i]));
        }
    } else {
        ...
        for (size_t i = 0; i < config.num_connections; ++i) {
            workers.emplace_back(client_connection_worker, i, std::ref(config),
                                 std::ref(all_stats[i]));
        }
    }
