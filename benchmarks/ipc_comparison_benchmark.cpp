...
protected:
    ...
    void generate_comparison_report() {
        ...
        std::cout << std::left << std::setw(20) << "Mechanism" << std::setw(12)
                  << "Latency(μs)" << std::setw(15) << "Throughput(MB/s)"
                  << std::setw(15) << "Msg Rate(K/s)" << std::setw(12)
                  << "Msg Size"
                  << "Notes\n";
        std::cout << std::string(80, '-') << "\n";
        ...
        for (const auto &result : results_) {
            std::cout << std::left << std::setw(20) << result.mechanism
                      ...
                      << result.latency_us << std::setw(15) << std::fixed
                      << std::setprecision(1) << result.throughput_mbps
                      << std::setw(15) << std::fixed << std::setprecision(1)
                      << (result.message_rate / 1000.0) << std::setw(12)
                      << result.message_size << result.notes << "\n";
        }

        // Find Psyne results for comparison
        auto psyne_memory = std::find_if(
            results_.begin(), results_.end(), [](const BenchmarkResult &r) {
                return r.mechanism == "Psyne (Memory)";
            });
            ...
            for (const auto &result : results_) {
                if (result.mechanism.find("Psyne") == std::string::npos) {
                    double latency_advantage =
                        result.latency_us / psyne_memory->latency_us;
                    double throughput_advantage =
                        psyne_memory->throughput_mbps / result.throughput_mbps;
...
TEST_F(IPCComparisonBenchmark, PsyneMemoryChannel) {
    ...
    double throughput_mbps =
        (num_messages * message_size) / (duration * 1024 * 1024);
...
TEST_F(IPCComparisonBenchmark, PsyneIPCChannel) {
    ...
    try {
        auto channel =
            psyne::Channel::create("ipc://benchmark_ipc", 1024 * 1024);
...
TEST_F(IPCComparisonBenchmark, TCPLocalhost) {
    ...
    if (server_fd == -1) {
        add_result("TCP Localhost", 0, 0, 0, message_size,
...
TEST_F(IPCComparisonBenchmark, NamedPipes) {
    ...
    if (mkfifo(fifo_path, 0666) == -1) {
        add_result("Named Pipes", 0, 0, 0, message_size,
