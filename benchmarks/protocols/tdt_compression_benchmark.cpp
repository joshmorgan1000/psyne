 ...
 */
BenchmarkResults run_benchmark(const BenchmarkConfig& config, TensorDataGenerator::DataType data_type) {
    BenchmarkResults results;
    results.total_iterations = config.iterations;
    
    size_t tensor_size = config.width * config.height * config.channels * sizeof(float);
    std::vector<float> tensor_data(config.width * config.height * config.channels);
    
    TDTCompressionProtocol protocol;
    
    log_info("Running benchmark: ", config.name);
    log_info("  Tensor size: ", config.width, "x", config.height, "x", config.channels, 
             " (", tensor_size / 1024, " KB)");
    ...
    for (size_t iter = 0; iter < config.iterations; ++iter) {
        ...
        TensorDataGenerator::fill_tensor(tensor_data.data(), tensor_data.size(), data_type);
        ...
        auto compression_time = std::chrono::duration<double, std::milli>(compression_end - start_time);
        ...
        auto decompression_time = std::chrono::duration<double, std::milli>(decode_end - decode_start);
        double decompression_ms = decompression_time.count();
        
        // Verify correctness
        bool correct = (decoded.size() == tensor_size) && 
        ...
        // Calculate metrics
        double compression_ratio = static_cast<double>(tensor_size) / encoded.size();
        double compression_speed = (tensor_size / 1024.0 / 1024.0) / (compression_ms / 1000.0);
        double decompression_speed = (tensor_size / 1024.0 / 1024.0) / (decompression_ms / 1000.0);
        ...
        // Track min/max times
        results.min_compression_time_ms = std::min(results.min_compression_time_ms, compression_ms);
        results.max_compression_time_ms = std::max(results.max_compression_time_ms, compression_ms);
        results.min_decompression_time_ms = std::min(results.min_decompression_time_ms, decompression_ms);
        results.max_decompression_time_ms = std::max(results.max_decompression_time_ms, decompression_ms);
    ...
    results.avg_compression_speed_mbps = total_compression_time / config.iterations;
    results.avg_decompression_speed_mbps = total_decompression_time / config.iterations;
...
void print_results(const BenchmarkConfig& config, const BenchmarkResults& results) {
    ...
    log_info("  Compression Time:      ", std::fixed, std::setprecision(2), 
             results.min_compression_time_ms, " - ", results.max_compression_time_ms, " ms");
    log_info("  Decompression Time:    ", std::fixed, std::setprecision(2), 
             results.min_decompression_time_ms, " - ", results.max_decompression_time_ms, " ms");
    log_info("  Correctness:           ", results.all_correct ? "✅ PASS" : "❌ FAIL");
...
void run_comprehensive_benchmark() {
    ...
    std::vector<std::pair<BenchmarkConfig, TensorDataGenerator::DataType>> benchmarks = {
        // Small tensors (edge cases)
        {{"Small Dense Layer", 32, 32, 16, "weights", 50}, TensorDataGenerator::DataType::WEIGHTS},
        {{"Small Gradient", 64, 64, 8, "gradients", 50}, TensorDataGenerator::DataType::GRADIENTS},
        
        // Medium tensors (common sizes)
        {{"Conv Layer Weights", 128, 128, 32, "weights", 30}, TensorDataGenerator::DataType::WEIGHTS},
        {{"Feature Maps", 256, 256, 64, "activations", 30}, TensorDataGenerator::DataType::ACTIVATIONS},
        {{"Sparse Gradients", 256, 256, 32, "gradients", 30}, TensorDataGenerator::DataType::GRADIENTS},
        
        // Large tensors (challenging sizes)
        {{"Large Conv Weights", 512, 512, 128, "weights", 10}, TensorDataGenerator::DataType::WEIGHTS},
        {{"High-Res Feature Maps", 1024, 1024, 16, "activations", 10}, TensorDataGenerator::DataType::ACTIVATIONS},
        
        // Worst case (random data)
        {{"Random Data (Worst Case)", 256, 256, 32, "random", 20}, TensorDataGenerator::DataType::RANDOM},
    };
    ...
    for (const auto& result : all_results) {
        ...
        if (!result.all_correct) all_passed = false;
    }
    ...
    log_info("Overall Correctness:          ", all_passed ? "✅ ALL PASS" : "❌ SOME FAILURES");
    ...
    log_info("");
    log_info("✅ TDT protocol shows intelligent adaptation to data characteristics!");
}
...
void benchmark_network_sensitivity() {
    ...
    for (const auto& condition : conditions) {
        ...
        bool will_compress = protocol.should_transform(tensor_data.data(), tensor_size);
        
        log_info("Condition: ", condition.name);
        log_info("  Bandwidth: ", condition.bandwidth_mbps, " Mbps, CPU: ", 
                 condition.cpu_usage * 100, "%");
        log_info("  Expected: ", condition.should_compress ? "COMPRESS" : "PASSTHROUGH");
        log_info("  Actual: ", will_compress ? "COMPRESS" : "PASSTHROUGH");
        log_info("  Result: ", (will_compress == condition.should_compress) ? "✅ CORRECT" : "❌ WRONG");
        log_info("");
    }
}
...
int main() {
    ...
    try {
        ...
        log_info("• TDT excels at sparse data (gradients, post-ReLU activations)");
