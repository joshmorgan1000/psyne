...
public:
    void* allocate_memory_slab(size_t size_bytes) override { return std::aligned_alloc(64, size_bytes); }
    void deallocate_memory_slab(void* memory) override { std::free(memory); }
    void transport_send(void* data, size_t size) override {}
    void transport_receive(void* buffer, size_t buffer_size) override {}
...
public:
    ...
    void* coordinate_receive() override {
        ...
        size_t slot = current_read % 1024; read_pos_.fetch_add(1);
...
private: std::atomic<size_t> write_pos_{0}; std::atomic<size_t> read_pos_{0}; void* slab_memory_ = nullptr; size_t message_size_ = 0;
};
...
public:
    void* coordinate_allocation(void* slab_memory, size_t message_size) override {
        ...
        if (current_write - current_read >= 1024) return nullptr;
        size_t slot = write_pos_.fetch_add(1) % 1024;
        return static_cast<char*>(slab_memory) + (slot * message_size);
    }
    void* coordinate_receive() override {
...
void quick_test(const std::string& test_name, size_t num_producers, size_t num_consumers) {
    std::cout << "Testing " << test_name << "... ";
    
    try {
        using ChannelType = psyne::behaviors::ChannelBridge<TestMessage, SubstrateType, PatternType>;
            ...
            producers.emplace_back([&, id = i]() {
                while (running.load()) {
                    try {
                        ...
                        sent.fetch_add(1);
                    } catch (...) { std::this_thread::sleep_for(std::chrono::microseconds(1)); }
                }
            });
        }
        ...
        for (size_t i = 0; i < num_consumers; ++i) {
            consumers.emplace_back([&]() {
                while (running.load()) {
                    ...
                    if (msg_opt) { received.fetch_add(1); }
