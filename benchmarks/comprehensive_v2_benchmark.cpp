    ...
    try {
        ...
        results.success =
            ...
            (results.total_messages_received >=
             results.total_messages_sent * 0.95); // 95% receive rate
    ...
    for (const auto &result : all_results) {
        csv << result.test_name << ","
            ...
            << result.test_name.substr(result.test_name.find('_') + 1,
                                       result.test_name.rfind('_') -
                                           result.test_name.find('_') - 1)
...
int main() {
    ...
    all_results.push_back(
        run_benchmark<SmallMessage, BenchmarkInProcess, psyne::pattern::SPSC>(
            {"SPSC", "InProcess", "Small", 1, 1, std::chrono::seconds(30),
             sizeof(SmallMessage)}));

    // SPSC + InProcess + Medium
    all_results.push_back(
        run_benchmark<MediumMessage, BenchmarkInProcess, psyne::pattern::SPSC>(
            {"SPSC", "InProcess", "Medium", 1, 1, std::chrono::seconds(30),
             sizeof(MediumMessage)}));

    // SPSC + InProcess + Large
    all_results.push_back(
        run_benchmark<LargeMessage, BenchmarkInProcess, psyne::pattern::SPSC>(
            {"SPSC", "InProcess", "Large", 1, 1, std::chrono::seconds(30),
             sizeof(LargeMessage)}));
    ...
    all_results.push_back(
        run_benchmark<SmallMessage, BenchmarkInProcess, psyne::pattern::MPSC>(
            {"MPSC", "InProcess", "Small", 4, 1, std::chrono::seconds(30),
             sizeof(SmallMessage)}));

    all_results.push_back(
        run_benchmark<MediumMessage, BenchmarkInProcess, psyne::pattern::MPSC>(
            {"MPSC", "InProcess", "Medium", 4, 1, std::chrono::seconds(30),
             sizeof(MediumMessage)}));
    ...
    all_results.push_back(
        run_benchmark<SmallMessage, BenchmarkInProcess, psyne::pattern::SPMC>(
            {"SPMC", "InProcess", "Small", 1, 4, std::chrono::seconds(30),
             sizeof(SmallMessage)}));

    all_results.push_back(
        run_benchmark<MediumMessage, BenchmarkInProcess, psyne::pattern::SPMC>(
            {"SPMC", "InProcess", "Medium", 1, 4, std::chrono::seconds(30),
             sizeof(MediumMessage)}));
    ...
    all_results.push_back(
        run_benchmark<SmallMessage, BenchmarkInProcess, psyne::pattern::MPMC>(
            {"MPMC", "InProcess", "Small", 4, 4, std::chrono::seconds(30),
             sizeof(SmallMessage)}));

    all_results.push_back(
        run_benchmark<MediumMessage, BenchmarkInProcess, psyne::pattern::MPMC>(
            {"MPMC", "InProcess", "Medium", 4, 4, std::chrono::seconds(30),
             sizeof(MediumMessage)}));
