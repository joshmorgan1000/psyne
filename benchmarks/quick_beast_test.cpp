...
void quick_beast_test(const std::string &name, size_t producers,
                      size_t consumers) {
    std::cout << std::setw(20) << name << " (" << std::setw(2) << producers
              << "P × " << std::setw(2) << consumers << "C): ";
    std::cout.flush();
    ...
    try {
        ...
        for (auto &t : producers_vec)
            t.join();
        for (auto &t : consumers_vec)
            t.join();
...
int main() {
    ...
    if (cores >= 64)
        quick_beast_test<QuickMPSC>("MPSC", 64, 1);

    std::cout << "\n📡 SPMC SCALING (1 Producer → Many Consumers) [KEY FOR "
                 "MANIFOLDB!]\n";
    std::cout
        << "================================================================\n";
    ...
    if (cores >= 64)
        quick_beast_test<QuickSPMC>("SPMC", 1, 64);
