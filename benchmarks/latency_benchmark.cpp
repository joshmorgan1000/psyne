...
public:
    ...
    explicit TimestampMessage(const void* data, size_t size) : FixedMessage(data, size) {}
...
LatencyStats benchmark_ping_pong(size_t iterations) {
    ...
    ChannelType request_channel("memory://request", 1024 * 1024, ChannelType::SingleType);
    ChannelType response_channel("memory://response", 1024 * 1024, ChannelType::SingleType);
    ...
    std::thread responder([&]() {
        for (size_t i = 0; i < iterations; ++i) {
            auto msg = request_channel.template receive_single<TimestampMessage>();
            if (msg) {
    ...
    for (size_t i = 0; i < iterations; ++i) {
        ...
        // Wait for response
        auto response = response_channel.template receive_single<TimestampMessage>();
...
int main() {
    ...
    // Run benchmarks
    std::cout << "\nRunning benchmarks (" << test_iterations << " iterations each)..." << std::endl;
    ...
    std::cout << std::setw(10) << "Channel" 
              << std::setw(12) << "Mean (μs)"
              << std::setw(12) << "P99 (μs)"
              << std::setw(12) << "P99.9 (μs)" << std::endl;
    std::cout << std::string(60, '-') << std::endl;
    
    std::cout << std::setw(10) << "SPSC" 
              << std::setw(12) << spsc_stats.mean
              << std::setw(12) << spsc_stats.p99
              << std::setw(12) << spsc_stats.p999 << std::endl;
              
    std::cout << std::setw(10) << "SPMC" 
              << std::setw(12) << spmc_stats.mean
              << std::setw(12) << spmc_stats.p99
              << std::setw(12) << spmc_stats.p999 << std::endl;
              
    std::cout << std::setw(10) << "MPSC" 
              << std::setw(12) << mpsc_stats.mean
              << std::setw(12) << mpsc_stats.p99
              << std::setw(12) << mpsc_stats.p999 << std::endl;
              
    std::cout << std::setw(10) << "MPMC" 
              << std::setw(12) << mpmc_stats.mean
              << std::setw(12) << mpmc_stats.p99
              << std::setw(12) << mpmc_stats.p999 << std::endl;
