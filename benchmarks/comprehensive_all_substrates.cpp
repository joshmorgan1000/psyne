...
public:
    ...
    bool is_zero_copy() const override {
        return false;
    } // Network involves serialization
    bool is_cross_process() const override {
        return true;
    }
...
BenchmarkResults run_benchmark(const BenchmarkConfig &config) {
    ...
    results.test_name = config.pattern_name + "_" + config.substrate_name +
                        "_" + config.message_size;
    ...
    try {
        using ChannelType =
            psyne::behaviors::ChannelBridge<MessageType, SubstrateType,
                                            PatternType>;
        ...
        results.success = (results.total_messages_received > 0) &&
                          (results.total_messages_received >=
                           results.total_messages_sent * 0.90);
...
void save_results_to_csv(const std::vector<BenchmarkResults> &all_results) {
    ...
    csv << "test_name,pattern,substrate,message_size,messages_sent,messages_"
           "received,"
    ...
    for (const auto &result : all_results) {
        ...
        std::string substrate = result.test_name.substr(
            first_underscore + 1, last_underscore - first_underscore - 1);
        std::string message_size = result.test_name.substr(last_underscore + 1);

        csv << result.test_name << "," << pattern << "," << substrate << ","
            << message_size << "," << result.total_messages_sent << ","
...
#define RUN_PATTERN_SUBSTRATE_COMBO(pattern_class, substrate_class,            \
                                    pattern_name, substrate_name, num_prod,    \
                                    num_cons)                                  \
    std::cout << "\n🔹 " << pattern_name << " + " << substrate_name            \
              << " Tests\n";                                                   \
    std::cout << "=====================================\n";                    \
                                                                               \
    all_results.push_back(                                                     \
        run_benchmark<SmallMessage, substrate_class, pattern_class>(           \
            {pattern_name, substrate_name, "Small", num_prod, num_cons,        \
             std::chrono::seconds(30), sizeof(SmallMessage)}));                \
                                                                               \
    all_results.push_back(                                                     \
        run_benchmark<MediumMessage, substrate_class, pattern_class>(          \
            {pattern_name, substrate_name, "Medium", num_prod, num_cons,       \
             std::chrono::seconds(30), sizeof(MediumMessage)}));               \
                                                                               \
    all_results.push_back(                                                     \
        run_benchmark<LargeMessage, substrate_class, pattern_class>(           \
            {pattern_name, substrate_name, "Large", num_prod, num_cons,        \
             std::chrono::seconds(30), sizeof(LargeMessage)}));

int main() {
    std::cout << "Psyne v2.0 COMPREHENSIVE Pattern × Substrate × Message Size "
                 "Benchmark\n";
    std::cout << "============================================================="
                 "========\n";
    ...
    RUN_PATTERN_SUBSTRATE_COMBO(WorkingSPSC, BenchmarkInProcess, "SPSC",
                                "InProcess", 1, 1);
    ...
    RUN_PATTERN_SUBSTRATE_COMBO(WorkingMPSC, BenchmarkInProcess, "MPSC",
                                "InProcess", 4, 1);
    ...
    RUN_PATTERN_SUBSTRATE_COMBO(WorkingSPMC, BenchmarkInProcess, "SPMC",
                                "InProcess", 1, 4);
    ...
    RUN_PATTERN_SUBSTRATE_COMBO(WorkingMPMC, BenchmarkInProcess, "MPMC",
                                "InProcess", 4, 4);
    ...
    std::cout << "==================================\n";
    std::cout << std::left << std::setw(25) << "Test Name" << std::right
              << std::setw(15) << "Throughput" << std::setw(12) << "Data Rate"
              << std::setw(12) << "Latency" << std::setw(8) << "Status" << "\n";
    ...
    for (const auto &result : all_results) {
        if (result.success) {
            ...
            pattern_throughputs[pattern].push_back(
                result.throughput_msgs_per_sec);
            substrate_throughputs[substrate].push_back(
                result.throughput_msgs_per_sec);
        }
    }
    ...
    for (const auto &[pattern, throughputs] : pattern_throughputs) {
        double avg =
            ...
            throughputs.size();
        std::cout << "  " << pattern << ": " << std::fixed
                  << std::setprecision(0) << avg << " msgs/s\n";
    }
