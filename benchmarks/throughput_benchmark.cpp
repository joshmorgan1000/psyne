...
BenchmarkResult run_benchmark(const BenchmarkConfig &config) {
    // Create channel
    ChannelType channel("memory://bench", config.buffer_size,
    ...
    std::thread consumer([&]() {
        while (!done || messages_received < config.num_messages) {
            ...
            auto msg =
                channel.template receive_single<FloatVector>(milliseconds(10));
            if (msg) {
    ...
    result.throughput_mbps =
        (total_bytes / (1024.0 * 1024.0)) / total_time.count();
...
void print_result(const std::string &name, const BenchmarkConfig &config,
                  const BenchmarkResult &result) {
    std::cout << "\n=== " << name << " ===" << std::endl;
    std::cout << "Message size: " << config.message_size << " bytes"
              << std::endl;
    std::cout << "Buffer size: " << config.buffer_size / (1024 * 1024) << " MB"
              << std::endl;
    std::cout << "Messages sent: " << result.messages_sent << std::endl;
    std::cout << "Messages received: " << result.messages_received << std::endl;
    std::cout << "Total time: " << result.total_time.count() << " seconds"
              << std::endl;
...
int main(int argc, char *argv[]) {
    ...
    for (const auto &config : configs) {
        std::cout << "\nRunning benchmark: " << config.message_size
                  << " byte messages..." << std::endl;
