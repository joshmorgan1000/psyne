...
BenchmarkResults run_benchmark(const BenchmarkConfig& config) {
    ...
    results.test_name = config.pattern_name + "_InProcess_" + config.message_size;
    ...
    try {
        using ChannelType = psyne::behaviors::ChannelBridge<MessageType, BenchmarkSubstrate, PatternType>;
        ...
        results.success = (results.total_messages_received > 0) && 
                         (results.total_messages_received >= results.total_messages_sent * 0.90); // 90% receive rate
...
int main() {
    ...
    all_results.push_back(run_benchmark<SmallMessage, WorkingSPSC>({
        "SPSC", "Small", 1, 1, std::chrono::seconds(30), sizeof(SmallMessage)
    }));
    
    all_results.push_back(run_benchmark<MediumMessage, WorkingSPSC>({
        "SPSC", "Medium", 1, 1, std::chrono::seconds(30), sizeof(MediumMessage)
    }));
    
    all_results.push_back(run_benchmark<LargeMessage, WorkingSPSC>({
        "SPSC", "Large", 1, 1, std::chrono::seconds(30), sizeof(LargeMessage)
    }));
    ...
    all_results.push_back(run_benchmark<SmallMessage, WorkingMPSC>({
        "MPSC", "Small", 4, 1, std::chrono::seconds(30), sizeof(SmallMessage)
    }));
    
    all_results.push_back(run_benchmark<MediumMessage, WorkingMPSC>({
        "MPSC", "Medium", 4, 1, std::chrono::seconds(30), sizeof(MediumMessage)
    }));
    
    all_results.push_back(run_benchmark<LargeMessage, WorkingMPSC>({
        "MPSC", "Large", 4, 1, std::chrono::seconds(30), sizeof(LargeMessage)
    }));
    ...
    all_results.push_back(run_benchmark<SmallMessage, WorkingSPMC>({
        "SPMC", "Small", 1, 4, std::chrono::seconds(30), sizeof(SmallMessage)
    }));
    
    all_results.push_back(run_benchmark<MediumMessage, WorkingSPMC>({
        "SPMC", "Medium", 1, 4, std::chrono::seconds(30), sizeof(MediumMessage)
    }));
    
    all_results.push_back(run_benchmark<LargeMessage, WorkingSPMC>({
        "SPMC", "Large", 1, 4, std::chrono::seconds(30), sizeof(LargeMessage)
    }));
    ...
    all_results.push_back(run_benchmark<SmallMessage, WorkingMPMC>({
        "MPMC", "Small", 4, 4, std::chrono::seconds(30), sizeof(SmallMessage)
    }));
    
    all_results.push_back(run_benchmark<MediumMessage, WorkingMPMC>({
        "MPMC", "Medium", 4, 4, std::chrono::seconds(30), sizeof(MediumMessage)
    }));
    
    all_results.push_back(run_benchmark<LargeMessage, WorkingMPMC>({
        "MPMC", "Large", 4, 4, std::chrono::seconds(30), sizeof(LargeMessage)
    }));
    ...
    for (const auto& result : all_results) {
        ...
        std::cout << std::left << std::setw(20) << result.test_name 
                  << std::right << std::setw(15) << std::fixed << std::setprecision(0) << result.throughput_msgs_per_sec << " msgs/s"
                  << std::setw(12) << std::fixed << std::setprecision(1) << result.throughput_mbps << " MB/s";
        
        if (result.avg_latency_ns > 0) {
            std::cout << std::setw(12) << std::fixed << std::setprecision(1) << result.avg_latency_ns / 1000.0 << " μs";
        } else {
