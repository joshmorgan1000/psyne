...
void init_benchmark_pool(const BenchmarkConfig& config) {
    if (!benchmark_pool) {
        ...
        benchmark_pool = std::make_unique<PsynePool>(
            std::min(num_threads, std::thread::hardware_concurrency()), // min threads
            std::max(num_threads, std::thread::hardware_concurrency()), // max threads
            5000, // thread evict wait (5 seconds)
            true,  // enable work stealing
            affinity
        );
        
        log_info("Initialized benchmark thread pool with ", num_threads, " threads");
    }
}
...
template <typename T>
std::shared_ptr<Channel<T>> create_benchmark_channel(const BenchmarkConfig& config,
                                                     bool is_producer) {
...
void throughput_consumer(std::shared_ptr<Channel<BenchmarkMessage>> channel,
    ...
    while (received < messages_per_consumer) {
        ...
        // Verify checksum
        uint32_t expected_checksum = calculate_checksum(msg->payload, 
                                                       config.message_size);
...
void bandwidth_test(const BenchmarkConfig& config) {
    ...
    // Print results
    auto duration = duration_cast<milliseconds>(stats.end_time - stats.start_time).count();
    double throughput_mbps = (stats.bytes_sent.load() / 1024.0 / 1024.0) / 
                            (duration / 1000.0);
    
    log_info("Duration: ", duration, " ms");
    log_info("Total data transferred: ", stats.bytes_sent.load() / 1024 / 1024, " MB");
    log_info("Throughput: ", std::fixed, std::setprecision(2), throughput_mbps, " MB/s");
}
    ...
    auto producer_future = benchmark_pool->enqueuePriority(
        ...
        [&]() {
            for (size_t burst = 0; burst < num_bursts; ++burst) {
                ...
                auto burst_duration = duration_cast<microseconds>(burst_end - burst_start).count();
...
void stress_test(const BenchmarkConfig& config) {
    ...
    log_info("Messages per producer: ", config.num_messages / config.num_producers);
    ...
    for (size_t i = 0; i < config.num_consumers; ++i) {
        auto future = benchmark_pool->enqueuePriority(
            PsynePool::HIGH_PRIORITY,
            [&config, &stats, i]() {
                auto channel = create_benchmark_channel<BenchmarkMessage>(config, false);
    ...
    auto duration = duration_cast<seconds>(stats.end_time - stats.start_time).count();
    ...
    log_info("Throughput: ", std::fixed, std::setprecision(2), msg_per_sec, " msg/s");
    log_info("Bandwidth: ", mb_per_sec, " MB/s");
    log_info("Average latency: ", stats.total_latency_ns.load() /
                                        stats.messages_received.load() / 1000, " us");
...
void print_usage(const char* program) {
    ...
    log_info("  --count <num>                Number of messages (default: 1000000)");
    ...
    log_info("  --host <hostname>            TCP server host (default: localhost)");
...
int main(int argc, char* argv[]) {
    ...
    try {
        if (test_pattern == "throughput") {
            ...
            log_info("Avg latency: ", stats.total_latency_ns.load() /
                                            stats.messages_received.load() / 1000, " us");
            
        } else if (test_pattern == "bandwidth") {
            // Test with various message sizes
            std::vector<size_t> sizes = {1024, 16384, 65536, 1048576}; // 1KB to 1MB
            for (size_t size : sizes) {
                config.message_size = size;
                config.num_messages = std::min(10000UL, 10UL * 1024 * 1024 * 1024 / size);
