...
public:
    ...
    void* coordinate_receive() override {
        size_t current_read = read_pos_.load(); size_t current_write = write_pos_.load();
        if (current_read >= current_write) return nullptr;
        size_t slot = current_read % buffer_size_; read_pos_.fetch_add(1);
        return static_cast<char*>(slab_memory_) + (slot * message_size_);
    }
    void producer_sync() override {} void consumer_sync() override {}
    const char* pattern_name() const override { return "SPSC"; }
    bool needs_locks() const override { return false; }
    size_t max_producers() const override { return 1; } size_t max_consumers() const override { return 1; }
private: 
    static constexpr size_t buffer_size_ = 1024 * 1024; // 1M slots!
    std::atomic<size_t> write_pos_{0}; std::atomic<size_t> read_pos_{0}; 
    void* slab_memory_ = nullptr; size_t message_size_ = 0;
};
...
public:
    ...
    size_t max_producers() const override { return SIZE_MAX; } size_t max_consumers() const override { return 1; }
private: 
...
public:
    ...
    bool needs_locks() const override { return true; }
    size_t max_producers() const override { return 1; } size_t max_consumers() const override { return SIZE_MAX; }
private: 
    static constexpr size_t buffer_size_ = 1024 * 1024;
    std::atomic<size_t> write_pos_{0}; std::atomic<size_t> read_pos_{0}; 
    void* slab_memory_ = nullptr; size_t message_size_ = 0;
...
public:
    void* coordinate_allocation(void* slab_memory, size_t message_size) override {
        ...
        if (current_write - current_read >= buffer_size_) return nullptr;
        size_t slot = write_pos_.fetch_add(1) % buffer_size_;
        return static_cast<char*>(slab_memory) + (slot * message_size);
    }
    void* coordinate_receive() override {
    ...
    bool needs_locks() const override { return true; }
    size_t max_producers() const override { return SIZE_MAX; } size_t max_consumers() const override { return SIZE_MAX; }
private: 
    static constexpr size_t buffer_size_ = 1024 * 1024;
    std::atomic<size_t> write_pos_{0}; std::atomic<size_t> read_pos_{0}; 
    void* slab_memory_ = nullptr; size_t message_size_ = 0;
...
void beast_test(const std::string& pattern_name, size_t num_producers, size_t num_consumers, int duration_sec = 10) {
    std::cout << "\n🔥 " << pattern_name << " with " << num_producers << "P × " << num_consumers << "C";
    ...
    try {
        ...
        std::cout << "Producer errors:   " << std::setw(12) << producer_errors.load() << "\n";
        std::cout << "Duration:          " << std::fixed << std::setprecision(2) << actual_duration << "s\n";
        std::cout << "Throughput:        " << std::fixed << std::setprecision(2) << throughput_millions << "M msgs/s";
        std::cout << " (" << std::fixed << std::setprecision(0) << throughput << " msgs/s)\n";
        
        // Per-thread stats
        if (num_producers > 1) {
            std::cout << "Per producer:      " << std::fixed << std::setprecision(2) 
                      << (sent.load() / (double)num_producers / actual_duration / 1'000'000.0) << "M msgs/s\n";
        }
        if (num_consumers > 1) {
            std::cout << "Per consumer:      " << std::fixed << std::setprecision(2) 
                      << (received.load() / (double)num_consumers / actual_duration / 1'000'000.0) << "M msgs/s\n";
        }
...
int main() {
    ...
    if (cores >= 64) beast_test<BeastMPSC>("MPSC", 64, 1);
    ...
    if (cores >= 64) beast_test<BeastSPMC>("SPMC", 1, 64);
    
    // MPMC MADNESS
    beast_test<BeastMPMC>("MPMC", 8, 8);
    beast_test<BeastMPMC>("MPMC", 16, 16);
    if (cores >= 32) beast_test<BeastMPMC>("MPMC", 32, 32);
