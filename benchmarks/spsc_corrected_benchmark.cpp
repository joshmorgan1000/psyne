...
struct BenchmarkMessage {
    ...
    BenchmarkMessage(uint64_t id) : id(id), receive_timestamp(0) {
        ...
        std::snprintf(payload, sizeof(payload), "SPSC_Msg_%llu", static_cast<unsigned long long>(id));
    }
...
public:
    ...
    const char* substrate_name() const override { return "SPSCInProcess"; }
...
public:
    void* coordinate_allocation(void* slab_memory, size_t message_size) override {
        size_t current_slot = write_pos_.load(std::memory_order_relaxed) % max_messages_;
    ...
    const char* pattern_name() const override { return "TrueSPSC"; }
...
SPSCBenchmarkResults run_single_threaded_spsc_benchmark(size_t num_messages = 1000000) {
    std::cout << "\n=== Single-Threaded SPSC Benchmark ===\n";
    std::cout << "Mode: One thread doing send->receive loop\n";
    std::cout << "Messages: " << num_messages << "\n";
    
    using ChannelType = psyne::behaviors::ChannelBridge<BenchmarkMessage, SPSCInProcessSubstrate, TrueSPSCPattern>;
    ...
    results.avg_latency_ns = results.latencies.empty() ? 0 : 
        std::accumulate(results.latencies.begin(), results.latencies.end(), 0.0) / results.latencies.size();
    results.min_latency_ns = results.latencies.empty() ? 0 : results.latencies.front();
    results.max_latency_ns = results.latencies.empty() ? 0 : results.latencies.back();
    results.p95_latency_ns = results.latencies.empty() ? 0 : results.latencies[results.latencies.size() * 95 / 100];
    results.p99_latency_ns = results.latencies.empty() ? 0 : results.latencies[results.latencies.size() * 99 / 100];
...
void print_spsc_results(const std::string& test_name, const SPSCBenchmarkResults& results) {
    std::cout << "\n" << test_name << " Results:\n";
    std::cout << "================================\n";
    ...
    std::cout << "Data Rate:          " << std::fixed << std::setprecision(2) 
              << (results.total_messages * sizeof(BenchmarkMessage) / results.duration_seconds) / (1024*1024) << " MB/sec\n";
...
int main() {
    ...
    std::cout << "================================================\n";
    std::cout << "SPSC = Single Producer, Single Consumer (exactly 1 of each!)\n";
    ...
    try {
        ...
        std::cout << "Cross-thread SPSC:  " << std::fixed << std::setprecision(0) 
                  << true_spsc.throughput_msgs_per_sec << " msgs/sec\n";
        std::cout << "Cross-thread ratio: " << std::fixed << std::setprecision(2) 
                  << (true_spsc.throughput_msgs_per_sec / single_threaded.throughput_msgs_per_sec) << "x\n";
        
        std::cout << "\nArchitecture:       Physical substrate + Abstract message lens\n";
        ...
        std::cout << "Ready for MPSC (Multi-Producer, Single-Consumer) implementation!\n";
