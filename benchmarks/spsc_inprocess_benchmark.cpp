...
struct BenchmarkMessage {
    ...
    BenchmarkMessage()
        : id(0), send_timestamp(0), receive_timestamp(0), payload_checksum(0) {
        std::memset(payload, 0, sizeof(payload));
    }

    BenchmarkMessage(uint64_t id, const char *data = nullptr)
        : id(id), receive_timestamp(0) {
        ...
        if (data) {
            ...
            std::snprintf(payload, sizeof(payload), "BenchMsg_%llu",
...
public:
    ...
    uint64_t send_count() const {
        return send_count_.load();
    }
    uint64_t receive_count() const {
        return receive_count_.load();
    }
...
public:
    void *coordinate_allocation(void *slab_memory,
        ...
        size_t current_slot =
            write_pos_.fetch_add(1, std::memory_order_relaxed) % max_messages_;
...
BenchmarkResults run_throughput_benchmark(size_t num_messages = 1000000) {
    ...
    using ChannelType = psyne::behaviors::ChannelBridge<
        BenchmarkMessage, BenchmarkInProcessSubstrate, BenchmarkSPSCPattern>;
    ...
    results.p95_latency_ns =
        latencies.empty() ? 0 : latencies[latencies.size() * 95 / 100];
    results.p99_latency_ns =
        latencies.empty() ? 0 : latencies[latencies.size() * 99 / 100];
    results.data_transferred_bytes =
...
void print_results(const std::string &test_name,
    ...
    std::cout << "Data Rate:          " << std::fixed << std::setprecision(2)
              << (results.data_transferred_bytes / results.duration_seconds) /
                     (1024 * 1024)
              << " MB/sec\n";
    std::cout << "\nLatency Statistics:\n";
    std::cout << "Average:            " << std::fixed << std::setprecision(1)
              << results.avg_latency_ns << " ns\n";
    std::cout << "Minimum:            " << std::fixed << std::setprecision(1)
              << results.min_latency_ns << " ns\n";
    std::cout << "Maximum:            " << std::fixed << std::setprecision(1)
              << results.max_latency_ns << " ns\n";
    std::cout << "95th percentile:    " << std::fixed << std::setprecision(1)
              << results.p95_latency_ns << " ns\n";
    std::cout << "99th percentile:    " << std::fixed << std::setprecision(1)
              << results.p99_latency_ns << " ns\n";
    std::cout << "\nData Integrity:     "
              << (results.all_checksums_valid ? "✅ PASSED" : "❌ FAILED")
              << "\n";
}
...
int main() {
    ...
    try {
        ...
        std::cout << "\n=== Summary ===\n";
        std::cout << "Max Throughput:     " << std::fixed
                  << std::setprecision(0)
                  << throughput_results.throughput_msgs_per_sec
                  << " msgs/sec\n";
        std::cout << "Min Latency:        " << std::fixed
                  << std::setprecision(1) << latency_results.min_latency_ns
                  << " ns\n";
        std::cout << "Architecture:       Physical substrate + Abstract "
                     "message lens\n";
