 ...
 * @brief TCP Server/Client benchmark with TDT compression for cross-machine testing
 * 
 * Usage:
...
public:
    ...
    static std::vector<uint8_t> compress(const float* data, size_t count, CompressionStats& stats) {
        ...
        stats.compression_time_us = duration_cast<microseconds>(end - start).count();
        
        return compressed;
    }
    
    static std::vector<float> decompress(const uint8_t* data, size_t size, CompressionStats& stats) {
        auto start = high_resolution_clock::now();
        ...
        for (size_t i = 0; i < size; ) {
            if (data[i] == 0xFF && i + 3 < size) {
                ...
                uint16_t value = data[i + 2] | (static_cast<uint16_t>(data[i + 3]) << 8);
                
                // Dequantize and add to result
                float float_val = (static_cast<float>(value) - 32768.0f) / 32767.0f;
        ...
        auto end = high_resolution_clock::now();
        stats.decompression_time_us = duration_cast<microseconds>(end - start).count();
            ...
            case GRADIENTS:
                {
                    ...
                    for (auto& val : data) {
                        val = dist(gen);
                        if (std::abs(val) < 0.0001f) val = 0.0f; // Sparsity
                    }
                }
                break;
...
void run_server(int port, const BenchmarkConfig& config) {
    ...
    std::cout << "  Tensor size:  " << config.tensor_size << " floats (" 
              << (config.tensor_size * sizeof(float) / 1024 / 1024) << " MB)" << std::endl;
    std::cout << "  Data type:    " << TensorGenerator::type_name(config.data_type) << std::endl;
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < config.tensor_count; ++i) {
        ...
        auto tensor = TensorGenerator::generate(config.tensor_size, config.data_type);
        
        // Compress
        auto compressed = SimpleTDT::compress(tensor.data(), tensor.size(), stats[i]);
        
        // Send compressed size first
        uint32_t compressed_size = compressed.size();
        if (!NetworkUtils::send_all(client_sock, &compressed_size, sizeof(compressed_size))) {
            std::cerr << "Failed to send compressed size" << std::endl;
            break;
        }
        
        // Send compressed data
        if (!NetworkUtils::send_all(client_sock, compressed.data(), compressed.size())) {
        ...
        if (config.verbose && (i + 1) % 100 == 0) {
            std::cout << "Sent " << (i + 1) << "/" << config.tensor_count 
                      << " tensors (compression: " << std::fixed << std::setprecision(2) 
                      << stats[i].compression_ratio << "x)" << std::endl;
        }
    }
    ...
    double total_time_s = duration_cast<microseconds>(total_end - total_start).count() / 1e6;
    size_t total_original = std::accumulate(stats.begin(), stats.end(), 0UL,
        [](size_t sum, const auto& s) { return sum + s.original_size; });
    size_t total_compressed = std::accumulate(stats.begin(), stats.end(), 0UL,
        [](size_t sum, const auto& s) { return sum + s.compressed_size; });
    
    std::cout << std::endl << "📊 Server Results:" << std::endl;
    std::cout << "==================" << std::endl;
    std::cout << "Total time:           " << std::fixed << std::setprecision(3) << total_time_s << " seconds" << std::endl;
    std::cout << "Tensors sent:         " << config.tensor_count << std::endl;
    std::cout << "Original data:        " << (total_original / 1024 / 1024) << " MB" << std::endl;
    std::cout << "Compressed data:      " << (total_compressed / 1024 / 1024) << " MB" << std::endl;
    std::cout << "Compression ratio:    " << std::fixed << std::setprecision(2) 
              << (static_cast<double>(total_original) / total_compressed) << "x" << std::endl;
    std::cout << "Network throughput:   " << std::fixed << std::setprecision(2) 
              << (total_compressed / 1024.0 / 1024.0 / total_time_s) << " MB/s (compressed)" << std::endl;
    std::cout << "Effective throughput: " << std::fixed << std::setprecision(2) 
              << (total_original / 1024.0 / 1024.0 / total_time_s) << " MB/s (original)" << std::endl;
...
void run_client(const char* host, int port) {
    std::cout << "🔌 TDT TCP Client connecting to " << host << ":" << port << std::endl;
    ...
    std::cout << "  Tensor count: " << config.tensor_count << std::endl;
    std::cout << "  Tensor size:  " << config.tensor_size << " floats" << std::endl;
    std::cout << "  Data type:    " << TensorGenerator::type_name(config.data_type) << std::endl;
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < config.tensor_count; ++i) {
        ...
        if (!NetworkUtils::recv_all(sock, &compressed_size, sizeof(compressed_size))) {
        ...
        // Decompress
        auto decompressed = SimpleTDT::decompress(compressed.data(), compressed.size(), stats[i]);
        
        stats[i].original_size = decompressed.size() * sizeof(float);
        stats[i].compressed_size = compressed_size;
        stats[i].compression_ratio = static_cast<double>(stats[i].original_size) / stats[i].compressed_size;
        
        if (config.verbose && (i + 1) % 100 == 0) {
            std::cout << "Received " << (i + 1) << "/" << config.tensor_count 
                      << " tensors (decompressed to " << decompressed.size() << " floats)" << std::endl;
        }
    }
    ...
    std::cout << "Decompressed data:    " << (total_original / 1024 / 1024) << " MB" << std::endl;
    std::cout << "Network data:         " << (total_compressed / 1024 / 1024) << " MB" << std::endl;
    std::cout << "Compression ratio:    " << std::fixed << std::setprecision(2) 
              << (static_cast<double>(total_original) / total_compressed) << "x" << std::endl;
    ...
    double avg_decomp_time = std::accumulate(stats.begin(), stats.end(), 0.0,
        [](double sum, const auto& s) { return sum + s.decompression_time_us; }) / stats.size();
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        ...
        std::cout << "  Client: " << argv[0] << " client <server_ip> [port]" << std::endl;
        std::cout << std::endl;
        std::cout << "Example:" << std::endl;
        std::cout << "  On Linux:  " << argv[0] << " server 8080" << std::endl;
        std::cout << "  On Mac:    " << argv[0] << " client 192.168.1.100 8080" << std::endl;
        return 1;
    }
