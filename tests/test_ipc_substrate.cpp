...
struct IPCTestMessage {
    ...
    IPCTestMessage(uint64_t id, const char *message)
...
void test_ipc_single_process() {
    ...
    std::cout << "Testing IPC substrate within one process (shared memory "
                 "creation)\n";

    try {
        using ChannelType = psyne::behaviors::ChannelBridge<
            IPCTestMessage, psyne::substrate::IPC, SimpleIPCPattern>;
        ...
        std::cout << "Created message: ID=" << msg1->id
                  << " PID=" << msg1->sender_pid << " Data='" << msg1->data
                  << "'\n";
        ...
        if (received) {
            std::cout << "Received message: ID=" << (*received)->id
                      << " PID=" << (*received)->sender_pid << " Data='"
                      << (*received)->data << "'\n";
            std::cout << "✅ Single process IPC test PASSED\n";
        } else {
            std::cout
                << "❌ Single process IPC test FAILED - no message received\n";
        }
...
void producer_process(const std::string &shm_name, int producer_id,
                      int num_messages) {
    try {
        std::cout << "Producer " << producer_id << " (PID " << getpid()
                  << ") starting...\n";

        // Attach to existing shared memory (created by parent)
        psyne::substrate::IPC ipc_substrate(
            shm_name, false); // false = attach to existing
        ...
        for (int i = 1; i <= num_messages; ++i) {
            ...
            size_t offset = ((producer_id - 1) * num_messages + (i - 1)) *
                            sizeof(IPCTestMessage);
            if (offset + sizeof(IPCTestMessage) <= 16384) {
                ...
                new (msg_ptr) IPCTestMessage(i, ("Producer" +
    ...
    try {
        ...
        while (received_count < expected_messages) {
            ...
            if (ipc_substrate.try_transport_receive()) {
                received_count++;
                std::cout << "Consumer received message notification "
                          << received_count << "\n";
            } else {
                usleep(1000); // 1ms
            }
        ...
        std::cout << "Consumer finished - received " << received_count
                  << " messages\n";
...
void test_ipc_cross_process() {
    ...
    try {
        ...
        psyne::substrate::IPC ipc_substrate(shm_name,
                                            true); // true = create new
        void *shm_memory = ipc_substrate.allocate_memory_slab(16384);
        ...
        std::cout << "Cross-process test completed: " << success_count << "/"
                  << child_pids.size() << " processes succeeded\n";
