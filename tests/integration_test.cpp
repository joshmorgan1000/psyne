...
int main() {
    ...
    try {
        ...
        {
            auto channel = psyne::create_channel("memory://integration_test",
                                                 4 * 1024 * 1024);
            ...
            std::thread consumer([&]() {
                try {
                    while (!producer_done ||
                        ...
                        auto msg =
                            channel->template receive<psyne::FloatVector>(
            ...
            std::cout << "✓ End-to-end message passing (" << messages_sent
                      << " sent, " << messages_received << " received)"
                      << std::endl;
        }
        ...
        // Test 2: Multi-type channel test
        {
            auto channel = psyne::create_channel(
                "memory://multi_type_test", 2 * 1024 * 1024,
                psyne::ChannelMode::MPMC, psyne::ChannelType::MultiType);
        ...
        {
            auto channel =
                psyne::create_channel("memory://perf_test", 8 * 1024 * 1024);
            ...
            double messages_per_second =
                static_cast<double>(test_messages) /
        ...
        // Test 4: Error handling and recovery
        {
            ...
            auto channel =
                psyne::create_channel("memory://bounds_test", 1024 * 1024);
            auto msg = psyne::FloatVector(*channel);
            msg.resize(10);
