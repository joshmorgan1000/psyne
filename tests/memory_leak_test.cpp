...
protected:
     ...
     */
    bool has_memory_leak(size_t threshold_bytes = 1024 *
                                                  1024) { // 1MB threshold
...
void test_channel_creation_destruction() {
    ...
    EXPECT_FALSE(has_memory_leak())
        << "Memory leak detected: " << get_memory_increase()
        << " bytes increased";
}
...
TEST_F(MemoryLeakTestFixture, RingBufferOperations) {
    auto channel =
        create_test_channel("ring_buffer_test", 64 * 1024); // Small buffer
...
TEST_F(MemoryLeakTestFixture, MultiThreadedOperations) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, t, messages_per_thread]() {
            auto channel =
                create_test_channel("thread_test_" + std::to_string(t));
...
TEST_F(MemoryLeakTestFixture, IPCChannelMemory) {
    ...
    for (int i = 0; i < num_iterations; ++i) {
        try {
            auto ipc_channel =
                create_test_ipc_channel("ipc_leak_test_" + std::to_string(i));
            if (ipc_channel) {
                ...
                for (int j = 0; j < 10; ++j) {
                    ...
                    std::string data = "IPC test " + std::to_string(i) + "_" +
                                       std::to_string(j);
                ...
                // Receive messages
                while (auto received =
                           ipc_channel->receive_single<ByteVector>()) {
                    // Message automatically cleaned up
                }
            }
...
TEST_F(MemoryLeakTestFixture, LargeMessageHandling) {
    auto channel =
        create_test_channel("large_msg_test", 10 * 1024 * 1024); // 10MB buffer
