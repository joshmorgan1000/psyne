...
protected:
    ...
    void generate_memory_report() {
        if (memory_samples_.empty()) return;
        ...
        for (const auto& sample : memory_samples_) {
            auto elapsed = std::chrono::duration<double>(
                sample.timestamp - start_time).count();
            
            std::cout << elapsed << "\t"
                     << (sample.rss_bytes / 1024.0 / 1024.0) << "\t"
        ...
        auto max_rss = std::max_element(memory_samples_.begin(), memory_samples_.end(),
            [](const MemorySample& a, const MemorySample& b) {
                return a.rss_bytes < b.rss_bytes;
            });
        
        auto max_heap = std::max_element(memory_samples_.begin(), memory_samples_.end(),
            [](const MemorySample& a, const MemorySample& b) {
                return a.heap_bytes < b.heap_bytes;
            });
        
        std::cout << "\n=== PEAK USAGE ===\n";
        std::cout << "Peak RSS:  " << (max_rss->rss_bytes / 1024.0 / 1024.0) << " MB\n";
        std::cout << "Peak Heap: " << (max_heap->heap_bytes / 1024.0 / 1024.0) << " MB\n";
        std::cout << "Baseline:  " << (baseline_memory_ / 1024.0 / 1024.0) << " MB\n";
        std::cout << "Growth:    " << ((max_rss->rss_bytes - baseline_memory_) / 1024.0 / 1024.0) << " MB\n";
    }
...
TEST_F(MemoryProfilingBenchmark, ChannelLifecycleMemory) {
    ...
    for (int i = 0; i < num_channels; ++i) {
        channels.push_back(create_test_channel("memory_test_" + std::to_string(i), 1024 * 1024));
...
TEST_F(MemoryProfilingBenchmark, HighVolumeMessagingMemory) {
    auto channel = create_test_channel("high_volume_test", 64 * 1024 * 1024); // 64MB buffer
    ...
    while (auto received = channel->receive_single<ByteVector>()) {
        ...
        if (received_count % 1000 == 0) {
            record_memory_sample("received_" + std::to_string(received_count) + "_messages");
        }
    }
...
TEST_F(MemoryProfilingBenchmark, BufferSizeScaling) {
    ...
    for (size_t buffer_size : buffer_sizes) {
        auto channel = create_test_channel("buffer_test_" + std::to_string(buffer_size), buffer_size);
        channels.push_back(std::move(channel));
        
        record_memory_sample("buffer_" + std::to_string(buffer_size / 1024) + "KB");
    }
...
TEST_F(MemoryProfilingBenchmark, MultiThreadedMemoryUsage) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        channels.push_back(create_test_channel("thread_" + std::to_string(i), 1024 * 1024));
    }
