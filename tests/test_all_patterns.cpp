...
struct TestMessage {
    ...
    TestMessage(uint64_t id, uint32_t producer_id)
        ...
        std::snprintf(data, sizeof(data), "P%u_M%llu", producer_id,
...
void test_spsc() {
    ...
    using ChannelType =
        psyne::behaviors::ChannelBridge<TestMessage, TestSubstrate, SimpleSPSC>;
    ...
    std::cout << "SPSC: Sent " << messages_sent.load() << ", Received "
              << messages_received.load() << "\n";
    std::cout << "SPSC: "
              << (messages_sent.load() == messages_received.load()
                      ? "✅ PASSED"
                      : "❌ FAILED")
              << "\n";
}
...
void test_mpsc() {
    ...
    using ChannelType =
        psyne::behaviors::ChannelBridge<TestMessage, TestSubstrate,
                                        psyne::patterns::MPSC>;
    ...
    std::cout << "MPSC: Sent " << messages_sent.load() << ", Received "
              << messages_received.load() << "\n";
    std::cout << "MPSC: "
              << (messages_sent.load() == messages_received.load()
                      ? "✅ PASSED"
...
void test_spmc() {
    ...
    std::cout << "SPMC: Sent " << messages_sent.load() << ", Received "
              << messages_received.load() << "\n";
    std::cout << "SPMC: "
              << (messages_sent.load() == messages_received.load()
                      ? "✅ PASSED"
...
void test_mpmc() {
    ...
    std::cout << "MPMC: Sent " << messages_sent.load() << ", Received "
              << messages_received.load() << "\n";
    std::cout << "MPMC: "
              << (messages_sent.load() == messages_received.load()
                      ? "✅ PASSED"
