...
public:
    void* coordinate_allocation(void* slab_memory, size_t message_size) override {
        ...
        void* slot = static_cast<char*>(slab_memory) + (current_slot * message_size);
        ...
        std::cout << "SPSC: Allocated slot " << current_slot << " for writing\n";
...
void test_basic_functionality() {
    ...
    // Create channel
    psyne::behaviors::ChannelBridge<TestMessage, SimpleInProcessSubstrate, SimpleSPSCPattern> channel(4096);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (received) {
            std::cout << "Received message: ID=" << (*received)->id << " Data='" << (*received)->data << "'\n";
        } else {
            std::cout << "No message available (pattern implementation incomplete)\n";
        }
    }
...
void test_producer_consumer() {
    ...
    psyne::behaviors::ChannelBridge<TestMessage, SimpleInProcessSubstrate, SimpleSPSCPattern> channel(8192);
    ...
    // Consumer thread
    std::thread consumer([&]() {
        while (!producer_done.load() || messages_received.load() < total_messages) {
            auto msg = channel.try_receive();
            if (msg) {
                messages_received.fetch_add(1);
                std::cout << "Consumer received message " << messages_received.load() 
                          << " ID=" << (*msg)->id << "\n";
            } else {
                std::this_thread::sleep_for(std::chrono::milliseconds(1));
            }
        }
        std::cout << "Consumer finished receiving " << messages_received.load() << " messages\n";
    });
    ...
    std::cout << "  Success: " << (messages_sent.load() == messages_received.load() ? "YES" : "NO") << "\n";
}
