...
// Latency benchmark
void benchmark_latency(const std::string &transport_name,
                       const std::string &uri) {
    std::cout << "=== " << transport_name
    ...
    if (!latencies.empty()) {
        ...
        std::cout << "Avg latency: " << (avg_latency / 1000.0) << " μs"
                  << std::endl;
        std::cout << "Min latency: " << (min_latency / 1000.0) << " μs"
                  << std::endl;
        std::cout << "Max latency: " << (max_latency / 1000.0) << " μs"
                  << std::endl;
        std::cout << "P50 latency: " << (p50_latency / 1000.0) << " μs"
                  << std::endl;
        std::cout << "P99 latency: " << (p99_latency / 1000.0) << " μs"
                  << std::endl;
    }
    std::cout << std::endl;
}
...
void benchmark_throughput(const std::string &transport_name,
                          const std::string &uri) {
    std::cout << "=== " << transport_name
              << " High-Performance Throughput Benchmark ===" << std::endl;

    auto channel = create_channel(
        uri, 64 * 1024 * 1024);      // 64MB buffer for high throughput
    const int num_messages = 100000; // Optimal for ring buffer size
    const int message_sizes[] = {64, 256, 1024, 4096,
                                 16384}; // Including larger messages
    ...
    for (size_t msg_size : message_sizes) {
        ...
        std::thread consumer([&]() {
            while (!producer_done || messages_received < messages_sent) {
                ...
                if (received) {
                    ...
                    uint8_t expected =
                        static_cast<uint8_t>((messages_received - 1) & 0xFF);
        ...
        double mb_per_second = (messages_received * msg_size) /
        ...
        std::cout << "  Success rate: " << std::fixed << std::setprecision(1)
                  << (100.0 * messages_received / messages_sent) << "%"
                  << std::endl;
        ...
        if (gb_per_second >= 1.0) {
            std::cout << " (" << std::setprecision(2) << gb_per_second
                      << " GB/s)";
        }
        std::cout << std::endl;
        std::cout << "  Avg per-message latency: " << std::fixed
                  << std::setprecision(3)
                  << (duration.count() / (2.0 * messages_received)) << " μs"
                  << std::endl;
        std::cout << std::endl;
    }
}
...
int main() {
    ...
    std::cout
        << "║          High-Performance Zero-Copy Messaging Library        ║\n";
    std::cout << "╚════════════════════════════════════════════════════════════"
                 "═══╝\n\n";
    ...
    try {
        ...
        std::cout << "Note: IPC benchmarks may require elevated permissions "
                     "for shared memory\n\n";
        try {
        ...
        std::cout << "║                  Benchmark Summary Complete            "
                     "       ║\n";
        std::cout << "║   Psyne v1.2.0 demonstrates exceptional performance "
                     "with     ║\n";
