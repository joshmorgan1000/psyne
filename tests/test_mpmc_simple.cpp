...
struct SimpleMessage {
    ...
    SimpleMessage(uint64_t msg_id, uint64_t prod_id)
        : id(msg_id), producer_id(prod_id) {
...
int main() {
    ...
    try {
        using ChannelType =
            psyne::behaviors::ChannelBridge<SimpleMessage, TestSubstrate,
                                            psyne::patterns::MPMC>;
        ...
        std::cout << "Starting " << num_producers << " producers, "
