...
void test_tcp_in_process() {
    ...
    std::thread server_thread([&]() {
        try {
            ...
            psyne::substrate::SimpleTCP server_substrate("localhost", test_port,
                                                         true);

            // Wait for connection
            if (server_substrate.wait_for_connection(
                ...
                while (!test_complete.load() && messages_received.load() < 3) {
                    try {
                        server_substrate.transport_receive(&recv_buffer,
                                                           sizeof(recv_buffer));
                        std::cout << "Server received: ID=" << recv_buffer.id
    ...
    std::thread client_thread([&]() {
        try {
            ...
            psyne::substrate::SimpleTCP client_substrate("localhost", test_port,
                                                         false);

            if (client_substrate.wait_for_connection(
                ...
                for (int i = 1; i <= 3; ++i) {
                    NetworkMessage msg(
                        i, ("Test_Message_" + std::to_string(i)).c_str());
