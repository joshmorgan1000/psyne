...
public:
    ...
    void set_data(const std::string& text, uint64_t counter) {
        ...
        *reinterpret_cast<uint64_t*>(data_) = counter;
        ...
        size_t text_len = std::min(text.size(), payload_size - sizeof(uint64_t) - 1);
    ...
    std::string get_text() const {
        if (!data_) return "";
        return std::string(reinterpret_cast<const char*>(data_ + sizeof(uint64_t)));
    }
...
void test_tcp_basic() {
    ...
    std::thread server_thread([&]() {
        try {
            TCPChannel<SPSCRingBuffer> server("0.0.0.0", 9997, buffer_size, true);
            server_ready = true;
            ...
            while (messages_received < num_messages) {
                ...
                if (rb) {
                    ...
                    if (read_handle) {
                        ...
                        // Verify message
                        assert(text == "Test message " + std::to_string(counter));
    ...
    std::thread client_thread([&]() {
        try {
            ...
            TCPChannel<SPSCRingBuffer> client("localhost", 9997, buffer_size, false);
...
void test_channel_factory() {
    ...
    auto [host1, port1] = ChannelFactory::extract_tcp_endpoint("tcp://localhost:8080");
    assert(host1 == "localhost");
    assert(port1 == 8080);
    
    auto [host2, port2] = ChannelFactory::extract_tcp_endpoint("tcp://192.168.1.1:9999");
    assert(host2 == "192.168.1.1");
    assert(port2 == 9999);
    
    auto [host3, port3] = ChannelFactory::extract_tcp_endpoint("tcp://example.com");
