...
protected:
    ...
    create_test_channel(const std::string &name = "test",
                        size_t buffer_size = 1024 * 1024,
                        ChannelMode mode = ChannelMode::SPSC,
                        ChannelType type = ChannelType::SingleType) {
        std::string uri =
            "memory://" + name + "_" + std::to_string(++channel_counter_);
     ...
     */
    std::unique_ptr<Channel>
    create_test_ipc_channel(const std::string &name = "ipc_test",
                            size_t buffer_size = 1024 * 1024) {
        std::string uri =
            "ipc://" + name + "_" + std::to_string(++channel_counter_);
        return Channel::create(uri, buffer_size, ChannelMode::SPSC,
...
protected:
    void setup_performance() {
        ...
        perf_channel_ =
            create_test_channel("perf_test", 64 * 1024 * 1024); // 64MB
...
protected:
    void setup_concurrency() {
        ...
        for (int i = 0; i < 4; ++i) {
            auto channel =
                create_test_channel("concurrent_test_" + std::to_string(i));
    ...
    void run_concurrent(Func &&func, int num_threads = 4) {
        ...
        for (int i = 0; i < num_threads &&
                        i < static_cast<int>(concurrent_channels_.size());
             ++i) {
            threads.emplace_back([&func, &channel = *concurrent_channels_[i],
                                  i]() { func(channel, i); });
        }
