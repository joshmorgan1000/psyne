...
int main() {
    try {
        ...
        vec *= 2.0f;
        std::cout << "After scaling by 2: (" << vec.x() << ", " << vec.y() << ", " << vec.z() << ")" << std::endl;
        ...
        std::cout << "Scale: " << qvec.header().scale << ", Zero point: " << qvec.header().zero_point << std::endl;
        std::cout << "Values: ";
        for (size_t i = 0; i < qvec.size(); ++i) {
            std::cout << (int)qvec[i] << " ";
        }
        ...
        std::cout << "Sparse matrix: " << sparse.rows() << "x" << sparse.cols() 
                  << " with " << sparse.nnz() << " non-zero elements" << std::endl;
