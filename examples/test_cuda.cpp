...
int main() {
    ...
    if (std::find(backends.begin(), backends.end(), gpu::GPUBackend::CUDA) != backends.end()) {
        ...
        if (gpu_context) {
            ...
            // Note: Can't call methods on GPUContext as it's only forward declared in public API
            // This is intentional - internal GPU details are hidden from public API
        } else {
