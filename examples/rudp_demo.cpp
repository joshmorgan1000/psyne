...
void demo_basic_connection() {
    ...
    std::cout << YELLOW << "Testing basic RUDP connection..." << RESET << std::endl;
    ...
    auto server_future = std::async(std::launch::async, []() {
        ...
        std::cout << GREEN << "[SERVER] Listening on port 8080" << RESET << std::endl;
        ...
        if (connection) {
            ...
            for (int i = 0; i < 5; ++i) {
                ...
                if (received > 0) {
                    ...
                    std::cout << GREEN << "[SERVER] Received: " << msg << RESET << std::endl;
                    
                    std::string echo = "Echo: " + msg;
                    connection->send(echo.data(), echo.size());
                    std::cout << GREEN << "[SERVER] Sent: " << echo << RESET << std::endl;
                }
            }
    ...
    if (client) {
        ...
        for (int i = 1; i <= 5; ++i) {
            ...
            std::cout << BLUE << "[CLIENT] Sent: " << message << RESET << std::endl;
            ...
            if (received > 0) {
                ...
                std::cout << BLUE << "[CLIENT] Received: " << echo << RESET << std::endl;
            }
        ...
        std::cout << "  Retransmissions: " << stats.packets_retransmitted << std::endl;
...
void demo_reliability_test() {
    ...
    std::cout << YELLOW << "Testing RUDP reliability with simulated packet loss..." << RESET << std::endl;
...
void demo_flow_control() {
    ...
    std::cout << YELLOW << "Testing RUDP flow and congestion control..." << RESET << std::endl;
    ...
    for (int round = 1; round <= 10; ++round) {
        if (cwnd < ssthresh) {
            // Slow start
            cwnd *= 2;
            std::cout << "  Round " << std::setw(2) << round << ": cwnd=" << std::setw(3) << cwnd 
                      << " (slow start)" << std::endl;
        } else {
        ...
        if (round == 7) {
            ssthresh = cwnd / 2;
            cwnd = ssthresh;
            std::cout << "  Round " << std::setw(2) << round << ": PACKET LOSS! cwnd=" << cwnd 
...
void demo_performance_comparison() {
    ...
    std::cout << YELLOW << "Comparing RUDP vs TCP vs UDP performance..." << RESET << std::endl;
    ...
    for (const auto& p : protocols) {
        std::cout << std::setw(10) << p.protocol
                  << std::setw(12) << std::fixed << std::setprecision(1) << p.latency_ms
                  << std::setw(12) << std::setprecision(0) << p.throughput_mbps << " Mbps"
    ...
    std::cout << "  • Configurable reliability/performance trade-offs" << std::endl;
    std::cout << "  • Better for real-time applications needing some reliability" << std::endl;
...
void demo_use_cases() {
    ...
    std::cout << YELLOW << "RUDP applications in practice..." << RESET << std::endl;
...
void demo_tuning_guide() {
    ...
    std::cout << YELLOW << "RUDP configuration parameters and tuning..." << RESET << std::endl;
    ...
    std::cout << "  • Lower = faster recovery, more spurious retransmits" << std::endl;
    ...
    std::cout << "  • Higher = more reliable, slower failure detection" << std::endl;
    ...
    std::cout << "\n" << MAGENTA << "High Throughput (File Transfer):" << RESET << std::endl;
    ...
    std::cout << "\n" << MAGENTA << "Reliable Control (Industrial):" << RESET << std::endl;
...
int main() {
    std::cout << CYAN;
    std::cout << "╔═══════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║             Psyne Reliable UDP (RUDP) Demo                ║" << std::endl;
    std::cout << "╚═══════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << RESET;
    ...
    try {
        ...
        std::cout << GREEN << "RUDP implementation provides:" << RESET << std::endl;
        std::cout << "  ✓ TCP-like reliability over UDP" << std::endl;
        std::cout << "  ✓ Lower latency than TCP" << std::endl;
        std::cout << "  ✓ Configurable reliability/performance trade-offs" << std::endl;
        std::cout << "  ✓ Flow control and congestion control" << std::endl;
        std::cout << "  ✓ Automatic retransmission and error recovery" << std::endl;
        std::cout << "  ✓ Selective acknowledgments" << std::endl;
        std::cout << "  ✓ Connection management and state tracking" << std::endl;
        
        std::cout << "\n" << BLUE << "Perfect for:" << RESET << std::endl;
        std::cout << "  • Real-time applications needing reliability" << std::endl;
