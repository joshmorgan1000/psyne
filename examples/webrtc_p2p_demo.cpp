...
public:
    ...
    Header& header() { 
        return *reinterpret_cast<Header*>(Message<PlayerPositionMessage>::data()); 
    }
    
    const Header& header() const { 
        return *reinterpret_cast<const Header*>(Message<PlayerPositionMessage>::data()); 
    }
    
    PositionData& position() {
        return *reinterpret_cast<PositionData*>(
            reinterpret_cast<uint8_t*>(Message<PlayerPositionMessage>::data()) + sizeof(Header)
        );
    }
    
    const PositionData& position() const {
        return *reinterpret_cast<const PositionData*>(
            reinterpret_cast<const uint8_t*>(Message<PlayerPositionMessage>::data()) + sizeof(Header)
        );
    }
...
public:
    ...
    Header& header() { 
        return *reinterpret_cast<Header*>(Message<CombatActionMessage>::data()); 
    }
    
    ActionData& action() {
        return *reinterpret_cast<ActionData*>(
            reinterpret_cast<uint8_t*>(Message<CombatActionMessage>::data()) + sizeof(Header)
        );
    }
...
public:
    ...
    void start_simulation(std::shared_ptr<Channel> channel) {
        ...
        std::cout << "🎮 Player " << player_id_ << " (" << peer_id_ << ") started simulation\n";
    }
    ...
    void simulate_combat_action() {
        ...
        action.header().action_type = static_cast<CombatActionMessage::ActionType>(
            1 + (uniform_dist_(gen_) * 4)  // Random action 1-5
        );
        
        action.action().target_player_id = (player_id_ == 1) ? 2 : 1;  // Target other player
        action.action().weapon_id = position_.weapon_id;
        action.action().damage = 10 + (uniform_dist_(gen_) * 20);  // 10-30 damage
        action.action().accuracy = 0.7f + (uniform_dist_(gen_) * 0.3f);  // 70-100% accuracy
    ...
    void print_stats() const {
        ...
        std::cout << "   Health: " << static_cast<int>(position_.health) << "/100\n";
        std::cout << "   Messages sent: " << messages_sent_.load() << "\n";
        std::cout << "   Messages received: " << messages_received_.load() << "\n";
        std::cout << "   Average latency: " << average_latency_.load() << " ms\n";
...
private:
    ...
    void update_position(float dt) {
        ...
        float velocity_mag = std::sqrt(position_.velocity_x * position_.velocity_x + 
                                     position_.velocity_z * position_.velocity_z);
        if (velocity_mag > max_velocity) {
            position_.velocity_x = (position_.velocity_x / velocity_mag) * max_velocity;
            position_.velocity_z = (position_.velocity_z / velocity_mag) * max_velocity;
        }
        ...
        if (velocity_mag > 0.1f) {
            position_.rotation_yaw = std::atan2(position_.velocity_z, position_.velocity_x);
        }
    }
    ...
    void message_receive_loop() {
        while (running_) {
            ...
            auto pos_msg = channel_->receive<PlayerPositionMessage>(std::chrono::milliseconds(10));
            if (pos_msg && pos_msg->header().player_id != player_id_) {
                handle_position_message(*pos_msg);
            }
            
            // Try to receive combat actions
            auto action_msg = channel_->receive<CombatActionMessage>(std::chrono::milliseconds(10));
    ...
    void handle_position_message(const PlayerPositionMessage& msg) {
        ...
        std::cout << "📍 Peer " << msg.header().player_id 
                  << " at (" << pos.x << ", " << pos.z << ") "
                  << "latency: " << latency << "ms\n";
    }
    ...
    void handle_combat_message(const CombatActionMessage& msg) {
        ...
        if (msg.action().target_player_id == player_id_) {
            ...
            if (msg.action().accuracy > uniform_dist_(gen_)) {
                position_.health = std::max(0, static_cast<int>(position_.health) - damage);
                std::cout << "💥 Hit by Player " << msg.header().player_id 
                          << " for " << damage << " damage! Health: " << static_cast<int>(position_.health) << "\n";
            } else {
                std::cout << "🛡️  Player " << msg.header().player_id << " missed!\n";
            }
        }
        
        std::cout << "⚔️  Peer " << msg.header().player_id 
                  << " executed " << action_type_name(msg.header().action_type)
                  << " latency: " << latency << "ms\n";
    }
    
    static const char* action_type_name(CombatActionMessage::ActionType type) {
        switch (type) {
            case CombatActionMessage::Attack: return "Attack";
            case CombatActionMessage::Block: return "Block";
            case CombatActionMessage::Dodge: return "Dodge";
            case CombatActionMessage::Reload: return "Reload";
            case CombatActionMessage::UseItem: return "UseItem";
...
private:
    ...
    void run_demo_loop(GamePlayer& player) {
        ...
        while (running_) {
            ...
            auto time_since_action = std::chrono::duration_cast<std::chrono::seconds>(now - last_action);
        ...
        std::cout << "\n🏁 Demo completed after " << duration.count() << " seconds\n";
    }
