...
public:
    InMemoryCollectiveGroup(RankId rank, size_t world_size)
        ...
        for (size_t i = 0; i < world_size; ++i) {
            if (i != rank_) {
                std::string uri = "memory://collective_" + std::to_string(rank_) + "_to_" + std::to_string(i);
                channels_[i] = create_channel(uri, 1024 * 1024, ChannelMode::SPSC);
            }
        }
    }
    
    RankId rank() const override { return rank_; }
    size_t size() const override { return world_size_; }
    ...
    std::shared_ptr<Channel> get_channel(RankId target) override {
        ...
        std::string uri = "memory://collective_" + std::to_string(target) + "_to_" + std::to_string(rank_);
...
void test_allreduce(RankId rank) {
    ...
    if (!correct) {
        std::cerr << "[Rank " << rank << "] All-reduce failed! Got: " 
                  << data[0] << ", " << data[1] << std::endl;
        errors++;
    } else {
        std::cout << "[Rank " << rank << "] All-reduce successful! Result: "
                  << data[0] << ", " << data[1] << std::endl;
    }
}
...
void test_scatter_gather(RankId rank) {
    ...
    if (!scatter_correct) {
        ...
        std::cout << "[Rank " << rank << "] Scatter successful! Received: "
                  << scatter_recv[0] << ", " << scatter_recv[1] << std::endl;
    }
    ...
    if (rank == 0) {
        ...
        for (size_t i = 0; i < 3; ++i) {
            if (gather_recv[i*2] != i * 100 || gather_recv[i*2+1] != i * 100 + 1) {
        ...
        if (!gather_correct) {
            ...
            for (int v : gather_recv) std::cout << v << " ";
            std::cout << std::endl;
        }
    }
}
...
void run_rank_tests(RankId rank) {
    std::cout << "\n=== Starting tests for Rank " << rank << " ===" << std::endl;
