...
public:
    ...
    void initialize() {
        ...
        auto tensor_span = typed_data_span<float>();
    ...
    float& operator()(size_t batch, size_t feature) noexcept {
        return typed_data_span<float>()[batch * FEATURES + feature];
    }
...
auto create_channel_info(const std::string& uri) {
    auto channel = Channel::create(uri, 1024 * 1024, ChannelMode::SPSC);
    auto& ring_buffer = channel->get_ring_buffer();
    auto buffer_span = ring_buffer.buffer_span();
    
    return std::make_tuple(std::move(channel), buffer_span.size(), ring_buffer.capacity());
}
...
int main() {
    try {
        ...
        constexpr auto tensor_size = TensorBatch::static_size();
        ...
        auto raw_span = tensor.data_span();
