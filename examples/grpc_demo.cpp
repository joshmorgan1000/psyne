...
public:
    ...
    void Run() {
        ...
        while (!channel_->is_stopped()) {
            // Receive request
            auto request = channel_->receive<ByteVector>(std::chrono::milliseconds(100));
            if (!request) continue;
...
void run_grpc_server() {
    ...
    auto request_channel = create_channel("memory://calc_requests", 1024 * 1024);
    auto response_channel = create_channel("memory://calc_responses", 1024 * 1024);
    
    // Create calculator service
    CalculatorService calc_service(request_channel);
    std::thread service_thread([&calc_service]() {
        calc_service.Run();
    });
...
void run_grpc_client() {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        *reinterpret_cast<float*>(request_data + 8) = b;
        ...
        auto status = client.CallUnary("/Calculator/Calculate",
...
int main(int argc, char* argv[]) {
    if (argc != 2) {
        std::cerr << "Usage: " << argv[0] << " [server|native-client|grpc-client]" << std::endl;
        return 1;
    }
    ...
    try {
        if (mode == "server") {
            ...
            std::cerr << "Invalid mode. Use 'server', 'native-client', or 'grpc-client'" << std::endl;
            return 1;
        }
