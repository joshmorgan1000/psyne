...
public:
    SimpleMetalContext() : device(nullptr), queue(nullptr) {
        ...
        id (*MTLCreateSystemDefaultDevice)(void) = (id (*)(void))dlsym(RTLD_DEFAULT, "MTLCreateSystemDefaultDevice");
        ...
        // Get device name
        id nameStr = ((id (*)(id, SEL))objc_msgSend)(device, sel_registerName("name"));
        const char* name = ((const char* (*)(id, SEL))objc_msgSend)(nameStr, sel_registerName("UTF8String"));
        
        std::cout << "Metal device: " << (name ? name : "Unknown") << std::endl;
        
        // Check for unified memory
        BOOL hasUnifiedMemory = ((BOOL (*)(id, SEL))objc_msgSend)(device, sel_registerName("hasUnifiedMemory"));
        std::cout << "Unified memory: " << (hasUnifiedMemory ? "Yes" : "No") << std::endl;
    }
    
    ~SimpleMetalContext() {
        if (queue) ((void (*)(id, SEL))objc_msgSend)(queue, sel_registerName("release"));
        if (device) ((void (*)(id, SEL))objc_msgSend)(device, sel_registerName("release"));
    }
    ...
    id createBuffer(const void* data, size_t length) {
        ...
        NSUInteger options = (MTLResourceStorageModeShared << 0) | (MTLResourceCPUCacheModeDefaultCache << 4);
        
        id buffer = ((id (*)(id, SEL, const void*, NSUInteger, NSUInteger))objc_msgSend)(
            device, 
    ...
    void* getBufferContents(id buffer) {
        return ((void* (*)(id, SEL))objc_msgSend)(buffer, sel_registerName("contents"));
    }
    
    size_t getBufferLength(id buffer) {
        return ((NSUInteger (*)(id, SEL))objc_msgSend)(buffer, sel_registerName("length"));
    }
...
int main() {
    ...
    try {
        ...
        // Create Metal buffer
        id buffer = metal.createBuffer(data.data(), data.size() * sizeof(float));
        if (!buffer) {
            throw std::runtime_error("Failed to create Metal buffer");
        }
        
        std::cout << "\nMetal buffer created: " << metal.getBufferLength(buffer) << " bytes" << std::endl;
        ...
        std::cout << "   - Same memory visible to both CPU and GPU" << std::endl;
