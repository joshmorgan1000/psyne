...
// Demonstrates fixed matrices, quantized vectors, complex numbers, ML tensors, and sparse matrices
...
void demo_fixed_matrices() {
    ...
    std::cout << "Transform matrix determinant: " << transform.determinant() << std::endl;
    ...
    // In-place operations
    position *= 2.0f;
    std::cout << "Scaled position vector length: " << position.length() << std::endl;
    ...
    if (received_transform && received_position) {
        ...
        std::cout << "Received transform determinant: " << received_transform->determinant() << std::endl;
        std::cout << "Received position length: " << received_position->length() << std::endl;
    }
}
...
void demo_quantized_vectors() {
    ...
    // Original float data
    std::vector<float> float_data = {0.1f, 0.5f, -0.3f, 0.8f, -0.9f, 0.2f, 0.7f, -0.1f};
    ...
    float scale = 0.01f;
    int8_t zero_point = 0;
    quantized.quantize_from_float(float_data.data(), float_data.size(), scale, zero_point);
    
    std::cout << "Original float data size: " << float_data.size() * sizeof(float) << " bytes\n";
    std::cout << "Quantized int8 data size: " << quantized.size() * sizeof(int8_t) << " bytes\n";
    std::cout << "Compression ratio: " << (float_data.size() * sizeof(float)) / (float)(quantized.size() * sizeof(int8_t)) << "x\n";
    
    std::cout << "Min quantized value: " << (int)quantized.min_value() << std::endl;
    std::cout << "Max quantized value: " << (int)quantized.max_value() << std::endl;
...
void demo_complex_vectors() {
    ...
    std::cout << "Dot product result: " << dot_result.real() << " + " << dot_result.imag() << "i" << std::endl;
}

void demo_ml_tensors() {
    std::cout << "\n=== ML Tensor Demo ===\n";
    
    auto channel = create_channel("memory://ml", 2 * 1024 * 1024); // Larger buffer for tensors
    ...
    std::cout << std::endl;
    std::cout << "Total elements: " << image_batch.total_elements() << std::endl;
    ...
    float pixel_value = image_batch.at_nchw(0, 1, 4, 4); // Batch 0, Channel 1, Y=4, X=4
    std::cout << "Pixel at (0,1,4,4): " << pixel_value << std::endl;
    
    // Normalize using L2 norm
    image_batch.normalize_l2();
    std::cout << "After L2 normalization - Mean: " << image_batch.mean() << std::endl;
}
...
void demo_sparse_matrices() {
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        if (row != col) { // Avoid overwriting diagonal
            sparse.set(row, col, dis(gen) * 0.1f); // Smaller off-diagonal values
        }
    }
    
    std::cout << "Sparse matrix dimensions: " << sparse.rows() << "x" << sparse.cols() << std::endl;
    std::cout << "Number of non-zeros: " << sparse.nnz() << std::endl;
    std::cout << "Sparsity: " << (1.0 - (double)sparse.nnz() / (sparse.rows() * sparse.cols())) * 100.0 << "%" << std::endl;
    ...
    std::cout << "Matrix-vector product range: [" << min_y << ", " << max_y << "]" << std::endl;
    
    // Test row access
    auto row_view = sparse.row(0);
    std::cout << "Row 0 has " << row_view.size << " non-zero elements:" << std::endl;
    size_t count = 0;
    ...
    if (row_view.size > 5) {
        std::cout << "  ... and " << (row_view.size - 5) << " more" << std::endl;
    }
}
