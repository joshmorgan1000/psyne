...
void producer_thread(uint32_t id, const std::string &channel_name,
    ...
    try {
        ...
        MetricsCollector::instance().register_channel(
            channel_name, config.transport, config.size_mb * 1024 * 1024);
        ...
        for (size_t i = 0; i < message_count; ++i) {
            ...
            auto alloc_time =
                std::chrono::high_resolution_clock::now() - start_alloc;
            ...
            MetricsCollector::instance().record_allocation(channel_name,
                                                           total_size);
            ...
            // Record send metrics
            MetricsCollector::instance().record_send(channel_name, total_size,
                                                     i);
...
void consumer_thread(uint32_t id, const std::string &channel_name,
    ...
    try {
        ...
        while (received < expected_messages) {
            ...
            MetricsCollector::instance().record_deallocation(channel_name,
                                                             msg_size);

            // Record receive metrics
            MetricsCollector::instance().record_receive(channel_name, msg_size,
                                                        msg->sequence, latency);
        ...
        log_info("[Consumer ", id, "] Completed. Received: ", received,
                 ", Errors: ", errors);
...
void stress_test_scenario() {
    ...
    for (size_t i = 0; i < num_channels; ++i) {
        ...
        producers.emplace_back(producer_thread, i, channel_name,
                               messages_per_channel, message_size, true,
                               true); // variable size and delays
...
void latency_test_scenario() {
    ...
    std::thread producer(producer_thread, 0, channel_name, message_count,
                         message_size, false, false);

    std::thread consumer(consumer_thread, 0, channel_name, message_count,
                         false);
...
int main(int argc, char *argv[]) {
    ...
    if (config.event_tracing) {
        ...
        for (size_t i = start; i < events.size(); ++i) {
            ...
            event_msg << "[" << event.timestamp_ns << "] " << event.channel_name
                      << " - ";
