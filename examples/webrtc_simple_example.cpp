...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            msg.send();
            std::cout << "📤 Sent message " << i << " with " << msg.size()
                      << " floats\n";
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (received) {
                std::cout << "📨 Received message with " << received->size()
                          << " floats: ";
                for (size_t j = 0; j < std::min(received->size(), size_t(5));
                     ++j) {
                    std::cout << (*received)[j] << " ";
                }
                if (received->size() > 5)
