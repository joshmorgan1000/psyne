...
void test_multicast_basic() {
    ...
    try {
        ...
        while (received < 10) {
            ...
            if (msg) {
                received++;
                std::cout << "Received message " << received << " with " << msg->size() << " floats: ";
                for (size_t i = 0; i < msg->size() && i < 3; ++i) {
                    std::cout << (*msg)[i] << " ";
                }
                if (msg->size() > 3) std::cout << "...";
                std::cout << std::endl;
            }
        ...
        std::cout << "Successfully received " << received << " out of 10 messages" << std::endl;
...
void test_multicast_with_compression() {
    ...
    try {
        ...
        auto publisher = multicast::create_publisher(multicast_addr, port, 1024*1024, comp_config);
        ...
        std::cout << "Publishing large messages with compression..." << std::endl;
        ...
        while (received < 5) {
            ...
            if (msg) {
                received++;
                std::cout << "Received compressed message " << received << " with " << msg->size() << " floats" << std::endl;
            ...
            auto elapsed = std::chrono::steady_clock::now() - start_time;
            if (elapsed > std::chrono::seconds(5)) {
                std::cout << "Timeout waiting for compressed messages" << std::endl;
                break;
            }
        }
        
        std::cout << "Successfully received " << received << " out of 5 compressed messages" << std::endl;
        
    } catch (const std::exception& e) {
        std::cerr << "Error in compression multicast test: " << e.what() << std::endl;
    }
}
...
void test_multiple_subscribers() {
    ...
    try {
        ...
        for (int i = 0; i < 3; ++i) {
            subscribers.push_back(multicast::create_subscriber(multicast_addr, port));
            receive_counts[i] = 0;
        }
        ...
        for (int i = 0; i < 3; ++i) {
            receiver_threads.emplace_back([&, i]() {
                while (!stop_receiving) {
                    ...
                    if (msg) {
                        receive_counts[i]++;
                        std::cout << "Subscriber " << (i + 1) << " received message " 
                                  << receive_counts[i] << " (size: " << msg->size() << ")" << std::endl;
                    }
                }
            });
        }
        ...
        for (int i = 0; i < 3; ++i) {
            std::cout << "  Subscriber " << (i + 1) << ": " << receive_counts[i] << " messages" << std::endl;
        }
        
    } catch (const std::exception& e) {
        std::cerr << "Error in multiple subscribers test: " << e.what() << std::endl;
    }
}
...
int main() {
    try {
        ...
        std::cout << "========================" << std::endl;
        std::cout << "Note: This demo uses local multicast addresses (239.255.x.x)" << std::endl;
        std::cout << "Make sure your system supports multicast networking." << std::endl << std::endl;
