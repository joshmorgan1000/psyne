...
void demo_basic_routing() {
    ...
    router.add_route<ByteVector>([](ByteVector &&msg) {
        std::cout << "Received ByteVector with " << msg.size() << " bytes"
                  << std::endl;
    });
    ...
    router.set_default_route([](uint32_t type, void *data, size_t size) {
        (void)data; // Unused
        std::cout << "Unmatched message: type=" << type << ", size=" << size
                  << std::endl;
    });
...
void demo_filtered_channels() {
    ...
    std::thread sender([&base_channel]() {
        ...
        // Small message
        {
            ...
            std::cout << "Sent FloatVector with 10 floats (~40 bytes)"
                      << std::endl;
        }
        ...
        // Large message
        {
            ...
            base_channel->send(msg);
            std::cout << "Sent FloatVector with 50 floats (~200 bytes)"
                      << std::endl;
        }
        ...
        // Another small message
        {
            ...
            msg.resize(size);
            base_channel->send(msg);
            std::cout << "Sent ByteVector with " << size << " bytes"
                      << std::endl;
        }
...
void demo_composite_filters() {
    ...
    auto composite_filter =
        std::make_unique<CompositeFilter>(CompositeFilter::Mode::AND);
    composite_filter->add_filter(
        std::make_unique<TypeFilter>(FloatVector::message_type));
    composite_filter->add_filter(std::make_unique<SizeFilter>(100, SIZE_MAX));

    // Create router with composite filter
    MessageRouter router;
    router.add_route(std::move(composite_filter), [](uint32_t type, void *data,
                                                     size_t size) {
        (void)type; // Unused
        (void)data; // Unused
        std::cout << "Received large FloatVector: size=" << size << " bytes"
                  << std::endl;
    });
    ...
    router.set_default_route([](uint32_t type, void *data, size_t size) {
        (void)data; // Unused
        std::cout << "Other message: type=" << type << ", size=" << size
                  << " bytes" << std::endl;
    });
...
void demo_performance() {
    ...
    std::cout << "  Total messages: " << (float_count + byte_count)
              << std::endl;
    std::cout << "  Time: " << duration.count() << " ms" << std::endl;
    std::cout << "  Throughput: "
              << (messages_per_type * 2 * 1000 / duration.count())
...
void demo_predicate_filter() {
    ...
    router.add_route(
        ...
        [](uint32_t type, void *data, size_t size) {
            ...
            const float *floats = reinterpret_cast<const float *>(
                static_cast<const uint8_t *>(data) + sizeof(size_t));
            std::cout << "FloatVector with first element > 5.0: " << floats[0]
                      << std::endl;
        });
    ...
    std::cout << "\nSending FloatVectors with different first elements..."
              << std::endl;
