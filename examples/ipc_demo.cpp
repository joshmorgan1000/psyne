...
void run_consumer() {
    ...
    try {
        ...
        std::cout << "Waiting for messages (timeout after 5 seconds of inactivity)...\n\n";
        ...
        while (true) {
            ...
            // Try FloatVector
            auto float_msg = channel->receive_single<FloatVector>(std::chrono::milliseconds(10));
            if (float_msg) {
            ...
            // Try ByteVector
            auto byte_msg = channel->receive_single<ByteVector>(std::chrono::milliseconds(10));
            if (byte_msg) {
            ...
            // Try DoubleMatrix
            auto matrix_msg = channel->receive_single<DoubleMatrix>(std::chrono::milliseconds(10));
            if (matrix_msg) {
                ...
                for (size_t i = 0; i < matrix_msg->rows(); ++i) {
                    ...
                    for (size_t j = 0; j < matrix_msg->cols(); ++j) {
                        ...
                        if (j < matrix_msg->cols() - 1) std::cout << ", ";
                    }
                    std::cout << "]\n";
                }
                received_any = true;
            ...
            if (elapsed.count() > 5) {
                std::cout << "\nTimeout reached. No messages received for 5 seconds.\n";
                break;
            }
        ...
        std::cout << "Consumer finished. Total messages received: " << received_count << "\n";
...
void run_test() {
    ...
    try {
        ...
        std::thread producer([&]() {
            ...
            producer_done = true;
            std::cout << "Producer finished. Sent " << messages_sent.load() << " messages\n";
        });
        
        // Consumer thread
        std::thread consumer([&]() {
            while (!producer_done || messages_received < messages_sent) {
                auto msg = channel->receive_single<FloatVector>(std::chrono::milliseconds(100));
                if (msg) {
                    messages_received++;
                    if (messages_received % 5 == 0) {
                        std::cout << "Consumer received " << messages_received.load() << " messages\n";
                    }
                }
            }
        ...
        std::cout << "  Success rate: " << (100.0 * messages_received.load() / messages_sent.load()) << "%\n";
