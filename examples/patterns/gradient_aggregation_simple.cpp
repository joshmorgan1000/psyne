/**
 * @file gradient_aggregation_simple.cpp
 * @brief Simplified example of using MPSC channel for gradient aggregation
 *
...
#include "logger.hpp"
#include "psyne/channel/mpsc_channel.hpp"
 ...
 */
void gradient_worker(uint32_t worker_id, MPSCChannel::ProducerHandle *producer,
 ...
 */
void parameter_server(MPSCChannel *channel, size_t num_workers,
                      size_t num_iterations) {
...
int main() {
    ...
    // Enable debug logging temporarily to troubleshoot
    // psyne::set_internal_log_level(psyne::LogLevel::DEBUG);
    ...
    MPSCChannel channel(config);

    log_info("Created MPSC channel with ", config.size_mb, "MB capacity");
    log_info("Starting ", num_workers, " workers, ", num_iterations,
             " iterations each");

    // Register producers and start workers
    std::vector<std::thread> worker_threads;
    std::vector<std::unique_ptr<MPSCChannel::ProducerHandle>> producers;

    for (uint32_t i = 0; i < num_workers; ++i) {
        auto producer = channel.register_producer();
        if (!producer) {
            log_error("Failed to register producer ", i);
            return 1;
        }
        worker_threads.emplace_back(gradient_worker, i, producer.get(),
                                    num_iterations);
