...
void demo_basic_connection() {
    ...
    std::cout << YELLOW << "Testing basic QUIC connection with stream multiplexing..." << RESET << std::endl;
    ...
    auto server_future = std::async(std::launch::async, [config]() {
        ...
        if (!server->start()) {
            std::cout << RED << "[SERVER] Failed to start" << RESET << std::endl;
            return;
        }
        
        std::cout << GREEN << "[SERVER] Listening on port 9443" << RESET << std::endl;
        
        // Accept connection
        auto connection = server->accept();
        if (connection) {
            std::cout << GREEN << "[SERVER] Connection accepted" << RESET << std::endl;
            
            // Handle multiple streams
            for (int i = 0; i < 3; ++i) {
                auto stream = connection->accept_stream();
                if (stream) {
                    std::cout << GREEN << "[SERVER] Stream " << stream->id() << " accepted" << RESET << std::endl;
                    ...
                    if (received > 0) {
                        ...
                        std::string echo = "Echo from stream " + std::to_string(stream->id()) + ": " + msg;
    ...
    if (client && client->is_connected()) {
        std::cout << BLUE << "[CLIENT] Connected to server" << RESET << std::endl;
        ...
        for (int stream_num = 1; stream_num <= 3; ++stream_num) {
            stream_futures.push_back(std::async(std::launch::async, [client, stream_num]() {
                auto stream = client->create_stream();
                if (stream) {
                    std::string message = "Message from stream " + std::to_string(stream_num);
        ...
        std::cout << "  Streams created: " << stats.streams_created << std::endl;
        std::cout << "  Bytes sent: " << stats.bytes_sent << std::endl;
        std::cout << "  Bytes received: " << stats.bytes_received << std::endl;
        std::cout << "  RTT: " << std::fixed << std::setprecision(2) << stats.rtt_ms << " ms" << std::endl;
        std::cout << "  Using 0-RTT: " << (stats.using_0rtt ? "Yes" : "No") << std::endl;
    ...
    std::cout << GREEN << "✓ Basic connection demo completed" << RESET << std::endl;
}
...
void demo_stream_multiplexing() {
    ...
    std::cout << YELLOW << "Demonstrating QUIC stream multiplexing advantages..." << RESET << std::endl;
    ...
    std::cout << "\n" << std::setw(10) << "Protocol" 
              ...
              << std::setw(15) << "Multiplexing"
              << std::setw(18) << "HOL Blocking"
              << std::setw(20) << "Encryption"
              << std::endl;
    std::cout << std::string(75, '-') << std::endl;
    
    for (const auto& p : protocols) {
        std::cout << std::setw(10) << p.protocol
                  << std::setw(12) << p.transport
                  << std::setw(15) << p.multiplexing
                  << std::setw(18) << p.hol_blocking
                  << "  " << p.encryption
                  << std::endl;
    }
    ...
    std::cout << "\n" << MAGENTA << "Head-of-Line Blocking Simulation:" << RESET << std::endl;
    std::cout << "Scenario: 3 requests, middle one has packet loss" << std::endl;
    
    std::cout << "\n  TCP (HTTP/2):" << std::endl;
    std::cout << "    Request 1: [████████████████████] 100ms (blocked by #2)" << std::endl;
    std::cout << "    Request 2: [████████░░░░░░░░░░░░] 200ms (packet loss + retransmit)" << std::endl;
    std::cout << "    Request 3: [████████████████████] 100ms (blocked by #2)" << std::endl;
    std::cout << "    Total time: 200ms (all blocked by slowest)" << std::endl;
    
    std::cout << "\n  QUIC:" << std::endl;
    std::cout << "    Stream 1:  [████████████████████] 100ms ✓" << std::endl;
    std::cout << "    Stream 2:  [████████░░░░░░░░░░░░] 200ms (packet loss + retransmit)" << std::endl;
    std::cout << "    Stream 3:  [████████████████████] 100ms ✓" << std::endl;
    std::cout << "    Total time: 100ms (streams 1&3), 200ms (stream 2)" << std::endl;
    
    std::cout << GREEN << "✓ Stream multiplexing demo completed" << RESET << std::endl;
}
...
void demo_0rtt_resumption() {
    ...
    std::cout << YELLOW << "Demonstrating QUIC 0-RTT resumption..." << RESET << std::endl;
    
    std::cout << GREEN << "Connection Establishment Comparison:" << RESET << std::endl;
    ...
    std::cout << "\n" << std::setw(15) << "Protocol" 
              << std::setw(15) << "First Connect"
              << std::setw(12) << "Resumption"
              << std::setw(18) << "Data Transmission"
              << std::endl;
    std::cout << std::string(60, '-') << std::endl;
    
    for (const auto& h : handshakes) {
        std::cout << std::setw(15) << h.protocol
                  << std::setw(15) << h.first_connection
                  << std::setw(12) << h.resumption
                  << "  " << h.data_sent
                  << std::endl;
    }
    ...
    std::cout << "\n" << YELLOW << "0-RTT Security Considerations:" << RESET << std::endl;
    ...
    std::cout << "\n" << MAGENTA << "Latency Comparison (100ms RTT):" << RESET << std::endl;
    ...
    std::cout << GREEN << "✓ 0-RTT resumption demo completed" << RESET << std::endl;
}
...
void demo_connection_migration() {
    ...
    std::cout << YELLOW << "Demonstrating QUIC connection migration..." << RESET << std::endl;
    
    std::cout << GREEN << "Connection Migration Scenarios:" << RESET << std::endl;
    std::cout << "  • Mobile device switching from WiFi to cellular" << std::endl;
    ...
    std::cout << GREEN << "✓ Connection migration demo completed" << RESET << std::endl;
}
...
void demo_performance_comparison() {
    ...
    std::cout << YELLOW << "Comparing QUIC with other protocols..." << RESET << std::endl;
    ...
    std::cout << std::setw(10) << "Protocol" 
              << std::setw(15) << "Handshake RTT"
              << std::setw(15) << "Latency"
              << std::setw(15) << "CPU Overhead"
              << std::setw(25) << "Best Use Case"
              << std::endl;
    std::cout << std::string(80, '-') << std::endl;
    
    for (const auto& p : protocols) {
        std::cout << std::setw(10) << p.protocol
                  << std::setw(15) << std::fixed << std::setprecision(1) << p.handshake_rtt << "x"
                  << std::setw(15) << p.ongoing_latency << "x"
                  << std::setw(15) << p.cpu_overhead << "x"
                  << "  " << p.use_case
                  << std::endl;
    }
    
    std::cout << "\n" << GREEN << "QUIC Performance Characteristics:" << RESET << std::endl;
    ...
    std::cout << "\n" << BLUE << "Benchmark Results (1000 requests):" << RESET << std::endl;
...
void demo_use_cases() {
    ...
    std::cout << YELLOW << "QUIC adoption and use cases..." << RESET << std::endl;
    ...
    std::cout << "\n" << MAGENTA << "Deployment Statistics (2024):" << RESET << std::endl;
...
void demo_configuration() {
    ...
    std::cout << YELLOW << "QUIC configuration for different scenarios..." << RESET << std::endl;
    ...
    std::cout << "  • initial_max_data: Initial flow control limit" << std::endl;
    std::cout << "  • initial_max_streams: Maximum concurrent streams" << std::endl;
    ...
    std::cout << "  • congestion_control: CC algorithm (cubic, bbr)" << std::endl;
    ...
    std::cout << GREEN << "✓ Configuration demo completed" << RESET << std::endl;
}

int main() {
    std::cout << CYAN;
    std::cout << "╔═══════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║               Psyne QUIC Transport Demo                   ║" << std::endl;
    std::cout << "╚═══════════════════════════════════════════════════════════╝" << std::endl;
    std::cout << RESET;
    ...
    try {
        ...
        if (!is_quic_supported()) {
            std::cout << YELLOW << "Note: This is a conceptual demo. Full QUIC implementation " << std::endl;
            std::cout << "would require integration with libraries like quiche or msquic." << RESET << std::endl;
        }
        ...
        std::cout << GREEN << "QUIC implementation provides:" << RESET << std::endl;
        std::cout << "  ✓ Modern transport for low-latency applications" << RESET << std::endl;
        std::cout << "  ✓ Built-in security (TLS 1.3)" << RESET << std::endl;
        std::cout << "  ✓ Stream multiplexing without head-of-line blocking" << RESET << std::endl;
        ...
        std::cout << "  • Mobile applications with network changes" << std::endl;
