/**
 * @file compression_demo.cpp
 * @brief EXPERIMENTAL: Compression support (planned for v2.0)
 * 
 * WARNING: Compression is experimental and may change significantly.
 * For v1.3.0, focus on zero-copy performance without compression.
 * Compression only makes sense for network transfers of compressible data.
 */
...
int main() {
    try {
        std::cout << "Psyne Compression Demo (EXPERIMENTAL)" << std::endl;
        std::cout << "====================================" << std::endl;
        ...
        std::cout << "\nCreating TCP server with compression enabled..."
                  << std::endl;
        auto server =
            create_channel("tcp://:8888", 1024 * 1024, ChannelMode::SPSC,
                           ChannelType::SingleType, true, comp_config);

        // Give server time to start
        std::this_thread::sleep_for(std::chrono::milliseconds(100));

        // Create TCP client with compression
        std::cout << "Creating TCP client with compression enabled..."
                  << std::endl;
        ...
        std::cout << "Testing compression with various message sizes..."
                  << std::endl;
        ...
        for (size_t size : test_sizes) {
            ...
            if (recv_msg && recv_msg->size() == size) {
                std::cout << "  ✓ Round-trip successful in " << duration.count()
                          << " μs" << std::endl;
                ...
                if (client->has_metrics()) {
                    ...
                    std::cout << "  Original size: " << original_size
                              << " bytes" << std::endl;
                    std::cout << "  Transmitted: " << send_metrics.bytes_sent
                              << " bytes";

                    if (send_metrics.bytes_sent < original_size) {
                        float compression_ratio =
                            (float)original_size / send_metrics.bytes_sent;
                        std::cout
                ...
                std::cout << "  ✗ Failed to receive message or size mismatch"
                          << std::endl;
            }
        }
        ...
        auto client_no_comp = create_channel(
            "tcp://localhost:8889", 1024 * 1024, ChannelMode::SPSC,
            ChannelType::SingleType, true, no_comp_config);
        auto server_no_comp =
            create_channel("tcp://:8889", 1024 * 1024, ChannelMode::SPSC,
                           ChannelType::SingleType, true, no_comp_config);
        ...
        std::cout << "\nTesting " << test_size
                  << " elements without compression:" << std::endl;
        ...
        if (recv_msg_no_comp && client_no_comp->has_metrics()) {
            ...
            std::cout << "  Transmitted without compression: "
                      << metrics.bytes_sent << " bytes" << std::endl;
        }
