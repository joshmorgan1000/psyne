...
struct TensorMessage {
    enum class TensorType { WEIGHTS, GRADIENTS, ACTIVATIONS };
 ...
 */
struct SPSCPattern {
    void *coordinate_allocation(void *slab, size_t max_messages,
                                size_t message_size) {
    ...
    size_t max_producers() const {
        return 1;
    }
    size_t max_consumers() const {
        return 1;
    }
...
template <typename MessageType, typename SubstrateType, typename PatternType>
    requires psyne::concepts::ChannelConfiguration<MessageType, SubstrateType,
                                                   PatternType>
struct TDTChannel {
    explicit TDTChannel(size_t slab_size = 1024 * 1024)
        ...
        log_info("  Cross-process: ",
                 substrate_.is_cross_process() ? "Yes" : "No");
    }
    ...
    TensorLens<MessageType> create_tensor_message(size_t tensor_size) {
        ...
        void *memory = pattern_.coordinate_allocation(
            slab_memory_, slab_size_ / total_size, total_size);
...
void benchmark_tensor_compression() {
    ...
    std::vector<TestConfig> configs = {
        {"Small Weights (64x64x3)", 64, 64, 3,
         TensorMessage::TensorType::WEIGHTS},
        {"Large Weights (512x512x64)", 512, 512, 64,
         TensorMessage::TensorType::WEIGHTS},
        {"Sparse Gradients (256x256x32)", 256, 256, 32,
         TensorMessage::TensorType::GRADIENTS},
        {"ReLU Activations (128x128x128)", 128, 128, 128,
         TensorMessage::TensorType::ACTIVATIONS},
    };
    ...
    for (const auto &config : configs) {
        ...
        auto compress_ms =
            std::chrono::duration<double, std::milli>(compress_time).count();
        double compress_speed_mbps =
            (total_size / 1024.0 / 1024.0) / (compress_ms / 1000.0);
        ...
        auto decompress_ms =
            std::chrono::duration<double, std::milli>(decompress_time).count();
        double decompress_speed_mbps =
            (total_size / 1024.0 / 1024.0) / (decompress_ms / 1000.0);

        // Verify correctness
        bool correct = (decompressed.size() == total_size) &&
                       (std::memcmp(test_data.get(), decompressed.data(),
                                    total_size) == 0);

        log_info("  Original size: ", total_size / 1024, " KB");
        log_info("  Compressed size: ", compressed.compressed_size() / 1024,
                 " KB");
        log_info("  Compression ratio: ", std::fixed, std::setprecision(2),
                 compressed.compression_ratio, "x");
        log_info("  Compress speed: ", std::fixed, std::setprecision(1),
                 compress_speed_mbps, " MB/s");
        log_info("  Decompress speed: ", std::fixed, std::setprecision(1),
                 decompress_speed_mbps, " MB/s");
...
void run_tdt_server() {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        log_info("Received tensor ", tensor->tensor_id, " (", tensor->width,
                 "x", tensor->height, "x", tensor->channels, ") from layer ",
                 tensor->layer_id);
...
void run_tdt_client() {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        TensorMessage::TensorType type =
            static_cast<TensorMessage::TensorType>(i % 3);
        tensor->fill_with_realistic_data(type);

        client_channel.send_tensor(tensor);

        log_info("Sent tensor ", tensor->tensor_id,
                 " (type: ", static_cast<int>(type), ") to server");
...
void run_udp_demo() {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        tensor->height = 64;
        tensor->channels = 1;
        tensor->fill_with_realistic_data(
            TensorMessage::TensorType::ACTIVATIONS);
