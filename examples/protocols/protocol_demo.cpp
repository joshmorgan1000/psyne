...
struct MockNetworkSubstrate {
    ...
    void transport_send(void *data, size_t size) {
        ...
        last_sent_data.assign(static_cast<uint8_t *>(data),
    ...
    const std::vector<uint8_t> &get_last_data() const {
...
template <typename MessageType, typename SubstrateType, typename ProtocolType>
    requires psyne::concepts::Substrate<SubstrateType> &&
             psyne::concepts::Protocol<ProtocolType>
struct ProtocolChannel {
    ...
    void send_message(MessageType *message) {
        ...
        if (protocol_.should_transform(message, message_size)) {
            ...
            log_info("Channel: Sent ", message_size, " bytes as ",
                     encoded_data.size(), " bytes (",
                     protocol_.transformation_ratio(), "x ratio)");
        } else {
            log_info(
                "Channel: Protocol passing through without transformation");
    ...
    MessageType *receive_message() {
        // This is simplified - in real implementation, we'd need to know the
        // size For demo purposes, we'll decode the last sent data
        ...
        try {
            ...
            MessageType *result =
                create_message(decoded_data.size() - sizeof(MessageType));
    ...
    void update_network_conditions(double bandwidth_mbps, double latency_ms,
                                   double cpu_usage) {
    ...
    const SubstrateType &substrate() const {
        return substrate_;
    }
...
void demonstrate_protocol_intelligence() {
    ...
    ProtocolChannel<TensorMessage, MockNetworkSubstrate, TDTCompressionProtocol>
        channel;
    ...
    // Test 1: Small tensor (should pass through)
    {
        ...
        auto *small_tensor =
            channel.create_message(32 * sizeof(float)); // 128 bytes
        ...
        log_info("Result: ", original_size, " -> ", sent_size,
                 " bytes (passthrough expected)");
    }
    ...
    // Test 2: Large tensor on fast network (should pass through)
    {
        ...
        channel.update_network_conditions(1000.0, 1.0,
                                          0.3); // Fast network, low CPU

        auto *large_tensor =
            channel.create_message(256 * 256 * 3 * sizeof(float)); // 768KB
        ...
        size_t sent_size =
            channel.substrate().get_total_bytes_sent() - baseline_sent;
        log_info("Result: ", original_size, " -> ", sent_size,
                 " bytes (passthrough on fast network)");
    }
    ...
    // Test 3: Large tensor on slow network (should compress)
    {
        ...
        channel.update_network_conditions(50.0, 10.0,
                                          0.4); // Slow network, moderate CPU

        auto *compress_tensor =
            channel.create_message(128 * 128 * 32 * sizeof(float)); // 2MB
        ...
        double compression_ratio =
            static_cast<double>(original_size) / sent_size;
    ...
    // Test 4: High CPU usage (should pass through)
    {
        ...
        channel.update_network_conditions(30.0, 20.0,
                                          0.9); // Slow network but high CPU

        auto *cpu_tensor =
            channel.create_message(64 * 64 * 16 * sizeof(float)); // 256KB
        ...
        size_t sent_size =
            channel.substrate().get_total_bytes_sent() - baseline_sent;
        log_info("Result: ", original_size, " -> ", sent_size,
                 " bytes (passthrough due to high CPU)");
    }
    ...
    // Test 5: Round-trip test
    {
        ...
        auto *roundtrip_tensor =
            channel.create_message(100 * 100 * 8 * sizeof(float)); // 320KB
        ...
        if (received_tensor) {
            bool data_matches =
                (std::memcmp(original_data.data(), received_tensor,
                             original_size) == 0);
            log_info("Round-trip test: ", data_matches ? "✅ PASS" : "❌ FAIL");
            log_info("Original tensor ID: ",
                     *reinterpret_cast<uint32_t *>(original_data.data()));
    ...
    log_info(
        "Key insight: Protocols provide intelligent semantic transformation");
...
void demonstrate_substrate_composability() {
    ...
    ProtocolChannel<TensorMessage, TCPSubstrate, TDTCompressionProtocol>
        tcp_channel;
    ProtocolChannel<TensorMessage, UDPSubstrate, TDTCompressionProtocol>
        udp_channel;
    ProtocolChannel<TensorMessage, SharedMemSubstrate, TDTCompressionProtocol>
        shm_channel;
