...
__global__ void process_tensor_kernel(float* data, uint32_t size, float scalar) {
...
void tensor_producer(std::shared_ptr<GPUChannel> channel, 
                    size_t num_tensors,
                    uint32_t width, uint32_t height) {
    log_info("Producer: Creating ", num_tensors, " tensors of size ",
              width, "x", height);
...
void tensor_consumer(std::shared_ptr<GPUChannel> channel,
                    size_t num_tensors,
                    uint32_t width, uint32_t height) {
    ...
    log_info("Consumer: Processed ", num_tensors, " tensors in ",
              duration, " ms");
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (argc > 1) num_tensors = std::stoul(argv[1]);
        if (argc > 2) tensor_width = std::stoul(argv[2]);
        if (argc > 3) tensor_height = std::stoul(argv[3]);
        ...
        config.memory_flags = static_cast<uint32_t>(GPUMemoryFlags::UNIFIED_MEMORY);
#else
        // Use host-visible coherent memory
        config.memory_flags = static_cast<uint32_t>(GPUMemoryFlags::HOST_VISIBLE) |
        ...
        log_info("  Total data: ", (stats.bytes_sent + stats.bytes_received)
