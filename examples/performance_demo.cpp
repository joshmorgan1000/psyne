...
void demonstrate_simd_operations() {
    ...
    auto simd_ms =
        std::chrono::duration_cast<std::chrono::microseconds>(simd_time)
            .count();
    auto scalar_ms =
        std::chrono::duration_cast<std::chrono::microseconds>(scalar_time)
            .count();

    std::cout << "Vector addition (1000 iterations, " << size
              << " elements):\n";
...
void demonstrate_huge_pages() {
    ...
    std::cout << "Huge page support: " << (info.supported ? "Yes" : "No")
              << "\n";
    std::cout << "Transparent huge pages: "
              << (info.transparent_enabled ? "Yes" : "No") << "\n";
    ...
    if (allocation.ptr) {
        ...
        std::cout << "Using huge pages: "
                  << (allocation.is_huge_page ? "Yes" : "No") << "\n";
        ...
        auto write_ms =
            std::chrono::duration_cast<std::chrono::milliseconds>(write_time)
                .count();
        auto read_ms =
        ...
        std::cout << "Failed to allocate huge pages: "
...
void demonstrate_performance_manager() {
    ...
    std::cout << "  Optimal configuration: "
              << (advice.optimal_configuration ? "Yes" : "No") << "\n";
    std::cout << "  Estimated gain: " << advice.estimated_performance_gain
              << "x\n";
...
int main() {
    ...
    try {
        ...
        if (enable_performance_optimizations(config)) {
            ...
            std::cout
                << "Some performance optimizations failed to initialize\n";
        }
