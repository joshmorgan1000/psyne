...
void test_channel_inspection() {
    ...
    auto channel =
        create_channel("memory://debug_test", 1024 * 1024, ChannelMode::SPSC,
                       ChannelType::SingleType, true);
    ...
    std::cout << debug::ChannelInspector::visualize_buffer(*channel)
              << std::endl;
    ...
    std::cout << debug::ChannelInspector::get_status_report(*channel)
              << std::endl;
...
void test_performance_profiling() {
    ...
    auto channel =
        create_channel("memory://perf_test", 1024 * 1024, ChannelMode::SPSC,
                       ChannelType::SingleType, true);
...
void test_message_tracing() {
    ...
    auto channel = create_channel(channel_uri, 1024 * 1024, ChannelMode::SPSC,
                                  ChannelType::SingleType, true);
    ...
    debug::MessageTracer::trace_event(channel_uri, "send", 400, 1,
                                      "FloatVector with 100 elements");
    debug::MessageTracer::trace_event(channel_uri, "receive", 400, 1,
                                      "Successfully received");
    debug::MessageTracer::trace_event(channel_uri, "send", 800, 1,
                                      "FloatVector with 200 elements");
    debug::MessageTracer::trace_event(channel_uri, "send", 1200, 1,
                                      "FloatVector with 300 elements");
    debug::MessageTracer::trace_event(channel_uri, "receive", 800, 1,
                                      "Successfully received");
    debug::MessageTracer::trace_event(channel_uri, "drop", 1200, 1,
                                      "Buffer full, message dropped");

    // Add a small delay to see timestamp differences
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
    debug::MessageTracer::trace_event(channel_uri, "error", 0, 0,
                                      "Connection timeout");

    // Generate trace report
    std::cout << "\nTrace Report:" << std::endl;
    std::cout << debug::MessageTracer::generate_trace_report(channel_uri)
              << std::endl;
...
void test_channel_monitoring() {
    ...
    auto channel1 =
        create_channel("memory://monitor1", 1024 * 1024, ChannelMode::SPSC,
                       ChannelType::SingleType, true);
    auto channel2 =
        create_channel("memory://monitor2", 512 * 1024, ChannelMode::SPSC,
                       ChannelType::SingleType, true);
    ...
    // Set alert callback
    monitor.set_alert_callback([](const std::string &name,
                                  debug::ChannelHealth health,
                                  const std::string &details) {
        std::cout << "🚨 ALERT: " << name << " health changed to ";
        switch (health) {
        case debug::ChannelHealth::Warning:
            std::cout << "WARNING";
            break;
        case debug::ChannelHealth::Critical:
            std::cout << "CRITICAL";
            break;
        case debug::ChannelHealth::Disconnected:
...
void test_debug_dashboard() {
    ...
    auto channel1 =
        create_channel("memory://dash1", 1024 * 1024, ChannelMode::SPSC,
                       ChannelType::SingleType, true);
    auto channel2 =
        create_channel("memory://dash2", 512 * 1024, ChannelMode::SPSC,
                       ChannelType::SingleType, true);

    // The debug console would normally register these, but for demo we'll just
    // show the concept
...
int main() {
    try {
        ...
        std::cout << "\n✅ All debugging features demonstrated successfully!"
                  << std::endl;
        std::cout << "\nDebugging utilities available:" << std::endl;
        std::cout << "  • Channel inspection and health monitoring"
                  << std::endl;
