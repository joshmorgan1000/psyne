...
int main() {
    ...
    try {
        ...
        if (!cuda_available) {
            std::cout << "\nCUDA not available on this system. Exiting."
                      << std::endl;
            return 0;
        }
        ...
        std::cout << "Total memory: "
                  << (gpu_context->total_memory() / (1024 * 1024)) << " MB"
                  << std::endl;
        std::cout << "Available memory: "
                  << (gpu_context->available_memory() / (1024 * 1024)) << " MB"
                  << std::endl;
        std::cout << "Unified memory: "
                  << (gpu_context->is_unified_memory() ? "Yes" : "No")
                  << std::endl;

        // Create channel for vector communication
        print_separator("Creating Communication Channel");
        auto channel =
            std::make_unique<Channel>(Transport::Memory, "cuda_demo");
        ...
        std::cout << "Initializing vectors with " << vector_size
                  << " elements..." << std::endl;
        ...
        std::cout << "GPU transfer completed in " << transfer_duration << " µs"
                  << std::endl;
        std::cout << "Vector A on GPU: " << (vec_a.is_on_gpu() ? "Yes" : "No")
                  << std::endl;
        std::cout << "Vector B on GPU: " << (vec_b.is_on_gpu() ? "Yes" : "No")
                  << std::endl;
        ...
        auto scale_duration =
            std::chrono::duration_cast<std::chrono::microseconds>(scale_time -
                                                                  start_time)
                .count();
        ...
        auto add_duration =
            std::chrono::duration_cast<std::chrono::microseconds>(add_time -
                                                                  start_time)
                .count();

        std::cout << "GPU vector addition completed in " << add_duration
                  << " µs" << std::endl;
        ...
        std::vector<float> cpu_a(vector_size), cpu_b(vector_size),
            cpu_result(vector_size);
        for (size_t i = 0; i < vector_size; ++i) {
        ...
        auto cpu_duration =
            std::chrono::duration_cast<std::chrono::microseconds>(cpu_time -
                                                                  start_time)
                .count();

        std::cout << std::fixed << std::setprecision(2);
        std::cout << "CPU operation time:    " << cpu_duration << " µs"
                  << std::endl;
        std::cout << "GPU operation time:    "
                  << (scale_duration + add_duration) << " µs" << std::endl;
        std::cout << "GPU transfer overhead: " << transfer_duration << " µs"
                  << std::endl;
        std::cout << "Total GPU time:        "
                  << (transfer_duration + scale_duration + add_duration)
                  << " µs" << std::endl;

        if (cpu_duration > 0) {
            double speedup = static_cast<double>(cpu_duration) /
                             (scale_duration + add_duration);
            std::cout << "GPU compute speedup:   " << speedup << "x"
                      << std::endl;
        }
        ...
        std::cout << "Total memory per vector: "
                  << (vector_size * sizeof(float) / 1024) << " KB" << std::endl;
        std::cout << "Total GPU memory used: "
                  << (3 * vector_size * sizeof(float) / 1024) << " KB"
                  << std::endl;
...
#else
        std::cout
            << "CUDA support not compiled in. Please rebuild with CUDA enabled."
            << std::endl;
        return 1;
#endif
