...
struct TDTConfig {
    ...
    double bandwidth_threshold_mbps =
        100.0;                        // Switch strategies based on bandwidth
...
struct TDTEncodedData {
    ...
    size_t encoded_size() const {
        ...
        return total + cluster_mapping.size() * sizeof(int) +
               sizeof(TDTEncodedData);
    }
    ...
    std::vector<uint8_t> serialize() const {
        ...
        result.insert(result.end(), (uint8_t *)&orig_size,
                      (uint8_t *)&orig_size + 4);

        uint32_t num_streams = static_cast<uint32_t>(streams.size());
        result.insert(result.end(), (uint8_t *)&num_streams,
                      (uint8_t *)&num_streams + 4);

        uint32_t word_sz = static_cast<uint32_t>(word_size);
        result.insert(result.end(), (uint8_t *)&word_sz,
                      (uint8_t *)&word_sz + 4);

        // Cluster mapping
        uint32_t mapping_size = static_cast<uint32_t>(cluster_mapping.size());
        result.insert(result.end(), (uint8_t *)&mapping_size,
                      (uint8_t *)&mapping_size + 4);
        result.insert(result.end(), (uint8_t *)cluster_mapping.data(),
                      (uint8_t *)cluster_mapping.data() +
                          cluster_mapping.size() * sizeof(int));

        // Streams
        for (const auto &stream : streams) {
            uint32_t stream_size = static_cast<uint32_t>(stream.size());
            result.insert(result.end(), (uint8_t *)&stream_size,
                          (uint8_t *)&stream_size + 4);
    ...
    static TDTEncodedData deserialize(const std::vector<uint8_t> &data) {
        ...
        std::memcpy(result.cluster_mapping.data(), data.data() + offset,
        ...
        for (uint32_t i = 0; i < num_streams; ++i) {
            ...
            std::memcpy(result.streams[i].data(), data.data() + offset,
                        stream_size);
            offset += stream_size;
        }

        result.compression_ratio =
            static_cast<double>(result.original_size) / result.encoded_size();
        return result;
    }
};
...
public:
    explicit TDTCompressionProtocol(const TDTConfig &config = {})
        : config_(config) {}
    ...
    bool should_transform(void *data, size_t size) {
        ...
        if (size < config_.min_tensor_size)
            return false;

        // Don't compress if CPU is busy
        if (cpu_usage_.load() > config_.cpu_usage_threshold)
            return false;

        // Only compress tensor-like data
        if (!is_tensor_data(data, size))
            return false;
    ...
    void analyze_data(void *data, size_t size) {
        ...
        for (int byte_pos = 0; byte_pos < config_.word_size; ++byte_pos) {
            auto features = extract_features(static_cast<uint8_t *>(data),
                                             sample_indices, byte_pos);
    ...
    std::vector<uint8_t> encode(void *data, size_t size) {
        ...
        try {
            ...
            log_debug("TDT: Compressed ", size, " -> ", result.size(),
                      " bytes (", compressed.compression_ratio, "x) in ",
                      last_encode_time_ms_, "ms");
    ...
    std::vector<uint8_t> decode(const std::vector<uint8_t> &encoded) {
        ...
        try {
            ...
            log_debug("TDT: Decompressed ", encoded.size(), " -> ",
                      result.size(), " bytes in ", last_decode_time_ms_, "ms");
    ...
    const char *protocol_name() const {
        return "TDT-Compression";
    }
    bool is_lossless() const {
        return true;
    }
    double transformation_ratio() const {
...
private:
    ...
    TDTEncodedData compress_tdt(const uint8_t *data, size_t size) {
        if (size == 0 || size % config_.word_size != 0) {
            throw std::invalid_argument(
                "Data size must be multiple of word size");
        }
        ...
        for (int byte_pos = 0; byte_pos < config_.word_size; ++byte_pos) {
            features[byte_pos] =
                extract_features(data, sample_indices, byte_pos);
        }
        ...
        result.compression_ratio =
            static_cast<double>(size) / result.encoded_size();
    ...
    bool is_tensor_data(void *data, size_t size) const {
        // Simple heuristic: data is tensor-like if size is multiple of
        // float32/float64
    ...
    std::vector<size_t> generate_sample_indices(size_t word_count) {
        size_t sample_count =
            static_cast<size_t>(word_count * config_.sample_fraction);
    ...
    ByteFeatures extract_features(const uint8_t *data,
        ...
        features.unique_count =
            std::count_if(features.histogram.begin(), features.histogram.end(),
                          [](int count) { return count > 0; });

        features.mean =
            std::accumulate(byte_values.begin(), byte_values.end(), 0.0) /
            byte_values.size();
        ...
        features.entropy =
            calculate_entropy(features.histogram, byte_values.size());
        features.autocorrelation = calculate_autocorrelation(byte_values);

        return features;
    }

    double calculate_entropy(const std::array<int, 256> &histogram,
                             size_t total_count) {
        double entropy = 0.0;
    ...
    std::vector<int>
    perform_clustering(const std::vector<ByteFeatures> &features) {
        ...
        double entropy_threshold =
            std::accumulate(entropies.begin(), entropies.end(), 0.0) /
            entropies.size();
    ...
    separate_byte_streams(const uint8_t *data, size_t size,
                          const std::vector<int> &cluster_mapping) {
        int max_cluster =
            *std::max_element(cluster_mapping.begin(), cluster_mapping.end());
    ...
    std::vector<uint8_t>
    simple_rle_decompress(const std::vector<uint8_t> &compressed) {
    ...
    std::vector<uint8_t>
    recombine_byte_streams(const std::vector<std::vector<uint8_t>> &streams,
        ...
        for (size_t word_idx = 0; word_idx < word_count; ++word_idx) {
            for (int byte_pos = 0; byte_pos < compressed.word_size;
                 ++byte_pos) {
                ...
                if (stream_positions[cluster] < streams[cluster].size()) {
                    result[byte_idx] =
                        streams[cluster][stream_positions[cluster]];
