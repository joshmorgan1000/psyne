...
concept Protocol = requires(P protocol, void* data, size_t size) {
    ...
    { protocol.update_network_metrics(0.0, 0.0) } -> std::same_as<void>;  // bandwidth, latency
...
concept ProtocolStack = requires(PS stack, void* data, size_t size) {
    ...
    { stack.encode_stack(data, size) } -> std::convertible_to<std::vector<uint8_t>>;
    { stack.decode_stack(std::declval<const std::vector<uint8_t>&>()) } -> std::convertible_to<std::vector<uint8_t>>;
 ...
 * 
 * // Composable Usage:
 * Channel<TensorMsg, TCPSubstrate, SPSCPattern, TDTProtocol> compressed_channel;
 * Channel<TensorMsg, TCPSubstrate, SPSCPattern, ProtocolStack<TDTProtocol, AESProtocol>> secure_channel;
 * 
