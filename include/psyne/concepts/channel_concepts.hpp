...
concept SubstrateType =
    requires(S substrate, T *msg_ptr, size_t size_bytes,
        ...
        {
            substrate.async_send_message(msg_ptr, listeners)
...
concept PatternType = requires(P pattern, T *slab, size_t max_messages,
    ...
    // Async operations
    {
        pattern.async_receive(io_context, timeout)
    } -> std::same_as<boost::asio::awaitable<T *>>;
...
concept ChannelConfiguration = (MessageType<T, S> || BasicMessageType<T>) &&
                               SubstrateType<S, T> && PatternType<P, T, S>;
 ...
 */
template <typename S, typename T>
concept HighPerformanceSubstrate =
    SubstrateType<S, T> && requires(S substrate) {
