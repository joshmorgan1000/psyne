 ...
 */
template<typename A>
concept AllocatorType = requires(A allocator, size_t size, size_t alignment, void* ptr) {
...
concept SubstrateType = requires(S substrate, size_t size_bytes) {
    ...
    { substrate.deallocate_slab(static_cast<void*>(nullptr)) } -> std::same_as<void>;
    ...
    { substrate.send_raw_message(static_cast<void*>(nullptr), size_t{}) } -> std::same_as<void>;
};
...
concept PatternType = requires(P pattern, void* slab, size_t max_messages) {
    ...
    { pattern.try_allocate_raw(slab, max_messages, sizeof(int)) } -> std::convertible_to<void*>;
...
concept ChannelConfiguration = MessageWithAllocator<T, A> && 
                              SubstrateType<S> && 
                              PatternType<P> &&
                              AllocatorType<A>;
