...
public:
    ...
    DynamicVectorMessage(Substrate& substrate, size_t size) 
        ...
        if (size_ > 0) {
            ...
            if constexpr (requires { substrate.allocate_additional(size_t{}); }) {
                data_ = static_cast<float*>(substrate_.allocate_additional(size_ * sizeof(float)));
            } else {
    ...
    DynamicVectorMessage(Substrate& substrate, std::vector<float> initial_data)
        ...
        if (size_ > 0) {
            if constexpr (requires { substrate.allocate_additional(size_t{}); }) {
                data_ = static_cast<float*>(substrate_.allocate_additional(size_ * sizeof(float)));
                std::copy(initial_data.begin(), initial_data.end(), data_.get());
            } else {
                data_ = std::make_unique<float[]>(size_);
                std::copy(initial_data.begin(), initial_data.end(), data_.get());
            }
    ...
    DynamicVectorMessage(DynamicVectorMessage&& other) noexcept 
        : substrate_(other.substrate_), size_(other.size_), data_(std::move(other.data_)) {
        other.size_ = 0;
    }
...
public:
    ...
    void set_content(const std::string& new_content) {
        ...
        if constexpr (requires { substrate_.get().compress_string(std::string{}); }) {
...
public:
    ...
    GPUTensorMessage(Substrate& substrate, std::vector<size_t> shape) 
        ...
        if constexpr (requires { substrate.allocate_gpu_memory(size_t{}); }) {
            gpu_data_ = substrate_.allocate_gpu_memory(total_elements * sizeof(float));
        }
    }
    ...
    ~GPUTensorMessage() {
        if (gpu_data_ && requires { substrate_.get().deallocate_gpu_memory(gpu_data_); }) {
...
public:
    explicit SelfDescribingMessage(Substrate& substrate) : substrate_(substrate) {
        // Could register message type with substrate
        if constexpr (requires { substrate.register_message_type("SelfDescribing"); }) {
