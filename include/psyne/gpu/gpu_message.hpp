...
public:
    ...
    void reserve(size_type new_capacity) {
        ...
        T* new_data = static_cast<T*>(std::aligned_alloc(64, new_capacity * sizeof(T)));
        if (!new_data) {
            throw std::bad_alloc();
        }
...
struct MessageTraits<GPUVector<T>> {
    ...
    static void serialize(uint8_t* buffer, const GPUVector<T>& vec) {
        ...
        *reinterpret_cast<size_t*>(buffer) = vec.size();
        ...
        // Write data
        if (vec.size() > 0) {
            std::memcpy(buffer, vec.begin(), vec.size() * sizeof(T));
        }
    }
    ...
    static GPUVector<T> deserialize(const uint8_t* buffer, Channel& channel) {
        ...
        // Read data
        if (size > 0) {
            std::memcpy(vec.begin(), buffer, size * sizeof(T));
        }
