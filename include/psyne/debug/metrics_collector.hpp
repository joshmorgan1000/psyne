...
public:
    ...
    Percentiles get_percentiles() const {
        ...
        for (size_t i = 0; i < NUM_BUCKETS; ++i) {
            ...
            if (result.p50 == 0 && running_sum >= p50_target)
                result.p50 = bucket_value;
            if (result.p90 == 0 && running_sum >= p90_target)
                result.p90 = bucket_value;
            if (result.p95 == 0 && running_sum >= p95_target)
                result.p95 = bucket_value;
            if (result.p99 == 0 && running_sum >= p99_target)
                result.p99 = bucket_value;
            if (result.p999 == 0 && running_sum >= p999_target)
                result.p999 = bucket_value;
        }
...
public:
    ...
    void configure(const MetricsConfig &config) {
        ...
        if (config_.file_output) {
            ...
            output_file_
                << "timestamp_ms,channel,transport,msg_sent,msg_recv,"
    ...
    void start() {
        ...
        running_ = true;
        collector_thread_ =
            std::thread(&MetricsCollector::collection_loop, this);

        if (config_.live_dashboard) {
            dashboard_thread_ =
                std::thread(&MetricsCollector::dashboard_loop, this);
        }
    }
    ...
    void record_send(const std::string &channel, size_t size,
                     uint64_t sequence) {
    ...
    void record_receive(const std::string &channel, size_t size,
                        uint64_t sequence, uint64_t latency_ns) {
        auto it = channels_.find(channel);
        if (it != channels_.end()) {
    ...
    void record_allocation_failure(const std::string &channel,
    ...
    void update_buffer_state(const std::string &channel, size_t used,
                             size_t available) {
        auto it = channels_.find(channel);
        if (it != channels_.end()) {
    ...
    void record_allocation(const std::string &channel, size_t size) {
        ...
        if (it != channels_.end()) {
            ...
            while (current > peak &&
                   !it->second.peak_allocations.compare_exchange_weak(peak,
                                                                      current))
                ;
        }
    }
...
private:
        ...
        while (running_) {
            {
                ...
                cv_.wait_for(
                    lock,
                    std::chrono::milliseconds(config_.sampling_interval_ms),
        ...
        for (auto &[name, metrics] : channels_) {
            ...
            if (duration > 0) {
                ...
                double msg_rate_send =
                    (msg_sent - metrics.last_messages_sent) / duration_sec;
                double msg_rate_recv =
                    (msg_recv - metrics.last_messages_received) / duration_sec;
                double bandwidth_send = (bytes_sent - metrics.last_bytes_sent) /
                                        1024.0 / 1024.0 / duration_sec;
                double bandwidth_recv =
                    (bytes_recv - metrics.last_bytes_received) / 1024.0 /
                    1024.0 / duration_sec;
                ...
                if (config_.file_output && output_file_.is_open()) {
                    output_file_ << timestamp << "," << name << ","
                                 << static_cast<int>(metrics.transport) << ","
                                 << msg_sent << "," << msg_recv << ","
                                 << bytes_sent << "," << bytes_recv << ","
                                 << std::fixed << std::setprecision(2)
                                 << msg_rate_send << "," << msg_rate_recv << ","
                                 << bandwidth_send << "," << bandwidth_recv
        ...
        while (running_) {
...

            {
                ...
                std::cout << std::left << std::setw(20) << "Channel"
                          << std::setw(10) << "Transport" << std::setw(15)
                          << "Send Rate" << std::setw(15) << "Recv Rate"
                          << std::setw(15) << "Bandwidth" << std::setw(20)
                          << "Latency (p50/p99)" << std::setw(15)
                ...
                for (const auto &[name, metrics] : channels_) {
                    ...
                    if (duration > 0) {
                        double msg_rate_send = (metrics.messages_sent -
                                                metrics.last_messages_sent) /
                                               duration;
                        double msg_rate_recv =
                            (metrics.messages_received -
                             metrics.last_messages_received) /
                            duration;
                        double bandwidth =
                            (metrics.bytes_sent - metrics.last_bytes_sent) /
                            1024.0 / 1024.0 / duration;

                        auto percentiles =
                            metrics.latency_histogram.get_percentiles();
                        double buffer_usage =
                            100.0 * metrics.bytes_used /
                            (metrics.bytes_used + metrics.bytes_available);

                        std::cout
                            << std::left << std::setw(20) << name
                            << std::setw(10)
                            << transport_to_string(metrics.transport)
                            << std::setw(15) << std::fixed
                            << std::setprecision(0) << msg_rate_send
                            << std::setw(15) << msg_rate_recv << std::setw(15)
                            << std::setprecision(2) << bandwidth
                            << std::setw(20)
                            << (std::to_string(percentiles.p50 / 1000) + "/" +
                                std::to_string(percentiles.p99 / 1000) + "µs")
                            << std::setw(15) << std::setprecision(1)
                            << buffer_usage << "%" << std::endl;
                    }
                }
                ...
                std::cout << "Total data: " << total_bytes / 1024 / 1024
                          << " MB" << std::endl;
                ...
                if (config_.memory_tracking) {
                    ...
                    for (const auto &[name, metrics] : channels_) {
                        std::cout
                            << name << ": " << metrics.current_allocations
                            << " allocations, "
    ...
    void add_event(EventType type, const std::string &channel,
                   uint64_t sequence, size_t size, const std::string &extra) {
        if (!config_.event_tracing)
            return;
    ...
    std::string transport_to_string(ChannelTransport transport) {
        switch (transport) {
        case ChannelTransport::IN_PROCESS:
            return "InProc";
        case ChannelTransport::IPC:
            return "IPC";
        case ChannelTransport::TCP:
            return "TCP";
        case ChannelTransport::UDP:
            return "UDP";
        case ChannelTransport::RDMA:
            return "RDMA";
        default:
            return "Unknown";
        }
...
#define PSYNE_METRICS_REGISTER_CHANNEL(name, transport, capacity)              \
    psyne::debug::MetricsCollector::instance().register_channel(               \
...
#define PSYNE_METRICS_RECORD_RECEIVE(channel, size, seq, latency)              \
    psyne::debug::MetricsCollector::instance().record_receive(channel, size,   \
                                                              seq, latency)
#define PSYNE_METRICS_UPDATE_BUFFER(channel, used, available)                  \
    psyne::debug::MetricsCollector::instance().update_buffer_state(            \
        channel, used, available)
#else
#define PSYNE_METRICS_REGISTER_CHANNEL(name, transport, capacity)
#define PSYNE_METRICS_RECORD_SEND(channel, size, seq)
#define PSYNE_METRICS_RECORD_RECEIVE(channel, size, seq, latency)
