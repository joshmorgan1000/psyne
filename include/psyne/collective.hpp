 ...
 * 
 * Provides high-performance collective operations optimized for AI/ML workloads:
...
public:
    ...
    RankId rank() const override { return rank_; }
    size_t size() const override { return channels_.size() + 1; }
...
public:
     ...
     */
    std::future<void> execute_async(std::span<T> data, CollectiveGroup::RankId root) {
        return std::async(std::launch::async, [this, data, root]() {
            execute(data, root);
        });
    }
...
public:
    ...
    std::future<void> execute_async(std::span<T> data, ReduceOp op) {
        return std::async(std::launch::async, [this, data, op]() {
            execute(data, op);
        });
    }
...
private:
    ...
    void ring_allreduce(std::span<T> data, ReduceFunc reduce_fn) {
        auto ring_group = std::dynamic_pointer_cast<RingCollectiveGroup>(group_);
        if (!ring_group) {
        ...
        for (size_t step = 0; step < size - 1; ++step) {
            ...
            auto next_channel = ring_group->get_channel(ring_group->next_neighbor());
            next_channel->send(std::span<T>(data.data() + send_offset, send_count));
            
            // Receive from previous neighbor
            auto prev_channel = ring_group->get_channel(ring_group->prev_neighbor());
            auto received = prev_channel->receive<std::vector<T>>();
            
            // Reduce received data
            if (received && received->size() == recv_count) {
                for (size_t i = 0; i < recv_count; ++i) {
                    data[recv_offset + i] = reduce_fn(data[recv_offset + i], (*received)[i]);
                }
            }
        }
        ...
        for (size_t step = 0; step < size - 1; ++step) {
            ...
            // Copy received data
            if (received && received->size() == recv_count) {
                std::copy(received->begin(), received->end(), data.begin() + recv_offset);
            }
        }
    }
        ...
        if (rank == root) {
            ...
            for (size_t r = 0; r < size; ++r) {
                ...
                if (r == rank) {
                    ...
                    auto channel = group_->get_channel(r);
                    channel->send(std::span<const T>(send_data.data() + offset, chunk_size));
                }
            }
        } else {
            // Non-root receives from root
            ...
            if (received && received->size() == chunk_size) {
                std::copy(received->begin(), received->end(), recv_data.begin());
            }
        }
...
private:
    void ring_allgather(std::span<const T> send_data, std::span<T> recv_data) {
        ...
        std::copy(send_data.begin(), send_data.end(),
                 recv_data.begin() + rank * chunk_size);
        ...
        for (size_t step = 0; step < size - 1; ++step) {
            ...
            // Send to next neighbor
            auto next_channel = ring_group->get_channel(ring_group->next_neighbor());
            next_channel->send(std::span<T>(recv_data.data() + send_offset, chunk_size));
            ...
            if (received && received->size() == chunk_size) {
                std::copy(received->begin(), received->end(),
                         recv_data.begin() + recv_offset);
            }
        }
    }
    
    void simple_allgather(std::span<const T> send_data, std::span<T> recv_data) {
        ...
        for (size_t root = 0; root < size; ++root) {
            if (rank == root) {
                ...
                std::copy(data.begin(), data.end(),
                         recv_data.begin() + root * chunk_size);
            } else {
                // Receive broadcast
 ...
 */
inline std::shared_ptr<CollectiveGroup> create_collective_group(
    CollectiveGroup::RankId rank,
