...
public:
    ...
    public:
        ...
        PooledPtr(T *ptr, ObjectPool *pool) : ptr_(ptr), pool_(pool) {}
     ...
     */
    explicit ObjectPool(
        size_t initial_size = 16, size_t max_size = 0,
...
public:
    struct Buffer {
        ...
        Buffer(size_t size)
            : data(std::make_unique<uint8_t[]>(size)), capacity(size) {}
...
public:
    ...
    explicit MessagePool(size_t initial_count = 64) : pool_(initial_count) {}
