 ...
 * 
 * Pure concept-based TCP substrate that implements the SubstrateBehavior interface
 * for network communication over TCP sockets.
 */
...
public:
    ...
    void transport_send(void* data, size_t size) override {
        ...
        try {
            ...
            if (socket_ && socket_->is_open()) {
                ...
                boost::asio::write(*socket_, boost::asio::buffer(&size_header, sizeof(size_header)));
            ...
            throw std::runtime_error("TCP send failed: " + std::string(e.what()));
        }
    }
    ...
    void transport_receive(void* buffer, size_t buffer_size) override {
        ...
        try {
            ...
            if (socket_ && socket_->is_open()) {
                ...
                boost::asio::read(*socket_, boost::asio::buffer(&size_header, sizeof(size_header)));
                
                if (size_header > buffer_size) {
                    throw std::runtime_error("TCP: Received message too large for buffer");
                }
                
                // Read data
                boost::asio::read(*socket_, boost::asio::buffer(buffer, size_header));
            ...
            connected_.store(false);
            throw std::runtime_error("TCP receive failed: " + std::string(e.what()));
        }
    }
     ...
     */
    bool try_transport_receive(void* buffer, size_t buffer_size, size_t& received_size) {
        if (!is_connected()) {
            return false;
        }
     ...
     */
    const char* substrate_name() const override { return "SimpleTCP"; }
    bool is_zero_copy() const override { return false; } // Network involves copying
    bool is_cross_process() const override { return true; } // Network is cross-process
     ...
     */
    bool wait_for_connection(std::chrono::milliseconds timeout = std::chrono::milliseconds(5000)) {
    ...
    bool is_server_mode() const { return is_server_; }
...
private:
    void initialize_tcp() {
        try {
            ...
            throw std::runtime_error("TCP initialization failed: " + std::string(e.what()));
        }
    }
