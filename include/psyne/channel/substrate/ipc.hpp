 ...
 * 
 * IPC substrate enables zero-copy communication between processes on the same machine
 * using POSIX shared memory and semaphores for synchronization.
 */
 ...
 * Uses named shared memory objects and semaphores for cross-process coordination.
 */
...
public:
     ...
     * @param create_new If true, creates new shared memory. If false, attaches to existing.
     */
    explicit IPC(const std::string& shm_name = "/psyne_ipc_default", bool create_new = true)
        : shm_name_(shm_name), create_new_(create_new), shm_fd_(-1), shm_ptr_(nullptr), shm_size_(0) {
    ...
    void* allocate_memory_slab(size_t size_bytes) override {
        ...
        if (create_new_) {
            ...
            if (shm_fd_ == -1) {
                throw std::runtime_error("IPC: Failed to create shared memory object: " + std::string(strerror(errno)));
            }
            ...
            if (ftruncate(shm_fd_, size_bytes) == -1) {
                ...
                throw std::runtime_error("IPC: Failed to set shared memory size: " + std::string(strerror(errno)));
            }
            
            // Create producer semaphore (starts at max capacity)
            sem_producer_ = sem_open(sem_producer_name_.c_str(), O_CREAT | O_EXCL, 0666, size_bytes / 64);
            if (sem_producer_ == SEM_FAILED) {
                close(shm_fd_);
                shm_unlink(shm_name_.c_str());
                throw std::runtime_error("IPC: Failed to create producer semaphore: " + std::string(strerror(errno)));
            }
            
            // Create consumer semaphore (starts at 0)
            sem_consumer_ = sem_open(sem_consumer_name_.c_str(), O_CREAT | O_EXCL, 0666, 0);
            if (sem_consumer_ == SEM_FAILED) {
                close(shm_fd_);
                shm_unlink(shm_name_.c_str());
                sem_close(sem_producer_);
                sem_unlink(sem_producer_name_.c_str());
                throw std::runtime_error("IPC: Failed to create consumer semaphore: " + std::string(strerror(errno)));
            }
            ...
            if (shm_fd_ == -1) {
                throw std::runtime_error("IPC: Failed to open existing shared memory: " + std::string(strerror(errno)));
            }
            ...
            if (sem_producer_ == SEM_FAILED) {
                close(shm_fd_);
                throw std::runtime_error("IPC: Failed to open producer semaphore: " + std::string(strerror(errno)));
            }
            ...
            if (sem_consumer_ == SEM_FAILED) {
                close(shm_fd_);
                sem_close(sem_producer_);
                throw std::runtime_error("IPC: Failed to open consumer semaphore: " + std::string(strerror(errno)));
            }
        }
        ...
        if (shm_ptr_ == MAP_FAILED) {
            cleanup();
            throw std::runtime_error("IPC: Failed to map shared memory: " + std::string(strerror(errno)));
        }
    ...
    void transport_send(void* data, size_t size) override {
        ...
        if (sem_post(sem_consumer_) == -1) {
            throw std::runtime_error("IPC: Failed to signal consumer semaphore: " + std::string(strerror(errno)));
        }
    }
    ...
    void transport_receive(void* buffer, size_t buffer_size) override {
        ...
        if (sem_wait(sem_consumer_) == -1) {
            throw std::runtime_error("IPC: Failed to wait on consumer semaphore: " + std::string(strerror(errno)));
        }
        
        // Signal producer that we consumed a message
        if (sem_post(sem_producer_) == -1) {
            throw std::runtime_error("IPC: Failed to signal producer semaphore: " + std::string(strerror(errno)));
        }
    }
    ...
    bool is_zero_copy() const override { return true; } // True shared memory
    bool is_cross_process() const override { return true; } // Cross-process by design
    ...
    void* get_shm_ptr() const { return shm_ptr_; }
    
    /**
     * @brief Check if we're the creator of the shared memory
     */
    bool is_creator() const { return create_new_; }
