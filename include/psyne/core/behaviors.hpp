...
public:
    ...
    virtual void* coordinate_allocation(void* slab_memory, size_t message_size) = 0;
...
class ChannelBridge {
public:
    explicit ChannelBridge(size_t slab_size = 32 * 1024 * 1024) {
    ...
    MessageLens<MessageType> create_message() {
        ...
        void* memory = pattern_->coordinate_allocation(slab_memory_, sizeof(MessageType));
        if (!memory) {
            throw std::runtime_error("Pattern allocation failed");
        }
