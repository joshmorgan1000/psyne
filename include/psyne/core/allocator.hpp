...
public:
    ...
    virtual size_t total_allocated() const {
        return 0;
    }
};
...
public:
    SlabAllocator(T *slab_memory, size_t slab_size, const char *name = "Slab")
        : slab_memory_(slab_memory), slab_size_(slab_size), allocated_bytes_(0),
          allocator_name_(name) {}

    void *allocate(size_t size_bytes, size_t alignment) override {
        // Align current position
        size_t aligned_pos =
            (allocated_bytes_ + alignment - 1) & ~(alignment - 1);
    ...
    size_t available() const {
        return slab_size_ - allocated_bytes_;
    }
    double utilization() const {
        return double(allocated_bytes_) / double(slab_size_);
    }
...
public:
    ...
    size_t get_ring_position() const {
        return current_pos_;
    }
