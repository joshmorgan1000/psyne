...
public:
    ...
    void send_binding_request(std::function<void(bool success, const NetworkAddress& mapped_address)> callback);
    
    // Synchronous binding request with timeout
    std::optional<NetworkAddress> get_mapped_address(std::chrono::milliseconds timeout = std::chrono::seconds(5));
    
    // Keep-alive functionality
    void start_keep_alive(std::chrono::milliseconds interval = std::chrono::seconds(30));
...
private:
    ...
    std::unordered_map<std::string, std::function<void(bool, const NetworkAddress&)>> pending_transactions_;
    
    void initialize_socket();
    void run_worker();
    void run_keep_alive(std::chrono::milliseconds interval);
    std::vector<uint8_t> create_binding_request(const std::string& transaction_id);
    bool parse_binding_response(const std::vector<uint8_t>& data, NetworkAddress& mapped_address);
...
public:
    ...
    void stop_gathering();
    bool is_gathering() const { return gathering_.load(); }
    ...
    bool is_checking() const { return checking_.load(); }
...
private:
    ...
    uint64_t calculate_pair_priority(const RTCIceCandidate& local, const RTCIceCandidate& remote);
    uint32_t calculate_candidate_priority(RTCIceCandidateType type, uint16_t local_pref, uint8_t component_id);
...
public:
    ...
    void allocate_relay(std::function<void(bool success, const NetworkAddress& relay_address)> callback);
    ...
    // Event callbacks
    std::function<void(const NetworkAddress& peer, const void* data, size_t size)> on_data;
