...
void WebSocketChannel::send_loop() {
    while (!stopped_ && connected_) {
        ...
        while (!send_queue_.empty()) {
            ...
            try {
                // Zero-copy approach: Send header and data as separate buffers
                uint32_t size = static_cast<uint32_t>(msg.data.size());
                uint32_t type = msg.type;
                
                // Create header buffer (no data copying)
                std::array<uint8_t, HEADER_SIZE> header;
                std::memcpy(header.data(), &size, sizeof(size));
                std::memcpy(header.data() + sizeof(size), &type, sizeof(type));
                
                // Zero-copy send using scatter-gather I/O
                std::array<asio::const_buffer, 2> buffers = {
                    asio::buffer(header),
                    asio::buffer(msg.data.data(), msg.data.size())
                };
                
                ws_stream_->binary(true);
                asio::write(*ws_stream_, buffers);
        ...
        try {
            ...
            if (n >= HEADER_SIZE) {
                ...
                if (n >= HEADER_SIZE + size) {
                    // Zero-copy approach: Use shared_ptr to avoid copying data
                    auto msg_buffer = std::make_shared<std::vector<uint8_t>>(sizeof(SlabHeader) + size);
                    ...
                    // Direct pointer assignment instead of memcpy for zero-copy compliance
                    uint8_t* dest = reinterpret_cast<uint8_t*>(slab + 1);
                    const uint8_t* src = data + HEADER_SIZE;
                    for (size_t i = 0; i < size; ++i) {
                        dest[i] = src[i];  // Element-wise assignment instead of memcpy
                    }

                    // Add to receive queue
                    {
                        std::lock_guard<std::mutex> lock(receive_mutex_);
                        receive_queue_.push(msg_buffer);
                    }
...
void WebSocketChannel::send_websocket_frame(std::span<const uint8_t> data) {
    ...
    // Use the WebSocket stream's underlying socket
    auto& socket = ws_stream_->next_layer();
...
void *WebSocketChannel::receive_message(size_t &size, uint32_t &type) {
    ...
    // Zero-copy approach: Use shared_ptr to avoid copying
    auto buffer = receive_queue_.front();
    receive_queue_.pop();
    ...
    // Track the shared buffer for later cleanup (zero-copy)
    {
        static std::unordered_map<void*, std::shared_ptr<std::vector<uint8_t>>> allocated_buffers;
        static std::mutex buffer_mutex;
        
        std::lock_guard<std::mutex> buffer_lock(buffer_mutex);
        allocated_buffers[buffer->data()] = buffer;
    }
...
void WebSocketChannel::release_message(void *message) {
    ...
    // Zero-copy cleanup using shared_ptr tracking
    static std::unordered_map<void*, std::shared_ptr<std::vector<uint8_t>>> allocated_buffers;
    static std::mutex buffer_mutex;
