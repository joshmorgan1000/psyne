...
DataChannelMessage::DataChannelMessage(const void *data, size_t size,
                                       MessagePriority priority)
    : data_(static_cast<const uint8_t *>(data),
            static_cast<const uint8_t *>(data) + size),
      is_text_(false) {
    metadata_.priority = priority;
    metadata_.timestamp_us =
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::steady_clock::now().time_since_epoch())
            .count();
}

DataChannelMessage::DataChannelMessage(const std::string &text,
                                       MessagePriority priority)
    : data_(text.begin(), text.end()), is_text_(true) {
    metadata_.priority = priority;
    metadata_.timestamp_us =
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::steady_clock::now().time_since_epoch())
...
DataChannelMessage::DataChannelMessage(std::vector<uint8_t> binary_data,
                                       MessagePriority priority)
    : data_(std::move(binary_data)), is_text_(false) {
    metadata_.priority = priority;
    metadata_.timestamp_us =
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::steady_clock::now().time_since_epoch())
...
std::vector<uint8_t> DataChannelMessage::serialize() const {
    ...
    result.insert(result.end(), reinterpret_cast<const uint8_t *>(&data_size),
...
EnhancedDataChannel::EnhancedDataChannel(const DataChannelConfig &config)
    : config_(config), congestion_window_(config.initial_congestion_window),
    ...
    if (should_fragment_message(msg)) {
        ...
        for (const auto &fragment : fragments) {
            ...
            std::lock_guard<std::mutex> lock(
                send_queues_[static_cast<int>(priority)].mutex);
...
bool EnhancedDataChannel::send_text(const std::string &text,
                                    MessagePriority priority) {
    DataChannelMessage msg(text, priority);
    return send_message(msg);
}

bool EnhancedDataChannel::send_binary(const void *data, size_t size,
                                      MessagePriority priority) {
    DataChannelMessage msg(data, size, priority);
    return send_message(msg);
}

bool EnhancedDataChannel::send_binary(const std::vector<uint8_t> &data,
                                      MessagePriority priority) {
...
bool EnhancedDataChannel::should_fragment_message(
    const DataChannelMessage &message) const {
    return message.size() > config_.max_message_size;
}

std::vector<DataChannelMessage>
EnhancedDataChannel::fragment_message(const DataChannelMessage &message) {
    std::vector<DataChannelMessage> fragments;

    size_t fragment_size =
        config_.max_message_size - 100; // Leave room for headers
    uint16_t fragment_id = generate_fragment_id();
    uint16_t total_fragments =
        (message.size() + fragment_size - 1) / fragment_size;

    for (uint16_t i = 0; i < total_fragments; ++i) {
        size_t offset = i * fragment_size;
        size_t size = std::min(fragment_size, message.size() - offset);

        std::vector<uint8_t> fragment_data(message.data() + offset,
                                           message.data() + offset + size);
...
bool EnhancedDataChannel::should_retransmit(
    const PendingMessage &pending) const {
...
std::unique_ptr<EnhancedDataChannel>
DataChannelFactory::create_gaming_channel(const std::string &label) {
...
std::unique_ptr<EnhancedDataChannel>
DataChannelFactory::create_streaming_channel(const std::string &label) {
    DataChannelConfig config;
    config.label = label;
...
std::unique_ptr<EnhancedDataChannel>
DataChannelFactory::create_file_transfer_channel(const std::string &label) {
    DataChannelConfig config;
    config.label = label;
    config.delivery_mode = DeliveryMode::RELIABLE_ORDERED;
...
std::unique_ptr<EnhancedDataChannel>
DataChannelFactory::create_chat_channel(const std::string &label) {
    DataChannelConfig config;
    config.label = label;
    config.delivery_mode = DeliveryMode::RELIABLE_ORDERED;
    config.default_priority = MessagePriority::NORMAL;
...
std::unique_ptr<EnhancedDataChannel>
DataChannelFactory::create_control_channel(const std::string &label) {
    DataChannelConfig config;
    config.label = label;
    config.delivery_mode = DeliveryMode::RELIABLE_ORDERED;
...
std::unique_ptr<EnhancedDataChannel>
DataChannelFactory::create_custom_channel(const DataChannelConfig &config) {
    return std::make_unique<EnhancedDataChannel>(config);
}
...
void DataChannelMultiplexer::add_channel(
    const std::string &name, std::unique_ptr<EnhancedDataChannel> channel) {
    std::lock_guard<std::mutex> lock(channels_mutex_);
...
EnhancedDataChannel *
DataChannelMultiplexer::get_channel(const std::string &name) {
    std::lock_guard<std::mutex> lock(channels_mutex_);
...
bool DataChannelMultiplexer::broadcast_text(const std::string &text,
                                            MessagePriority priority) {
    std::lock_guard<std::mutex> lock(channels_mutex_);
...
bool DataChannelMultiplexer::broadcast_binary(const void *data, size_t size,
                                              MessagePriority priority) {
    std::lock_guard<std::mutex> lock(channels_mutex_);
...
bool DataChannelMultiplexer::broadcast_message(
...
void DataChannelMultiplexer::setup_channel_callbacks(
    const std::string &name, EnhancedDataChannel *channel) {
