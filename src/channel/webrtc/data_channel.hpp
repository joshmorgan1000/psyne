...
public:
    ...
    DataChannelMessage(const void* data, size_t size, MessagePriority priority = MessagePriority::NORMAL);
    DataChannelMessage(const std::string& text, MessagePriority priority = MessagePriority::NORMAL);
    DataChannelMessage(std::vector<uint8_t> binary_data, MessagePriority priority = MessagePriority::NORMAL);
    ...
    bool is_binary() const { return !is_text_; }
    ...
    // Binary data
    const std::vector<uint8_t>& binary_data() const { return data_; }
    ...
    // Priority
    MessagePriority priority() const { return metadata_.priority; }
    void set_priority(MessagePriority priority) { metadata_.priority = priority; }
...
public:
    ...
    // Configuration
    const DataChannelConfig& config() const { return config_; }
    void update_config(const DataChannelConfig& config);
    
    // Message sending
    bool send_message(const DataChannelMessage& message);
    bool send_text(const std::string& text, MessagePriority priority = MessagePriority::NORMAL);
    bool send_binary(const void* data, size_t size, MessagePriority priority = MessagePriority::NORMAL);
    bool send_binary(const std::vector<uint8_t>& data, MessagePriority priority = MessagePriority::NORMAL);
    
    // Non-blocking send
    bool try_send_message(const DataChannelMessage& message);
    bool try_send_text(const std::string& text, MessagePriority priority = MessagePriority::NORMAL);
    bool try_send_binary(const void* data, size_t size, MessagePriority priority = MessagePriority::NORMAL);
    ...
    // Quality of Service
    void set_priority_weights(const std::unordered_map<MessagePriority, double>& weights);
...
private:
    ...
    std::vector<DataChannelMessage> fragment_message(const DataChannelMessage& message);
    bool process_received_message(const DataChannelMessage& message);
    bool reassemble_fragments(const DataChannelMessage& fragment, DataChannelMessage& complete_message);
    
    // Reliability
    void handle_acknowledgment(uint64_t sequence_number, std::chrono::steady_clock::time_point ack_time);
...
public:
    ...
    static std::unique_ptr<EnhancedDataChannel> create_gaming_channel(const std::string& label = "gaming");
    
    // Streaming profile - high throughput, ordered delivery
    static std::unique_ptr<EnhancedDataChannel> create_streaming_channel(const std::string& label = "streaming");
    
    // File transfer profile - reliable, large messages
    static std::unique_ptr<EnhancedDataChannel> create_file_transfer_channel(const std::string& label = "files");
    
    // Chat profile - reliable text messaging
    static std::unique_ptr<EnhancedDataChannel> create_chat_channel(const std::string& label = "chat");
    
    // Control profile - critical, low latency, reliable
    static std::unique_ptr<EnhancedDataChannel> create_control_channel(const std::string& label = "control");
    
    // Custom profile
    static std::unique_ptr<EnhancedDataChannel> create_custom_channel(const DataChannelConfig& config);
};
...
public:
    ...
    // Channel management
    void add_channel(const std::string& name, std::unique_ptr<EnhancedDataChannel> channel);
    ...
    bool broadcast_text(const std::string& text, MessagePriority priority = MessagePriority::NORMAL);
    bool broadcast_binary(const void* data, size_t size, MessagePriority priority = MessagePriority::NORMAL);
    bool broadcast_message(const DataChannelMessage& message);
    
    // Selective operations
    bool send_to_channels(const std::vector<std::string>& channel_names, const DataChannelMessage& message);
    
    // Aggregated statistics
    DataChannelStats get_aggregated_stats() const;
    std::unordered_map<std::string, DataChannelStats> get_per_channel_stats() const;
    
    // Event handling
    std::function<void(const std::string& channel_name, const DataChannelMessage&)> on_message;
    std::function<void(const std::string& channel_name, const std::string& error)> on_error;
    
private:
    std::unordered_map<std::string, std::unique_ptr<EnhancedDataChannel>> channels_;
    mutable std::mutex channels_mutex_;
    
    void setup_channel_callbacks(const std::string& name, EnhancedDataChannel* channel);
};
