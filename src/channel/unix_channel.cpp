...
UnixChannel::UnixChannel(const std::string &uri, size_t buffer_size,
    ...
    : ChannelImpl(uri, buffer_size, mode, type), is_server_(true),
      connected_(false), stopping_(false), socket_path_(socket_path) {
    // Remove existing socket file if it exists
    cleanup_socket_file();

    // Create acceptor for listening
    acceptor_ = std::make_unique<unix_socket::acceptor>(
        io_context_, unix_socket::endpoint(socket_path_));
    ...
    while (!connected_ && !stopping_) {
        ...
        if (std::chrono::steady_clock::now() - start >
            ...
            throw std::runtime_error(
                "Failed to connect to Unix socket: timeout");
        }
    }

    if (stopping_) {
        throw std::runtime_error(
            "Failed to connect to Unix socket: operation cancelled");
    }
}
...
uint32_t UnixChannel::reserve_write_slot(size_t size) noexcept {
    if (size == 0) {
        return BUFFER_FULL;
    }
    
    // Reserve space directly in ring buffer for zero-copy Unix domain socket
    auto& ring_buffer = get_ring_buffer();
    return ring_buffer.reserve_write_space(size);
}

void UnixChannel::notify_message_ready(uint32_t offset, size_t size) noexcept {
    if (size == 0) {
        return;
    }
    
    // Message is already in ring buffer at offset
    // Stream directly from ring buffer to Unix domain socket for zero-copy
    auto& ring_buffer = get_ring_buffer();
    auto data_span = ring_buffer.get_read_span(offset, size);
    
    // Prepare Unix frame header
    UnixFrameHeader header{};
    header.length = static_cast<uint32_t>(sizeof(UnixFrameHeader) + size);
    header.checksum = calculate_checksum(data_span.data(), data_span.size());
    header.type = 0;
    
    // Queue zero-copy Unix domain socket send
    asio::post(io_context_, [this, header, offset, size]() {
        start_zero_copy_unix_send(header, offset, size);
    });
}

std::span<uint8_t> UnixChannel::get_write_span(size_t size) noexcept {
...
std::span<const uint8_t> UnixChannel::buffer_span() const noexcept {
    auto& ring_buffer = get_ring_buffer();
    return ring_buffer.available_read_span();
}

void UnixChannel::advance_read_pointer(size_t size) noexcept {
    auto& ring_buffer = get_ring_buffer();
    ring_buffer.advance_read_pointer(size);
}

void UnixChannel::start_zero_copy_unix_send(const UnixFrameHeader& header, uint32_t offset, size_t size) {
    ...
    // Create scatter-gather buffer for efficient Unix socket send (header + data)
...
void *UnixChannel::receive_message(size_t &size, uint32_t &type) {
    ...
    // Wait for message
    incoming_cv_.wait(
        lock, [this] { return !incoming_messages_.empty() || stopping_; });
    ...
    const UnixFrameHeader *header =
        reinterpret_cast<const UnixFrameHeader *>(message.data());
...
void UnixChannel::start_accept() {
    ...
    acceptor_->async_accept(
        *client, [this, client](const boost::system::error_code &error) {
...
void UnixChannel::handle_accept(std::shared_ptr<unix_socket::socket> client,
    ...
    if (error || stopping_) {
        if (!stopping_) {
            std::cerr << "Unix socket accept error: " << error.message()
                      << std::endl;
        }
        return;
    }
...
void UnixChannel::handle_connect(const boost::system::error_code &error) {
    if (error || stopping_) {
        if (!stopping_) {
            std::cerr << "Unix socket connect error: " << error.message()
                      << std::endl;
        }
        return;
    }
    ...
    // Store active socket
    {
        std::lock_guard<std::mutex> lock(socket_mutex_);
        active_socket_ =
            std::shared_ptr<unix_socket::socket>(socket_.release());
    }
...
void UnixChannel::start_receive(std::shared_ptr<unix_socket::socket> sock) {
    ...
    asio::async_read(
        *sock, asio::buffer(header.get(), sizeof(UnixFrameHeader)),
        [this, sock, header](const boost::system::error_code &error,
                             size_t bytes_transferred) {
            handle_receive_header(sock, header, error, bytes_transferred);
        });
}

void UnixChannel::handle_receive_header(
    std::shared_ptr<unix_socket::socket> sock,
    std::shared_ptr<UnixFrameHeader> header,
    const boost::system::error_code &error, size_t bytes_transferred) {
    if (error || stopping_) {
        if (!stopping_) {
            std::cerr << "Unix socket receive header error: " << error.message()
                      << std::endl;
            connected_ = false;
        }
        return;
    ...
    // Validate header
    if (header->length < sizeof(UnixFrameHeader) ||
        header->length > 64 * 1024 * 1024) {
        std::cerr << "Unix socket invalid message length: " << header->length
                  << std::endl;
        start_receive(sock); // Try again
        return;
    }
    ...
    if (payload_size > 0) {
        asio::async_read(*sock,
                         asio::buffer(buffer->data() + sizeof(UnixFrameHeader),
                                      payload_size),
                         [this, sock, buffer, header_copy = *header](
                             const boost::system::error_code &error,
                             size_t bytes_transferred) {
                             handle_receive_payload(sock, buffer, header_copy,
                                                    error, bytes_transferred);
                         });
    } else {
        // Header-only message
        handle_receive_payload(sock, buffer, *header,
                               boost::system::error_code{}, 0);
    }
}

void UnixChannel::handle_receive_payload(
    std::shared_ptr<unix_socket::socket> sock,
    std::shared_ptr<std::vector<uint8_t>> buffer, const UnixFrameHeader &header,
    const boost::system::error_code &error, size_t bytes_transferred) {
    if (error || stopping_) {
        if (!stopping_) {
            std::cerr << "Unix socket receive payload error: "
    ...
    uint64_t calculated_checksum =
        calculate_checksum(payload, expected_payload);
...
void UnixChannel::start_send(std::shared_ptr<unix_socket::socket> sock) {
    ...
    outgoing_cv_.wait(
        lock, [this] { return !outgoing_messages_.empty() || stopping_; });
    ...
    auto buffer = std::make_shared<std::vector<uint8_t>>(
        std::move(outgoing_messages_.front()));
    outgoing_messages_.pop();
    lock.unlock();

    // Send asynchronously
    asio::async_write(
        *sock, asio::buffer(*buffer),
...
void UnixChannel::handle_send(std::shared_ptr<unix_socket::socket> sock,
    ...
    if (error || stopping_) {
        if (!stopping_) {
            std::cerr << "Unix socket send error: " << error.message()
                      << std::endl;
            connected_ = false;
        }
        return;
