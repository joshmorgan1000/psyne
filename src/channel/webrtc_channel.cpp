...
public:
    explicit WebSocketSignalingTransport(const std::string& uri) : server_uri_(uri) {}
    ...
    void send_message(const SignalingMessage& message) override {
        ...
        std::cout << "Sending signaling message: " << static_cast<int>(message.type) 
                  << " to peer " << message.peer_id << std::endl;
    }
...
public:
    explicit SimpleDataChannel(const std::string& label, const RTCDataChannelConfig& config)
...
public:
    explicit SimplePeerConnection(const WebRTCConfig& config) : config_(config) {}
    ...
    void set_local_description(const RTCSessionDescription& desc) override {
        ...
        std::cout << "Set local description: " << desc.sdp.substr(0, 50) << "..." << std::endl;
    }
    
    void set_remote_description(const RTCSessionDescription& desc) override {
        remote_description_ = desc;
        std::cout << "Set remote description: " << desc.sdp.substr(0, 50) << "..." << std::endl;
    ...
    void add_ice_candidate(const RTCIceCandidate& candidate) override {
        ...
        std::cout << "Added ICE candidate: " << candidate.candidate << std::endl;
    }
    
    std::shared_ptr<RTCDataChannel> create_data_channel(
    ...
    void close() override {
        ...
        for (auto& channel : data_channels_) {
            if (auto simple_channel = std::dynamic_pointer_cast<SimpleDataChannel>(channel)) {
    ...
    void simulate_peer_connection(std::shared_ptr<SimplePeerConnection> peer) {
        ...
        if (!data_channels_.empty() && !peer->data_channels_.empty()) {
            auto our_channel = std::dynamic_pointer_cast<SimpleDataChannel>(data_channels_[0]);
            auto peer_channel = std::dynamic_pointer_cast<SimpleDataChannel>(peer->data_channels_[0]);
...
private:
    ...
    std::string generate_simple_sdp(bool is_offer) {
        ...
        sdp << "v=0\r\n";
        sdp << "o=- " << std::chrono::system_clock::now().time_since_epoch().count() 
            << " 2 IN IP4 127.0.0.1\r\n";
        sdp << "s=-\r\n";
        sdp << "t=0 0\r\n";
...
WebRTCChannel::WebRTCChannel(const std::string& uri, size_t buffer_size,
    ...
    , signaling_server_uri_(signaling_server_uri)
    , is_offerer_(true)
...
WebRTCChannel::WebRTCChannel(const std::string& uri, size_t buffer_size,
    ...
    : ChannelImpl(uri, buffer_size, mode, type)
    , webrtc_config_(config)
    , signaling_transport_(signaling_transport)
    , is_offerer_(false)
    , compression_manager_(compression_config) {
...
void WebRTCChannel::parse_webrtc_uri(const std::string& uri) {
...
void WebRTCChannel::initialize_peer_connection() {
    peer_connection_ = std::make_unique<SimplePeerConnection>(webrtc_config_);
    
    // Set up event handlers
    peer_connection_->on_connection_state_change = [this](RTCPeerConnectionState state) {
        on_connection_state_change(state);
    };
    
    peer_connection_->on_ice_candidate = [this](const RTCIceCandidate& candidate) {
        handle_ice_candidate(candidate);
    };
    
    peer_connection_->on_local_description = [this](const RTCSessionDescription& desc) {
        handle_session_description(desc);
    };
    
    peer_connection_->on_data_channel = [this](std::shared_ptr<RTCDataChannel> channel) {
        data_channel_ = channel;
        setup_data_channel();
    };
}

void WebRTCChannel::setup_signaling() {
    if (!signaling_transport_) {
        signaling_transport_ = std::make_shared<WebSocketSignalingTransport>(signaling_server_uri_);
    }
...
void WebRTCChannel::setup_data_channel() {
...
void WebRTCChannel::on_data_channel_open() {
    ...
    std::cout << "WebRTC data channel opened for peer: " << peer_id_ << std::endl;
}

void WebRTCChannel::on_data_channel_close() {
    connected_.store(false);
    std::cout << "WebRTC data channel closed for peer: " << peer_id_ << std::endl;
}

void WebRTCChannel::on_data_channel_message(const void* data, size_t size) {
...
void WebRTCChannel::on_connection_state_change(RTCPeerConnectionState state) {
    std::cout << "WebRTC connection state changed to: " << static_cast<int>(state) << std::endl;
}

void WebRTCChannel::handle_signaling_message(const SignalingMessage& message) {
    switch (message.type) {
...
void WebRTCChannel::handle_ice_candidate(const RTCIceCandidate& candidate) {
...
void WebRTCChannel::handle_session_description(const RTCSessionDescription& desc) {
    SignalingMessage message;
    message.type = (desc.type == RTCSessionDescription::Offer) ? 
                   SignalingMessage::Offer : SignalingMessage::Answer;
    message.peer_id = peer_id_;
...
void WebRTCChannel::start_signaling_loop() {
...
void WebRTCChannel::start_connection_monitoring() {
...
void* WebRTCChannel::reserve_space(size_t size) {
    if (!connected_.load()) {
        return nullptr;
    }
...
void WebRTCChannel::commit_message(void* handle) {
    if (!data_channel_ || !data_channel_->is_open()) {
        return;
    }
    
    // Find the buffer and send it
    std::lock_guard<std::mutex> lock(message_mutex_);
    if (!outgoing_buffers_.empty()) {
        auto& buffer = outgoing_buffers_.front();
        
        // For simplicity, assume message size is stored at the beginning
        size_t size = *reinterpret_cast<size_t*>(buffer.get());
        uint32_t type_id = *reinterpret_cast<uint32_t*>(buffer.get() + sizeof(size_t));
        
        auto serialized = serialize_message(buffer.get() + sizeof(size_t) + sizeof(uint32_t), 
                                          size - sizeof(size_t) - sizeof(uint32_t), type_id);
        
        data_channel_->send(serialized.data(), serialized.size());
...
void* WebRTCChannel::receive_message(size_t& size, uint32_t& type) {
...
void WebRTCChannel::release_message(void* handle) {
    // For this simple implementation, messages are managed automatically
}

RTCPeerConnectionState WebRTCChannel::connection_state() const {
    return peer_connection_ ? peer_connection_->connection_state() : RTCPeerConnectionState::New;
}

RTCIceConnectionState WebRTCChannel::ice_connection_state() const {
    return peer_connection_ ? peer_connection_->ice_connection_state() : RTCIceConnectionState::New;
}

bool WebRTCChannel::is_connected() const {
    return connected_.load();
}

WebRTCChannel::ConnectionStats WebRTCChannel::get_connection_stats() const {
    std::lock_guard<std::mutex> lock(stats_mutex_);
    return stats_;
}

std::vector<uint8_t> WebRTCChannel::serialize_message(const void* data, size_t size, uint32_t type_id) {
    ...
    serialized.insert(serialized.end(),
                      reinterpret_cast<const uint8_t*>(&type_id),
...
void WebRTCChannel::deserialize_message(const void* data, size_t total_size, MessageHandle& handle) {
    ...
    handle.type_id = *reinterpret_cast<const uint32_t*>(bytes + sizeof(size_t));
...
std::unique_ptr<WebRTCChannel> create_webrtc_channel(
    ...
    return std::make_unique<WebRTCChannel>(
        uri, buffer_size, mode, type, config, signaling_server_uri, compression_config
    );
}
