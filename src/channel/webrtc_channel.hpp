...
public:
    ...
    virtual std::shared_ptr<RTCDataChannel> create_data_channel(
 ...
 * - webrtc://localhost:8080/game-room-1 - Join room "game-room-1" via localhost:8080
 */
...
public:
     ...
     */
    WebRTCChannel(const std::string& uri, size_t buffer_size,
    ...
    void set_peer_id(const std::string& peer_id) { peer_id_ = peer_id; }
...
private:
    ...
    // Utility methods
    std::vector<uint8_t> serialize_message(const void* data, size_t size, uint32_t type_id);
    void deserialize_message(const void* data, size_t size, MessageHandle& handle);
};
