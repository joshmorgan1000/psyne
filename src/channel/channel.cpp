...
std::unique_ptr<ChannelImpl> create_ring_buffer_channel(
    const std::string& uri, size_t buffer_size, ChannelMode mode, ChannelType type, bool enable_metrics) {
    return std::make_unique<ChannelImplT<RingBufferType>>(uri, buffer_size, mode, type, enable_metrics);
}
...
std::unique_ptr<Channel> Channel::create(
    ...
    if (scheme == "memory") {
        ...
        impl = detail::create_tcp_channel(uri, buffer_size, mode, type, compression_config);
    } else if (scheme == "unix") {
        ...
        if (is_server) {
            impl = std::make_unique<detail::UnixChannel>(uri, buffer_size, mode, type, socket_path);
        } else {
