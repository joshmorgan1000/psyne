...
static_assert(sizeof(UDPMulticastHeader) == 40, "UDPMulticastHeader must be 40 bytes");
...
public:
     ...
     */
    UDPMulticastChannel(const std::string& uri, size_t buffer_size,
    ...
    MulticastStats get_multicast_stats() const { return stats_; }
...
private:
    ...
    // Async operations
    void start_receive();
    void handle_receive(const boost::system::error_code& error, size_t bytes_transferred, udp::endpoint sender_endpoint);
    void start_send();
    ...
    bool validate_message(const UDPMulticastHeader& header, const uint8_t* payload, size_t payload_size);
...
inline std::unique_ptr<ChannelImpl> create_multicast_publisher(
    ...
    return create_udp_multicast_channel(uri, buffer_size, ChannelMode::SPMC, 
                                       ChannelType::MultiType, MulticastRole::Publisher, 
                                       compression_config);
}
 ...
 */
inline std::unique_ptr<ChannelImpl> create_multicast_subscriber(
    const std::string& multicast_address, uint16_t port,
    size_t buffer_size = 1024 * 1024,
    const std::string& interface_address = "") {
    ...
    std::string uri = "udp://" + multicast_address + ":" + std::to_string(port);
    return create_udp_multicast_channel(uri, buffer_size, ChannelMode::SPMC, 
                                       ChannelType::MultiType, MulticastRole::Subscriber, 
                                       {}, interface_address);
}
