...
std::optional<SPSCRingBuffer::WriteHandle> SPSCRingBuffer::reserve(size_t size) {
...
std::optional<SPSCRingBuffer::ReadHandle> SPSCRingBuffer::read() {
    ...
    auto* header = reinterpret_cast<SlabHeader*>(buffer_ + (current_read & mask_));
...
std::optional<MPSCRingBuffer::WriteHandle> MPSCRingBuffer::reserve(size_t size) {
    const size_t total_size = align_up(sizeof(SlabHeader) + size);
...
std::optional<SPMCRingBuffer::WriteHandle> SPMCRingBuffer::reserve(size_t size) {
    const size_t total_size = align_up(sizeof(SlabHeader) + size);
    size_t current_write = write_pos_;
...
std::optional<MPMCRingBuffer::WriteHandle> MPMCRingBuffer::reserve(size_t size) {
    const size_t total_size = align_up(sizeof(SlabHeader) + size);
    size_t current_write = write_pos_.load(std::memory_order_relaxed);
    size_t next_write;
    ...
    auto* header = reinterpret_cast<SlabHeader*>(buffer_ + (current_write & mask_));
    header->len = 0;  // Mark as not ready
