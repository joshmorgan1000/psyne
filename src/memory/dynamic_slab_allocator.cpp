...
Slab::Slab(size_t size)
    : memory_(static_cast<uint8_t*>(std::aligned_alloc(4096, size)), &std::free)
    , size_(size) {
...
DynamicSlabStats DynamicSlabAllocator::get_stats() const {
    ...
    for (const auto& slab : slabs_) {
        ...
        if (stats.smallest_slab_size == 0 || stats.smallest_slab_size > slab->size()) {
            stats.smallest_slab_size = slab->size();
        }
    }
    
    stats.usage_ratio = stats.total_capacity > 0 ? 
...
void DynamicSlabAllocator::grow_slabs() {
    ...
    std::cout << "[DynamicSlabAllocator] Grew to " << slabs_.size() 
              << " slabs, new slab size: " << (new_size / (1024 * 1024)) << " MB" << std::endl;
}

void DynamicSlabAllocator::shrink_slabs() {
    // Find and remove empty slabs (keep at least one)
    auto it = std::remove_if(slabs_.begin() + 1, slabs_.end(),
...
size_t DynamicSlabAllocator::calculate_next_slab_size() const {
    ...
    next_size = std::clamp(next_size, config_.min_slab_size, config_.max_slab_size);
...
bool DynamicSlabAllocator::should_grow() const {
    ...
    double usage_ratio = total_capacity > 0 ? 
        static_cast<double>(total_used) / total_capacity : 1.0;
...
thread_local std::unique_ptr<DynamicSlabAllocator> ThreadLocalSlabAllocator::tls_allocator_;

ThreadLocalSlabAllocator::ThreadLocalSlabAllocator(const DynamicSlabConfig& config)
    : config_(config) {
}
