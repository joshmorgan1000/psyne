...
// Platform-specific aligned allocation (duplicated for now - should be moved to
// common header)
static void *aligned_alloc_portable(size_t alignment, size_t size) {
    ...
    void *ptr = nullptr;
    // posix_memalign requires alignment to be power of 2 and multiple of
    // sizeof(void*)
    ...
    if (result != 0) {
        fprintf(stderr,
                "posix_memalign failed in ring_buffer_zero_copy: "
                "alignment=%zu, size=%zu, error=%d\n",
...
SPSCRingBuffer::SPSCRingBuffer(size_t size)
    : buffer_size_(next_power_of_2(size)), mask_(buffer_size_ - 1), head_(0),
      tail_(0) {
