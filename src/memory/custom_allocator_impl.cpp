...
public:
    ...
    void* allocate(size_t size, AllocFlags flags) {
        ...
        // Handle alignment
        size_t alignment = 1;
        if (static_cast<uint32_t>(flags) & static_cast<uint32_t>(AllocFlags::Aligned64)) {
            alignment = 64;
        } else if (static_cast<uint32_t>(flags) & static_cast<uint32_t>(AllocFlags::Aligned128)) {
            alignment = 128;
        } else if (static_cast<uint32_t>(flags) & static_cast<uint32_t>(AllocFlags::Aligned256)) {
            alignment = 256;
        }
        
        // Try huge pages if requested and available
        if ((static_cast<uint32_t>(flags) & static_cast<uint32_t>(AllocFlags::HugePage)) && 
        ...
        if (ptr) {
            ...
            if (static_cast<uint32_t>(flags) & static_cast<uint32_t>(AllocFlags::Zeroed)) {
                memset(ptr, 0, size);
            }
...
void* allocate_tensor(size_t size) {
    return CustomAllocator::instance().allocate(size, 
        AllocFlags::Aligned64 | AllocFlags::Zeroed);
}
