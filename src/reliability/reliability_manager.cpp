...
public:
    Impl(Channel &channel, const ReliabilityConfig &config)
        : channel_(channel), config_(config),
          ...
          heartbeat_manager_(
              std::make_unique<HeartbeatManager>(channel, config)),
          replay_buffer_(
              std::make_unique<ReplayBuffer>(config.replay_buffer_size)),
          running_(false) {}
...
ReliabilityManager::ReliabilityManager(Channel &channel,
