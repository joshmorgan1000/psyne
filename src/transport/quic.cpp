...
bool QUICStream::try_receive(void *buffer, size_t buffer_size,
...
bool QUICConnection::connect(const std::string &remote_address,
...
QUICConnection::create_stream(QUICStreamDirection direction) {
    if (!is_connected())
        return nullptr;
...
std::shared_ptr<QUICStream> QUICConnection::accept_stream() {
    ...
    stream_cv_.wait(lock, [this]() {
        return !incoming_streams_.empty() ||
...
void QUICConnection::set_stream_callback(
...
size_t QUICChannel::receive(void *buffer, size_t buffer_size,
                            uint32_t *type_id) {
