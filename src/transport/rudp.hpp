...
struct RUDPPacket {
    ...
    RUDPPacket() = default;
    RUDPPacket(RUDPPacketType type, uint32_t seq, const void *data = nullptr,
               size_t size = 0);
...
struct RUDPConfig {
    ...
    bool enable_fast_retransmit =
        true; ///< Enable fast retransmit on 3 duplicate ACKs
...
public:
     ...
     */
    RUDPConnectionState get_state() const {
        return state_;
    }
     ...
     */
    void set_config(const RUDPConfig &config) {
        config_ = config;
    }
    ...
    double get_rtt_ms() const {
        return rtt_estimate_ms_;
    }

    /**
     * @brief Get congestion window size
     */
    uint32_t get_congestion_window() const {
...
protected:
    ...
    bool establish_connection(const std::string &remote_addr,
...
private:
    ...
    // UDP socket (simplified - in real implementation would use actual UDP
    // socket)
...
public:
    ...
    void set_connection_handler(
        std::function<void(std::unique_ptr<RUDPChannel>)> handler);
