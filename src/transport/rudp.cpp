...
public:
    Impl(const std::string& remote_address, uint16_t remote_port, const RUDPConfig& config)
        : remote_address_(remote_address), remote_port_(remote_port), config_(config),
          buffer_(1024 * 1024), write_pos_(0), read_pos_(0) {
...
RUDPChannel::RUDPChannel(const std::string& remote_address, uint16_t remote_port, const RUDPConfig& config)
