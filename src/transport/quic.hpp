 ...
 */
enum class QUICStreamDirection {
    BIDIRECTIONAL,
    UNIDIRECTIONAL
};
...
public:
    ...
    QUICStreamState state() const { return state_; }
...
public:
    ...
    // Stream management
    std::shared_ptr<QUICStream> create_stream(QUICStreamDirection direction = QUICStreamDirection::BIDIRECTIONAL);
    ...
    bool is_connected() const { return state_ == QUICConnectionState::ESTABLISHED; }
    ...
    void migrate_to_path(const std::string& new_local_address, const std::string& new_remote_address);
    
    // Statistics
    QUICStats get_stats() const;
    
    // Event callbacks
    void set_stream_callback(std::function<void(std::shared_ptr<QUICStream>)> callback);
    void set_connection_close_callback(std::function<void(uint64_t error_code, const std::string& reason)> callback);
...
protected:
    ...
    // Stream management
    void handle_stream_frame(uint64_t stream_id, const std::vector<uint8_t>& data, bool fin);
    void send_stream_frame(uint64_t stream_id, const std::vector<uint8_t>& data, bool fin = false);
...
private:
    ...
    // Loss detection
    std::unordered_map<uint64_t, std::chrono::steady_clock::time_point> sent_packets_;
...
public:
     ...
     */
    void set_connection_handler(std::function<void(std::shared_ptr<QUICConnection>)> handler);
...
public:
    ...
    std::shared_ptr<QUICConnection> connection() const { return connection_; }
    std::shared_ptr<QUICStream> stream() const { return stream_; }
 ...
 */
std::shared_ptr<QUICConnection> create_quic_client(
 ...
 */
std::unique_ptr<QUICServer> create_quic_server(
    uint16_t port,
    const QUICConfig& config = {});
 ...
 */
std::unique_ptr<QUICChannel> create_quic_channel(
    const std::string& remote_address,
    uint16_t remote_port,
    const QUICConfig& config = {});
