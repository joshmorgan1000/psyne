...
bool ArrowRecordBatchMessage::SetRecordBatch(
    ...
    // Write using IPC format
    auto writer_result =
        arrow::ipc::MakeStreamWriter(buffer_output_stream, batch->schema());
    if (!writer_result.ok())
        return false;
...
ArrowRecordBatchMessage::GetRecordBatch() const {
    if (cached_batch_) {
...
ArrowConverter::FloatVectorToArrow(const FloatVector &vec) {
    // Create builder
...
FloatVector
ArrowConverter::ArrowToFloatVector(const std::shared_ptr<arrow::Array> &array,
                                   Channel &channel) {
...
std::shared_ptr<arrow::Array>
ArrowConverter::ByteVectorToArrow(const ByteVector &vec) {
    // Create binary array
...
std::shared_ptr<arrow::Table> ArrowChannel::ReceiveTable(int expected_batches) {
    ...
    while (expected_batches < 0 ||
           batches.size() < static_cast<size_t>(expected_batches)) {
    ...
    while (running_) {
        ...
        if (output_.SendBatch(output_batch)) {
            ...
            for (int i = 0; i < output_batch->num_columns(); ++i) {
                batch_size +=
                    output_batch->column(i)->data()->buffers[1]->size();
            }
            ...
            stats_.avg_latency_ms =
                (stats_.avg_latency_ms * (stats_.batches_processed - 1) +
                 latency) /
                stats_.batches_processed;
        }
    }
...
class ArrowFlightAdapter::FlightServerImpl
...
void ArrowFlightAdapter::RegisterStream(
