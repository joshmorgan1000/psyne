...
public:
    ...
    FloatVectorToArrow(const FloatVector &vec);
     ...
     */
    static FloatVector
    ArrowToFloatVector(const std::shared_ptr<arrow::Array> &array,
                       Channel &channel);
     ...
     */
    static std::shared_ptr<arrow::Array>
    ByteVectorToArrow(const ByteVector &vec);
...
public:
     ...
     */
    ArrowPipeline(const std::string &input_uri, const std::string &output_uri,
...
std::shared_ptr<T> ArrowConverter::CreateArrowView(void *data, int64_t length) {
    ...
    // Create array
    auto data_array =
        arrow::ArrayData::Make(arrow::TypeTraits<ValueType>::type_singleton(),
