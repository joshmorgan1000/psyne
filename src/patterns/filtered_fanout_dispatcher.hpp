...
template<typename T>
concept MessagePredicate = requires(T t, const void* data, size_t size, uint32_t type) {
    { t(data, size, type) } -> std::convertible_to<bool>;
};

template<typename T>
concept MessageHandler = requires(T t, const void* data, size_t size, uint32_t type, 
...
struct SenderInfo {
    ...
    std::vector<uint8_t> serialize() const {
        ...
        data.reserve(sizeof(uint64_t) + sizeof(uint32_t) + reply_channel_uri.size() + 1);
        
        // Write correlation_id
        const uint8_t* corr_ptr = reinterpret_cast<const uint8_t*>(&correlation_id);
        data.insert(data.end(), corr_ptr, corr_ptr + sizeof(correlation_id));
        
        // Write sender_id
        const uint8_t* sender_ptr = reinterpret_cast<const uint8_t*>(&sender_id);
        data.insert(data.end(), sender_ptr, sender_ptr + sizeof(sender_id));
        
        // Write URI as null-terminated string
        data.insert(data.end(), reply_channel_uri.begin(), reply_channel_uri.end());
        data.push_back(0);
...
public:
    ResponseAggregator(size_t expected_responses, 
                      std::function<void(std::vector<std::span<const uint8_t>>)> final_handler)
        : expected_(expected_responses), 
          final_handler_(std::move(final_handler)),
          timeout_ms_(5000) {}
...
public:
    ...
    public:
        ...
        const P& predicate() const { return predicate_; }
    ...
    public:
        ...
        virtual bool matches(const void* data, size_t size, uint32_t type) const = 0;
        virtual void process(const void* data, size_t size, uint32_t type,
    ...
    public:
        ...
        bool matches(const void* data, size_t size, uint32_t type) const override {
            return route_.matches(data, size, type);
        }
        
        void process(const void* data, size_t size, uint32_t type,
                    std::function<void(std::span<const uint8_t>)> reply) const override {
    ...
    void add_typed_route(const std::string& name,
        ...
        auto pred_wrapper = [predicate = std::forward<Predicate>(predicate)](
            ...
            if (type != MessageType::message_type) return false;
            const auto* msg = reinterpret_cast<const MessageType*>(data);
            return predicate(*msg);
        };
        
        auto handler_wrapper = [handler = std::forward<Handler>(handler)](
            const void* data, size_t size, uint32_t type, 
            std::function<void(std::span<const uint8_t>)> reply) {
            const auto* msg = reinterpret_cast<const MessageType*>(data);
            
            // Handler can return a response or void
            if constexpr (std::is_invocable_r_v<void, Handler, const MessageType&>) {
                handler(*msg);
                // Send empty response
...
private:
        ...
        while (running_) {
            ...
            if (size >= sizeof(uint32_t)) {
                ...
                if (magic == 0xDEADBEEF) { // Magic number for routable messages
                    // Skip magic and size
                    const uint8_t* header_data = static_cast<const uint8_t*>(msg_data) + sizeof(uint32_t);
                    uint32_t header_len = *reinterpret_cast<const uint32_t*>(header_data);
                    header_data += sizeof(uint32_t);
                    
                    sender_info = SenderInfo::deserialize(header_data, header_len);
                    header_size = sizeof(uint32_t) * 2 + header_len;
                }
            }
            
            // Adjust data pointer and size to skip header
            const void* actual_msg_data = static_cast<const uint8_t*>(msg_data) + header_size;
            ...
            std::vector<std::unique_ptr<RouteBase>*> matching_routes;
            {
                ...
                for (auto& route : routes_) {
                    if (route->matches(actual_msg_data, actual_msg_size, type)) {
            ...
            if (matching_routes.empty()) {
                ...
                if (!sender_info.reply_channel_uri.empty()) {
                    aggregator = std::make_shared<ResponseAggregator>(
                        ...
                        [this, sender_info](std::vector<std::span<const uint8_t>> responses) {
                ...
                for (auto* route_ptr : matching_routes) {
                    ...
                    auto msg_copy = std::make_shared<std::vector<uint8_t>>(
                        ...
                        static_cast<const uint8_t*>(actual_msg_data) + actual_msg_size
                    );
    ...
    void send_aggregated_response(const SenderInfo& sender_info,
        ...
        aggregated.insert(aggregated.end(), count_ptr, count_ptr + sizeof(count));
        
        // Each response with size prefix
        for (const auto& response : responses) {
            uint32_t resp_size = static_cast<uint32_t>(response.size());
            const uint8_t* size_ptr = reinterpret_cast<const uint8_t*>(&resp_size);
            aggregated.insert(aggregated.end(), size_ptr, size_ptr + sizeof(resp_size));
            aggregated.insert(aggregated.end(), response.begin(), response.end());
        }
        
        send_response_to_sender(sender_info, aggregated.data(), aggregated.size());
    }
...
public:
    static std::vector<uint8_t> build_with_reply_info(const T& message, 
                                                      const std::string& reply_uri,
                                                      uint64_t correlation_id = 0) {
        SenderInfo info;
        info.reply_channel_uri = reply_uri;
        info.correlation_id = correlation_id;
        info.sender_id = std::hash<std::thread::id>{}(std::this_thread::get_id());
        ...
        result.insert(result.end(), msg_ptr, msg_ptr + message.calculate_size());
