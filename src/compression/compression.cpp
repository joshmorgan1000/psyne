...
size_t SimpleCompressor::compress(const void* src, size_t src_size, 
...
size_t SimpleCompressor::decompress(const void* src, size_t src_size,
                                   void* dst, size_t dst_capacity) {
    if (!src || !dst || src_size == 0 || dst_capacity == 0) {
        return 0;
    }
...
size_t SimpleCompressor::compress_rle(const uint8_t* src, size_t src_size, 
    ...
    if (dst_capacity < 8) return 0; // Need space for header
    ...
    *reinterpret_cast<uint32_t*>(dst) = static_cast<uint32_t>(src_size);
    ...
    if (compressed_size >= src_size) {
        ...
        if (dst_capacity < src_size + 4) return 0;
        std::memcpy(dst_start + 4, src - (src_end - (src - src_size)), src_size);
...
size_t SimpleCompressor::decompress_rle(const uint8_t* src, size_t src_size,
    ...
    if (dst_capacity < original_size) return 0;
    ...
    while (src + 2 <= src_end && dst < dst_end) {
        ...
        if (dst + count > dst_end) break; // Prevent overflow
...
size_t CompressionManager::compress_message(const void* src, size_t src_size,
    ...
    size_t compressed_size = compressor_->compress(src, src_size, 
