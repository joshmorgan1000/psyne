...
EncryptedChannel::EncryptedChannel(std::shared_ptr<Channel> underlying,
    ...
    : underlying_(underlying), config_(config),
      encrypt_ctx_(nullptr), decrypt_ctx_(nullptr) {
...
std::vector<uint8_t> EncryptedChannel::Encrypt(const void* data, size_t size) {
    ...
    if (!EVP_EncryptInit_ex(encrypt_ctx_, cipher, nullptr, 
                           session_key_.data(), current_iv_.data())) {
        std::lock_guard<std::mutex> lock(stats_mutex_);
        stats_.encryption_errors++;
        return {};
    }
    ...
    std::vector<uint8_t> output(1 + iv_len + 1 + size + EVP_CIPHER_block_size(cipher) + tag_len);
    
    // Write IV length and IV
    output[0] = static_cast<uint8_t>(iv_len);
    std::memcpy(&output[1], current_iv_.data(), iv_len);
    ...
    
    {
        ...
        stats_.avg_encryption_time_us = 
            (stats_.avg_encryption_time_us * (stats_.messages_encrypted - 1) + duration) /
            stats_.messages_encrypted;
    }
...
std::vector<uint8_t> EncryptedChannel::Decrypt(const void* data, size_t size) {
    ...
    // Decrypt
    std::vector<uint8_t> plaintext(ciphertext_end + EVP_CIPHER_block_size(cipher));
    int len;
    int plaintext_len;
    ...
    
    {
        ...
        stats_.avg_decryption_time_us = 
            (stats_.avg_decryption_time_us * (stats_.messages_decrypted - 1) + duration) /
            stats_.messages_decrypted;
    }
...
void* EncryptedChannel::receive_raw_message(size_t& size, uint32_t& type) {
    ...
    void* encrypted_data = underlying_->receive_raw_message(encrypted_size, encrypted_type);
    ...
    // Decrypt
    auto decrypted = Decrypt(encrypted_data, encrypted_size);
    ...
    void* result = new uint8_t[decrypted.size()];
    std::memcpy(result, decrypted.data(), decrypted.size());
...
std::vector<uint8_t> GenerateRandomBytes(size_t size) {
    ...
    if (RAND_bytes(bytes.data(), size) != 1) {
        throw std::runtime_error("Failed to generate random bytes: " + GetOpenSSLError());
    }
    return bytes;
}
...
std::vector<uint8_t> CalculateHMAC(const std::vector<uint8_t>& key,
    ...
    if (!HMAC(EVP_sha256(), key.data(), key.size(),
              static_cast<const uint8_t*>(data), size,
              hmac.data(), &hmac_len)) {
        throw std::runtime_error("Failed to calculate HMAC: " + GetOpenSSLError());
    }
