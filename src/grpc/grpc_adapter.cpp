...
public:
    ...
    void Proceed() {
        if (status_ == CREATE) {
            ...
            size_t offset = 0;
            for (const auto &slice : slices) {
                std::memcpy(request_data.data() + offset, slice.begin(),
                            slice.size());
                offset += slice.size();
            }
...
GrpcAdapter::GrpcAdapter(std::shared_ptr<Channel> channel,
...
void GrpcAdapter::RegisterUnaryMethod(const std::string &method_name,
                                      MessageHandler handler) {
    handlers_[method_name] = handler;
}

void GrpcAdapter::RegisterServerStreamingMethod(const std::string &method_name,
                                                MessageHandler handler) {
    // For now, treat the same as unary - full implementation would handle
    // streaming
    handlers_[method_name] = handler;
}
...
grpc::Status GrpcClient::CallUnary(const std::string &method,
    ...
    grpc::Status status =
        stub_->UnaryCall(&context, method, request_buffer, &response_buffer);
...
grpc::Status GrpcClient::CallServerStreaming(
    const std::string &method, const void *request, size_t request_size,
    ...
    while (reader->Read(&response_buffer)) {
        ...
        for (const auto &slice : slices) {
            response_data.insert(response_data.end(), slice.begin(),
                                 slice.end());
        }
...
std::unique_ptr<GrpcAdapter>
CreateGrpcService(const std::string &service_name,
                  const std::string &server_address,
                  std::shared_ptr<Channel> request_channel,
                  std::shared_ptr<Channel> response_channel) {
    auto adapter =
        std::make_unique<GrpcAdapter>(request_channel, server_address);

    // Register a generic handler that forwards to Psyne channels
    auto handler = [request_channel,
                    response_channel](const void *request, size_t request_size,
                                      void *response, size_t &response_size) {
...
GrpcBridge::GetServiceChannel(const std::string &service_name) {
...
void GrpcBridge::Start() {
    ...
    if (!adapters_.empty() && !grpc_server_) {
        ...
        // Reflection service deferred to v2.0

        grpc_server_ = builder.BuildAndStart();
        std::cout << "GrpcBridge listening on " << config_.grpc_address
                  << std::endl;
    }
}
