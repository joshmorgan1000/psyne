...
public:
    using MessageHandler =
        std::function<void(const void *request, size_t request_size,
                           void *response, size_t &response_size)>;

    /**
     * @brief Create a gRPC adapter for a Psyne channel
     * @param channel The Psyne channel to adapt
     * @param server_address Address to bind the gRPC server (e.g.,
     * "0.0.0.0:50051")
     */
    GrpcAdapter(std::shared_ptr<Channel> channel,
     ...
     */
    void RegisterUnaryMethod(const std::string &method_name,
                             MessageHandler handler);

    /**
     * @brief Register a server streaming RPC method
     * @param method_name Full method name
     * @param handler Function to handle the RPC (called multiple times for
     * streaming)
     */
    void RegisterServerStreamingMethod(const std::string &method_name,
                                       MessageHandler handler);
...
public:
     ...
     * @return gRPC status
     */
    grpc::Status CallUnary(const std::string &method, const void *request,
     ...
     * @return gRPC status
     */
    grpc::Status CallServerStreaming(
...
public:
    static constexpr uint32_t message_type =
        1000; // Base type for protobuf messages
