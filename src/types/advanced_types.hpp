 ...
 * @brief Advanced message types for Psyne - complex numbers, ML tensors, and sparse matrices
 * 
...
public:
    ...
    void set_shape(const std::vector<uint32_t>& new_shape, Layout layout = Layout::Custom) {
        ...
        std::fill(header().shape + new_shape.size(), header().shape + MAX_DIMS, 0);
    ...
    float& operator()(uint32_t i0, uint32_t i1, uint32_t i2) {
        ...
        return data()[i0 * header().strides[0] + i1 * header().strides[1] + i2 * header().strides[2]];
    }
    ...
    void apply_softmax() {
        ...
        for (uint32_t batch = 0; batch < batch_size; ++batch) {
            ...
            float max_val = *std::max_element(batch_data, batch_data + last_dim);
