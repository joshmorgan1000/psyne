 ...
 * @brief Enhanced message types for Psyne - matrices, vectors, tensors, and specialized data types
 * 
 * This header provides advanced message types optimized for different use cases:
...
public:
    ...
    float* data() { return reinterpret_cast<float*>(Message<Matrix3x3f>::data()); }
    const float* data() const { return reinterpret_cast<const float*>(Message<Matrix3x3f>::data()); }
...
public:
    ...
    float* data() { return reinterpret_cast<float*>(Message<Vector4f>::data()); }
    const float* data() const { return reinterpret_cast<const float*>(Message<Vector4f>::data()); }
...
public:
    ...
    void resize(size_t new_size) {
        // Note: In real implementation, this would need to handle memory allocation
    ...
    void quantize_from(const float* values, size_t count) {
        ...
        for (size_t i = 0; i < count; ++i) {
            int32_t quantized = static_cast<int32_t>(std::round(values[i] / scale) + zero_point);
...
public:
    ...
    Header& header() { return *reinterpret_cast<Header*>(Message<UInt8Vector>::data()); }
    const Header& header() const { return *reinterpret_cast<const Header*>(Message<UInt8Vector>::data()); }
    
    uint8_t* data() { return reinterpret_cast<uint8_t*>(Message<UInt8Vector>::data()) + sizeof(Header); }
    const uint8_t* data() const { return reinterpret_cast<const uint8_t*>(Message<UInt8Vector>::data()) + sizeof(Header); }
    ...
    void dequantize_to(float* values, size_t count) const {
        ...
        for (size_t i = 0; i < copy_count; ++i) {
            values[i] = scale * (static_cast<int32_t>(data()[i]) - static_cast<int32_t>(zero_point));
        }
    }
