...
struct alignas(8) VariantHdr {
    uint32_t byteLen;          // payload size
    uint8_t  rank;             // 0 = scalar, 1 = vec, 2+ = tensor
    uint8_t  elemBytes;        // 1,2,4,8 …
    uint8_t  reserved[2]{};
};
...
struct VariantView {
    ...
    // scalar
    T value() const requires (Rank == 0) {
        static_assert(std::is_trivially_copyable_v<T>);
        T v; std::memcpy(&v, raw(), sizeof(T)); return v;
    }

    // 1-D span
    std::span<const T> span() const requires (Rank == 1) {
        return { reinterpret_cast<const T*>(raw()), count() };
    }

    // N-D extents stored after header (+ offset)
    std::array<std::size_t, Rank> shape() const requires (Rank >= 2) {
    ...
    const T* data() const requires (Rank >= 2) {
