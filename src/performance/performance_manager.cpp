...
public:
    ...
    std::string get_summary() const {
        ...
        ss << "  Prefetch enabled: "
           << (config_.enable_prefetching ? "Yes" : "No") << "\\n";
        ss << "  Huge pages enabled: "
           << (config_.enable_huge_pages ? "Yes" : "No") << "\\n";
        ss << "  NUMA binding enabled: "
           << (config_.enable_cpu_affinity ? "Yes" : "No") << "\\n";
        ss << "  Memory locking enabled: "
           << (config_.enable_simd ? "Yes" : "No") << "\\n";
        return ss.str();
    }
    ...
    std::vector<std::string> get_recommendations() const {
        ...
        recommendations.push_back(
            "Enable hardware prefetching for sequential access patterns");

        if (!config_.enable_prefetching) {
            recommendations.push_back("Consider enabling memory prefetching "
                                      "for better cache utilization");
        }

        if (!config_.enable_huge_pages) {
            recommendations.push_back(
                "Consider enabling huge pages for reduced TLB pressure");
        }
...
BenchmarkResult PerformanceManager::benchmark_channel(Channel &channel,
                                                      size_t message_size,
                                                      size_t num_messages) {
