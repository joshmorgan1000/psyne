...
protected:
    detail::ChannelImpl* impl() override { return nullptr; }
    const detail::ChannelImpl* impl() const override { return nullptr; }
...
ChannelMultiplexer::ChannelMultiplexer(std::shared_ptr<Channel> physical_channel,
                                     size_t max_logical_channels)
...
std::shared_ptr<Channel> ChannelMultiplexer::CreateLogicalChannel(uint8_t channel_id) {
    ...
    while (running_) {
        ...
        while (!send_queue_.empty()) {
            ...
            *reinterpret_cast<uint32_t*>(ptr) = msg.type;
            ptr += 4;
            *reinterpret_cast<uint32_t*>(ptr) = static_cast<uint32_t>(msg.data.size());
            ptr += 4;
...
void ChannelMultiplexer::ReceiveLoop() {
    while (running_) {
        ...
        if (!msg || msg->size() < 9) continue;  // Minimum header size
        ...
        if (msg->size() < 9 + size) continue;  // Invalid message
...
public:
    ...
    const std::string& uri() const override {
        static std::string uri = "demux://logical_" + std::to_string(channel_id_);
        return uri;
    }
    
    ChannelType type() const override { return ChannelType::MultiType; }
    ChannelMode mode() const override { return ChannelMode::MPMC; }
...
ChannelDemultiplexer::ChannelDemultiplexer(std::shared_ptr<Channel> physical_channel)
    : physical_channel_(physical_channel) {}
...
void ChannelDemultiplexer::RegisterHandler(uint8_t channel_id, MessageHandler handler) {
    std::lock_guard<std::mutex> lock(mutex_);
    handlers_[channel_id] = handler;
}

std::shared_ptr<Channel> ChannelDemultiplexer::GetLogicalChannel(uint8_t channel_id) {
    std::lock_guard<std::mutex> lock(mutex_);
...
ChannelPool::ChannelPool(const Config& config) : config_(config) {
    ...
    for (size_t i = 0; i < config_.initial_channels; ++i) {
        auto channel = create_channel(
            config_.channel_uri_prefix + std::to_string(i),
...
std::shared_ptr<Channel> ChannelPool::Acquire(int timeout_ms) {
    ...
    if (available_.empty()) {
        ...
        if (channels_.size() < config_.max_channels) {
            auto channel = create_channel(
                config_.channel_uri_prefix + std::to_string(channels_.size()),
                config_.channel_buffer_size);
            channels_.push_back(channel);
...
bool BondedChannel::Send(const void* data, size_t size, uint32_t type) {
    ...
    switch (mode_) {
        ...
        case BondingMode::LeastLoaded:
            ...
            channel_idx = std::distance(
                stats_.channel_sent.begin(),
            ...
            if (size > 64 * 1024 && channels_.size() > 1) {
                ...
                for (size_t i = 0; i < channels_.size(); ++i) {
                    size_t this_stripe = (i == channels_.size() - 1) ?
                        size - (stripe_size * i) : stripe_size;
                    ...
                    *reinterpret_cast<uint32_t*>(msg.data() + 4) = channels_.size();
...
std::unique_ptr<std::vector<uint8_t>> BondedChannel::Receive(uint32_t& type, int timeout_ms) {
...
BondedChannel::BondStats BondedChannel::GetStats() const {
    ...
    if (duration > 0) {
        stats_.aggregate_bandwidth_mbps = 
            (stats_.total_sent + stats_.total_received) / (duration * 1024 * 1024);
    }
