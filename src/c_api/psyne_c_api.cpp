...
static std::unordered_map<psyne_channel*, std::unique_ptr<psyne_channel>> g_channels;
...
psyne_error_t psyne_channel_create_compressed(
    ...
    try {
        ...
        if (compression) {
            cpp_compression.type = static_cast<compression::CompressionType>(compression->type);
            cpp_compression.level = compression->level;
            cpp_compression.min_size_threshold = compression->min_size_threshold;
...
void psyne_channel_destroy(psyne_channel_t* channel) {
    ...
    if (it != g_channels.end()) {
        if (it->second->callback_running) {
            ...
            if (it->second->callback_thread && it->second->callback_thread->joinable()) {
...
psyne_error_t psyne_message_reserve(psyne_channel_t* channel,
    ...
    try {
        ...
        msg_wrapper->send_msg = std::make_unique<ByteVector>(*channel->cpp_channel);
...
psyne_error_t psyne_message_receive_timeout(psyne_channel_t* channel,
    ...
    try {
        ...
        if (!msg) {
            return timeout_ms > 0 ? PSYNE_ERROR_TIMEOUT : PSYNE_ERROR_NO_MESSAGE;
        }
...
psyne_error_t psyne_channel_set_receive_callback(
    ...
    if (callback) {
        ...
        channel->callback_thread = std::make_unique<std::thread>([channel]() {
            while (channel->callback_running && !channel->cpp_channel->is_stopped()) {
                auto msg = channel->cpp_channel->receive<ByteVector>(
