 ...
 * support for:
 * - Unified memory
 * - GPUDirect for zero-copy operations
 * - Multi-GPU support
...
public:
    ...
    GPUBackend backend() const override { return GPUBackend::CUDA; }
     ...
     */
    std::unique_ptr<CudaBuffer> create_buffer_with_stream(
...
public:
     ...
     */
    const cudaDeviceProp& device_properties() const { return device_props_; }
