...
MetalBuffer::MetalBuffer(MTL::Buffer *buffer, BufferUsage usage,
                         MemoryAccess access)
    : buffer_(buffer), usage_(usage), access_(access), mapped_ptr_(nullptr),
      is_mapped_(false) {
    if (!buffer_) {
...
void MetalBuffer::unmap() {
    ...
    // On unified memory systems, we might need to ensure CPU writes are visible
    // to GPU
    if (access_ == MemoryAccess::Shared || access_ == MemoryAccess::Managed) {
        // For shared/managed storage mode, CPU and GPU automatically stay
        // coherent No explicit action needed on Apple Silicon
    }
...
MetalBufferFactory::MetalBufferFactory(MTL::Device *device) : device_(device) {
    if (!device_) {
...
MetalBufferFactory::create_buffer(size_t size, BufferUsage usage,
                                  MemoryAccess access) {
    if (size == 0) {
...
MTL::ResourceOptions
MetalBufferFactory::get_resource_options(BufferUsage usage,
...
MetalContext::MetalContext()
    : device_(nullptr), command_queue_(nullptr), default_library_(nullptr) {
    if (!initialize()) {
...
bool MetalContext::initialize() {
    ...
    std::cout << "Unified memory: " << (is_unified_memory() ? "Yes" : "No")
              << std::endl;
...
MetalContext::create_compute_pipeline(const std::string &function_name) {
    if (!default_library_) {
        throw std::runtime_error("No Metal library loaded");
    }

    NS::String *fn_name = NS::String::string(
        function_name.c_str(), NS::StringEncoding::UTF8StringEncoding);
...
MetalCommandBuffer::MetalCommandBuffer(MTL::CommandQueue *queue)
    : buffer_(nullptr), encoder_(nullptr) {
    if (!queue) {
